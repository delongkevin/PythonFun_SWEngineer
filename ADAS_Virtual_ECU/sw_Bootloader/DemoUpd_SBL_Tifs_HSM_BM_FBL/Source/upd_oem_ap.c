...
tFblResult ApplFblUpdOemInvalidateUpdater( void )
{
   ...
   uint8 AppDCLHeaderUuid_2951[16u]={0x47,0xbc,0x49,0xd0,0x77,0xfc,0x11,0xee,0x99,0x9d,0xfc,0x9a,0x85,0x04,0x13,0x15};
   tBlockDescriptor blockDescriptor;
   tFblResult retVal;
   blockDescriptor = FblLogicalBlockTable.logicalBlock[FBL_UPD_LBT_NR_OF_UPDATER];

   retVal = ApplFblClrModulePresence(&blockDescriptor);
   retVal = ApplFblClrApplPresence(&blockDescriptor);
   retVal |= ApplFblNvReadFingerprint((IO_PositionType)4u, &blockFingerprint);
   retVal |= ApplFblNvWriteFingerprint((IO_PositionType)0u, blockFingerprint);

   blockDescriptor = FblLogicalBlockTable.logicalBlock[0u];
   ApplFblIncProgAttempts(blockDescriptor);
   ApplFblNvWriteUuid((3u),&AppDCLHeaderUuid_2951[0u]);
   /* patch to erase trsc calibration to avoid A72 crash due to trailer calibration mismatch */
   /* erase A72 calibration data memory 0x6C80000 - 0x767FFFF */
   /* once this is officially implemented in bootloader to erase this memory along with A72 (ifs_Qnx) logical block, this patch can be removed here */
