...
tFblAddress ApplFblGetModuleHeaderAddress( vuint8 blockNr )
{
   ...
   if (FblLbtGetBlockDescriptorByNr(blockNr, &blockDescriptor) == kFblOk)
   {
      if (blockNr == FBL_LBT_MCU10_APPLICATION_LB_BLOCK_NUMBER)
      {
         ...
         moduleHeaderAddr = blockDescriptor.blockStartAddress + 0x200;
      }
      else
      {
         /* For all other blocks: */
    	  /*TODO: Make the change as needed refer if case*/
...
/* write Fingerprint and increment programming attempt counter for FBL */
   result |= ApplFblNvReadFingerprint((IO_PositionType)FBL_LBT_MCU10_APPLICATION_LB_BLOCK_NUMBER, &blockFingerprint);
   result |= ApplFblNvWriteFingerprint((IO_PositionType)FBL_LBT_FBL_BLOCK_NUMBER, blockFingerprint);

   blockDescriptor = FblLogicalBlockTable.logicalBlock[FBL_LBT_FBL_BLOCK_NUMBER];
...
tFblResult ApplFblIncProgCounts( tBlockDescriptor blockDescriptor )
{
   ...
   status = (tFblResult)ApplFblNvReadProgCounter((IO_PositionType)blockDescriptor.blockIndex, nvBuffer);
   ...
   if (status == kFblOk)
   {
      ...
      status = (tFblResult)ApplFblNvWriteProgCounter((IO_PositionType)blockDescriptor.blockIndex, nvBuffer);
   }
