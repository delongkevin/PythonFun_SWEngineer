...
void Can_EnableTransceivers(void)
{
  ...
  uint32 pin_level = 0U;
    ...
    /* Take MCU MCAN0 transceiver EN */
    regVal = 0x08050007U;
  CSL_REG32_WR(CSL_WKUP_CTRL_MMR0_CFG0_BASE + CSL_WKUP_CTRL_MMR_CFG0_PADCONFIG49, regVal);
  /* Set WAKEUP_GPIO0_1 Pin direction to output and drive High*/
  GPIOSetDirMode_v0(CSL_WKUP_GPIO0_BASE, 1U, GPIO_DIRECTION_OUTPUT);
  GPIOPinWrite_v0(CSL_WKUP_GPIO0_BASE, 1U, GPIO_PIN_HIGH);
  pin_level = GPIOPinRead_v0(CSL_WKUP_GPIO0_BASE, 1U);
//  while (GPIO_PIN_HIGH != pin_level)
//  {
//    pin_level = GPIOPinRead_v0(CSL_WKUP_GPIO0_BASE, 4U);
//  }

    /* Take MCU MCAN0 transceiver STB */
    regVal = 0x08050007U;
  CSL_REG32_WR(CSL_WKUP_CTRL_MMR0_CFG0_BASE + CSL_WKUP_CTRL_MMR_CFG0_PADCONFIG63, regVal);
  /* Set WAKEUP_GPIO0_15 Pin direction to output and drive High */
  GPIOSetDirMode_v0(CSL_WKUP_GPIO0_BASE, 15U, GPIO_DIRECTION_OUTPUT);
  GPIOPinWrite_v0(CSL_WKUP_GPIO0_BASE,15U, GPIO_PIN_HIGH);
  pin_level = GPIOPinRead_v0(CSL_WKUP_GPIO0_BASE, 15U);
//  while (GPIO_PIN_HIGH != pin_level)
//  {
//    pin_level = GPIOPinRead_v0(CSL_WKUP_GPIO0_BASE, 5U);
//  }
...
void BordPmConfig(void)
{
	...
	reqParam.messageType = (uint16_t) TISCI_MSG_BOARD_CONFIG_PM;
	reqParam.flags = (uint32_t) TISCI_MSG_FLAG_AOP;
	reqParam.pReqPayload = (const uint8_t *) &request;
	reqParam.reqPayloadSize = (uint32_t) sizeof (request);
	reqParam.timeout = (uint32_t) SCICLIENT_SERVICE_WAIT_FOREVER;
