...
#define Matrix_D_VERSION_ID "$Id: Matrix.h 1.67 2016/12/06 04:45:41EST Schulte, Michael (MEE_MiSch) draft  $"
//! @endcond
...
public:
  ...
  enum {
    ...
    channels = rows*cols,         ///< Number of elements
    shortdim = ((m < n) ? m : n), ///< Shortest dimension
    longdim  = ((m < n) ? n : m)  ///< Longest dimension
  };
  ...
  struct Config_s
  {
    T cVal_ax[channels];            ///< Value array
  ...
  explicit Matrix(const Array<T,channels>& i_Values_rx);
  ...
  Matrix<T, m, n, Mtype> add(const T value) const;

  //! Add two matrices element-wise
  Matrix<T, m, n, Mtype> add(const Matrix<T, m, n, Mtype>& i_M_rx) const;

  //! Subtract matrix by scalar value
  Matrix<T, m, n, Mtype> sub(const T value) const;

  //! Subtract two matrices element-wise
  Matrix<T, m, n, Mtype> sub(const Matrix<T, m, n, Mtype>& i_M_rx) const;

  //! Multiply matrix by scalar value
  Matrix<T, m, n, Mtype> mul(const T value) const;
  
  //! Multiply two matrices element-wise
  Matrix<T, m, n, Mtype> mul(const Matrix<T, m, n, Mtype>& i_M_rx) const;

  //! Divide matrix by scalar value
  Matrix<T, m, n, Mtype> div(const T value) const;

  //! Divide two matrices element-wise
  Matrix<T, m, n, Mtype> div(const Matrix<T, m, n, Mtype>& i_M_rx) const;
  ...
  //! Compute inverse matrix of largest square (sub)matrix, assert if determinant is zero, this is generic
  Matrix<T, m, n, Mtype> inverse_6x6() const;

  //! Compute inverse matrix of largest square (sub)matrix, assert if determinant is zero, this is generic
  Matrix<T, m, n, Mtype> inverse_4x4() const;

  //! Get adjoint of matrix
  void adjoint(float A[6][6],float adj[6][6])const;

  //! Get Cofactor of matrix elements
  void getCofactor(float A[6][6], float temp[6][6], int p, int q, int count)const;

  //! Compute determinate of largest square (sub)matrix
  T determinant(float A[6][6], int count) const;

  //! Get major diagonal
  // REMARK: must be implemented inline here since QA-cpp cannot parse function return type correctly
  Matrix<T, Matrix<T, m, n, Mtype>::shortdim, 1> diag() const
  {
    Matrix<T, Matrix<T, m, n, Mtype>::shortdim, 1> v_Diag_x;
 ...
 // * comparision
  ...
  template<uint32_t n1>
  Matrix<T, m, n+n1, Mtype> joinCol(const Matrix<T, m, n1, Mtype>& i_Mat_x) const;

  //! Join matrices row-wise (this , input matrix)
  template<uint32_t m1>
  Matrix<T, m+m1, n, Mtype> joinRow(const Matrix<T, m1, n, Mtype>& i_Mat_x) const;
  ...
  bool_t operator== (const Matrix<T, m, n, Mtype>& v) const;
  ...
  Matrix<T, m, n, Mtype> operator= (const Matrix<T, m, n, Mtype>& v_Mat_rx);
  ...
  Matrix<T, m, n, Mtype> operator+(const T& v) const
  {
    return add(v);
  }
  ...
  Matrix<T, m, n, Mtype> operator+(const Matrix<T, m, n, Mtype>& M) const
  {
    return add(M);
  }

  //! Operator of element-wise matrix addition + assignment
  void operator+=(const Matrix<T, m, n, Mtype> M)
  {
    *this = add(M);
  }
  
  //! Operator of element-wise matrix subtraction
  Matrix<T, m, n, Mtype> operator-(const Matrix<T, m, n, Mtype>& v) const
  {
    return sub(v);
  }
  ...
  template<uint32_t m1, uint32_t n1>
  Matrix<T, m, n1> operator%(const Matrix<T, m1, n1>& M) const
  {
    return mmul(M);
  }
  ...
  template<uint32_t n1>
  Matrix<T, m, n+n1> operator|(const Matrix<T, m, n1>& v) const
  {
    return this->joinCol(v);
  }

  //! Operator of joining matrices row-wise
  template<uint32_t m1>
  Matrix<T, m+m1, n> operator&(const Matrix<T, m1, n>& v) const
  {
    return this->joinRow(v);
  }
...
private:
  ...
  T val[channels];  ///< Matrix elements
