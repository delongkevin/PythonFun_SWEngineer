...
namespace fod
{
    StateMachine::StateMachine( const unsigned char maxStates ) :
        currentState( 0U ),
    ...
    void StateMachine::ExternalEvent(const unsigned char newState, EventData* pData )
    {
        // if we are supposed to ignore this event
        if( static_cast<uint8_t>( newState ) == static_cast<uint8_t>( EVENT_IGNORED ) )
        {
    ...
    void StateMachine::InternalEvent( const unsigned char newState, EventData* pData )
    {
        if( pData == NULL )
        {
            pData = new EventData();
