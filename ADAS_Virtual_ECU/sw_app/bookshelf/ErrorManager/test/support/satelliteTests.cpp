...
std::vector<int> CoresToTest_AllAsilLevels = { ErrorMgr_Asil_QM,ErrorMgr_Asil_A,ErrorMgr_Asil_B,ErrorMgr_Asil_C,ErrorMgr_Asil_D};
	...
	typedef ErrorMgr_ErrorCommPack_1_0_D ErrorMgr_D_SatOutputType;
...
public:
	...
	#ifdef BUILD_MCU1_0
		ErrorMgr_D_SatOutputType D_Data;
	...
	void m_Check_ifOutputStructure_Zero(StructType *Data,uint32_t FieldMaxValue)
	{
	...
	void m_CheckFaultValue(std::vector< ErrorMgr_enErrorNo> ToCheckErrorEnumVals, std::vector< ldef_ErrorMgr_ErrorField>ExpectedValues, ErrorMgr_CoreList testCore)
	{
	...
	#ifdef BUILD_MCU1_0
		GetSatOutput< ErrorMgr_D_SatOutputType>(&D_Data, T_enCoreToTest, ErrorMgr_Asil_D);
	#endif // BUILD_MCU1_0
		...
		for (unsigned int i = 0; i < ToCheckErrorEnumVals.size(); i++)
		{
			...
			if (ErrorMgr_TestDefs[i].enCore == testCore)
			{
				...
				#ifdef BUILD_MCU1_0
				case ErrorMgr_Asil_D:
					addData = D_Data.Data[index];
					ErrorValue = D_Data.Error[index];
					break;
				#endif // BUILD_MCU1_0
				...
				EXPECT_EQ(ErrorValue, ExpectedValues[i]);
				if (ExpectedValues[i] == 1)
				{
					for (int y = 0; y < ERRORMGR_ADDITIONALDATASIZE_COUNT; y++)
					{
						EXPECT_EQ(addData.AdditionalData[y], m_CurrentTestAdditionalDataLogged[CurrentError].AdditionalData[y]);
					}
				}
				else
				{
	...
	void m_MatureFaultFrom_Inactive_AndTest(std::vector< ErrorMgr_stTestErrorSpecs> ErrorsToMature,bool SpecificAsilLevel, ErrorMgr_AsilList AsilLevel,bool SetFaultEveryCycle, ErrorMgr_CoreList testCore)
	{
		...
		for (unsigned int i = 0; i < ErrorsToMature.size(); i++)
		{
			...
			ErrorMgrSatellite_ReportErrorStatus(ErrorsToMature[i].enGlobalErrNo, ErrorMgr_ErrorActive, &m_CurrentTestAdditionalDataLogged[ErrorsToMature[i].enGlobalErrNo]);
		}
		...
		for (unsigned int cnt = 0; cnt < MaxCount+1; cnt++)
		{
			...
			std::vector< ldef_ErrorMgr_ErrorField> ExpectedErrorValues(ErrorMgr_ErrorNum_Max,0);
...
TEST_P(Fixture_ErrorMgr_Sat_ErrorSetting, SetErrorTestCase_SingleFault)
{
	...
	m_Check_ifOutputStructure_Zero<ErrorMgr_QM_SatOutputType>(&QM_Data, u32CoreMaxQM);
	m_Check_ifOutputStructure_Zero<ErrorMgr_B_SatOutputType>(&B_Data, u32CoreMaxB);
