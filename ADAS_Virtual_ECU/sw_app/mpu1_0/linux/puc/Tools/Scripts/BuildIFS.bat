...
if [%1] == [] (
  set ELF_PATH=%CURDIR%\..\..\App\code\OS\BSP\images\
) else (
  set ELF_PATH=%1
)
...
@echo ###############################################
@echo Delete old QNX ifs (%ELF_PATH%\ifs-magna-ultrascale.elf)
del %ELF_PATH%\.\ifs-magna-ultrascale.elf
@echo.
...
make -C%ELF_PATH% ifs-magna-ultrascale.elf
@goto done
