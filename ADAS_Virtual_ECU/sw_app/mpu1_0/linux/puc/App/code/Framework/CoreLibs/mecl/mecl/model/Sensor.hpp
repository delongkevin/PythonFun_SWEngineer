...
, configured_b(false)
{
  initMinMax_v( core::Point2D<T>(i_Config_rs.pos1Cfg_x) , core::Point2D<T>(i_Config_rs.pos2Cfg_x));
}
...
void Sensor<T>::copyConfig_v(Config_s& o_Config_rs) const
{
  ...
  core::Point2D<T> v_Pos1_x = getUpperLeft_x();
  core::Point2D<T> v_Pos2_x = getLowerRight_x();

  v_Pos1_x.copyConfig_v(o_Config_rs.pos1Cfg_x);
  v_Pos2_x.copyConfig_v(o_Config_rs.pos2Cfg_x);
...
void Sensor<T>::copyConfigVolatile_v(volatile Config_s& o_Config_rs) const
{
  ...
  v_Pos1_x.copyConfigVolatile_v(o_Config_rs.pos1Cfg_x);
  v_Pos2_x.copyConfigVolatile_v(o_Config_rs.pos2Cfg_x);
...
void Sensor<T>::updateConfig_v(const Config_s& i_Config_rs)
{
  ...
  this->setImageOriginPosition_v(i_Config_rs.imageOriginPositionCfg_e);
  this->initMinMax_v(core::Point2D<T>(i_Config_rs.pos1Cfg_x), core::Point2D<T>(i_Config_rs.pos2Cfg_x));
  return;
}
...
{
  *this = Sensor<T1>(i_Config_rs).template convert_x<T>();
}
...
// -------------------------------------------------------------------------
/// @brief Get smallest visible metric coordinates
///
/// Get minimal metric values in x and y for a visible point position
/// of this sensor
///
/// @par Example usage:
/// @snippet Modeltester.cpp Sensor_ConstructorTest
/// @snippet Modeltester.cpp Sensor_getMinPos
///
/// @param[in] i_DoAssert_b  assert in case sensor not configured if true, otherwise do not.
///
/// @return Smallest visible metric 2D coordinates
// -------------------------------------------------------------------------

template<typename T>
const core::Point2D<T>& Sensor<T>::getMinPos_rx(const bool_t i_DoAssert_b) const
{
  AssertFunction(this->configured_b || not (i_DoAssert_b), "Sensor object wasn't configured." );
  return this->minPos_x;
}
// -------------------------------------------------------------------------
/// @brief Get largest visible metric coordinates
///
/// Get maximal metric values in x and y for a visible point position
/// of this sensor
///
/// @par Example usage:
/// @snippet Modeltester.cpp Sensor_ConstructorTest
/// @snippet Modeltester.cpp Sensor_getMaxPos
///
/// @param[in] i_DoAssert_b assert in case sensor not configured if true, otherwise do not.
///
/// @return Largest visible metric 2D coordinates
// -------------------------------------------------------------------------
template<typename T>
const core::Point2D<T>& Sensor<T>::getMaxPos_rx(const bool_t i_DoAssert_b) const
{
  AssertFunction(this->configured_b || not (i_DoAssert_b), "Sensor object wasn't configured.");
...
///
/// Returns pixel position of upper left sensor area corner
///
/// @par Example usage:
/// @snippet Modeltester.cpp Sensor_ConstructorTest
/// @snippet Modeltester.cpp Sensor_getUpperLeft
///
/// @return Pixel coordinates of upper left sensor corner
// -------------------------------------------------------------------------
template<typename T>
core::Point2D<T> Sensor<T>::getUpperLeft_x(void) const
{
  core::Point2D<T> v_Pos1_x;
  core::Point2D<T> v_Pos2_x;
  this->metricToPixel_v(this->minPos_x, v_Pos1_x);
  this->metricToPixel_v(this->maxPos_x, v_Pos2_x);
  this->recalcClipping_v(v_Pos1_x, v_Pos2_x);
  return v_Pos1_x;
}
...
///
/// Returns pixel position of lower right sensor area corner
///
/// @par Example usage:
/// @snippet Modeltester.cpp Sensor_ConstructorTest
/// @snippet Modeltester.cpp Sensor_getLowerRight
///
/// @return Pixel coordinates of upper left sensor corner
// -------------------------------------------------------------------------
template<typename T>
...
{
  if (this->imageOriginPosition_e != i_ImageOriginPosition_e)
  {
    const core::Point2D<T> c_UpperLeft_x = this->getUpperLeft_x();
    const core::Point2D<T> c_LowerRight_x = this->getLowerRight_x();
    this->imageOriginPosition_e = i_ImageOriginPosition_e;
    this->initMinMax_v(c_UpperLeft_x, c_LowerRight_x);
  }
  return;
}
...
/// @par Example usage:
/// @snippet Modeltester.cpp Sensor_ConstructorTest
/// @snippet Modeltester.cpp Sensor_setUpperLeft
///
...
template<typename T>
void Sensor<T>::setUpperLeft_v(const core::Point2D<T>& i_Pos_rx, const bool_t i_doAssert_b)
{
  core::Point2D<T> c_LowerRightPx_x = this->getLowerRight_x();
  AssertFunction( !(i_doAssert_b)
                  || (    ( c_LowerRightPx_x(0) > i_Pos_rx(0) )
  ...
  this->pixelToMetric_v(c_LowerRightPx_x, this->maxPos_x);
  this->recalcClipping_v(this->minPos_x, this->maxPos_x);
  return;
}
...
/// @par Example usage:
/// @snippet Modeltester.cpp Sensor_ConstructorTest
/// @snippet Modeltester.cpp Sensor_setLowerRight
///
/// @param[in] i_Pos_rx New position of upper left corner in pixel
/// @return void
// -------------------------------------------------------------------------
...
template<typename T>
void Sensor<T>::setLowerRight_v(const core::Point2D<T>& i_Pos_rx, const bool_t i_doAssert_b)
{
  core::Point2D<T> c_UpperLeftPx_x = this->getUpperLeft_x();
  AssertFunction(    !(i_doAssert_b) ||
                     (   ( c_UpperLeftPx_x(0) < i_Pos_rx(0) )
  ...
  this->pixelToMetric_v(c_UpperLeftPx_x, this->minPos_x);
  this->recalcClipping_v(this->minPos_x, this->maxPos_x);
  return;
}

// -------------------------------------------------------------------------
/// @brief Set clipping area where points are visible
///
/// Function sets position of lower right and upper right corner according to the bounding box
/// defined by the pixel point locations \p i_Pos1_rx, \p i_Pos2_rx.
///
/// @par Example usage:
/// @snippet Modeltester.cpp Sensor_ConstructorTest
/// @snippet Modeltester.cpp Sensor_setClippingArea
///
/// @param[in] i_Pos1_rx First corner point of the clipping area bounding box in pixel
/// @param[in] i_Pos2_rx Seconed corner point of the clipping area bounding box in pixel
/// @remark The corner points \p i_Pos1_rx and \p i_Pos2_rx must have different coordinates
/// @return void
// -------------------------------------------------------------------------
template<typename T>
void Sensor<T>::setClippingArea_v(const core::Point2D<T>& i_Pos1_rx, const core::Point2D<T>& i_Pos2_rx)
{
  this->setUpperLeft_v(i_Pos1_rx, false);
  this->setLowerRight_v(i_Pos2_rx, true);
...
Sensor<T>& Sensor<T>::get_rx(ISensor<T>& i_ISensor_rx)
{
  ...
  AssertFunction(isNotNull_b(v_Sensor_px),"Reference to ISensor is not a Sensor object");
  return *v_Sensor_px;
}
...
Sensor<T> Sensor<T>::convert_x(ISensor<T1>& i_ISensor_rx)
{
  ...
  AssertFunction(isNotNull_b(v_Sensor_px),"Reference to ISensor of different base type is not a Sensor object");
  return v_Sensor_px->template convert_x<T>();
}
...
template<typename T>
bool_t Sensor<T>::isFlipped_b() const
{
  return (    (e_UpperRight == this->imageOriginPosition_e)
           || (e_LowerRight == this->imageOriginPosition_e));
}
...
// -------------------------------------------------------------------------
template<typename T>
void Sensor<T>::initMinMax_v(const core::Point2D<T>& i_UpperLeft_rx, const core::Point2D<T>& i_LowerRight_rx)
{
    this->pixelToMetric_v(i_UpperLeft_rx, this->minPos_x);
    this->pixelToMetric_v(i_LowerRight_rx, this->maxPos_x);
    this->recalcClipping_v(this->minPos_x, this->maxPos_x);
    this->configured_b = true;
    return;
}

/// @brief (Re)-Calculate sensor area (minimal/maximal position in either pixel or metric coordinates)
///
/// @param[in,out] b_UpperLeft_x contains initial upperleft position, output value is corrected
/// @param[in,out] b_LowerLeft_x contains initial lowerleft position, output value is corredctd
/// @return void
// -------------------------------------------------------------------------
template<typename T>
void Sensor<T>::recalcClipping_v(core::Point2D<T>& b_UpperLeft_rx, core::Point2D<T>& b_LowerRight_rx) const
{
  const T c_Leftmost_x = math::min_x(b_UpperLeft_rx.getPosX(), b_LowerRight_rx.getPosX());
  const T c_Rightmost_x = math::max_x(b_UpperLeft_rx.getPosX(), b_LowerRight_rx.getPosX());
  const T c_Upmost_x = math::min_x(b_UpperLeft_rx.getPosY(), b_LowerRight_rx.getPosY());
  const T c_Lowest_x = math::max_x(b_UpperLeft_rx.getPosY(), b_LowerRight_rx.getPosY());

      b_UpperLeft_rx.setPosX(c_Leftmost_x);
      b_UpperLeft_rx.setPosY(c_Upmost_x);
      b_LowerRight_rx.setPosX(c_Rightmost_x);
      b_LowerRight_rx.setPosY(c_Lowest_x);
}
