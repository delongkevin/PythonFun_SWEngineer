...
// PRQA S 3708 EOF   // floating point arithmetic MUST be used here
// PRQA S 467 EOF // ignore QACPP maintenance waring
// PRQA S 3360 EOF // pre-increment is allowed here as sub-expression
...
: val()
{
  AssertFunction(isNotNull_b(values),"Passed over null pointer to array parameter.");
...
Matrix<T, m, n, Mtype> Matrix<T, m, n, Mtype>::minus() const
{
  Matrix<T, m, n, Mtype> v_Mret_x( (*this) );
  T* v_MretVal_px = v_Mret_x.val;
  T* const c_MatrixRetValEnd_px = &v_Mret_x.val[channels];

  do
  {
    *(v_MretVal_px++) *= math::constants<T>::minusOne_x();
  } while (v_MretVal_px != c_MatrixRetValEnd_px );
...
void Matrix<T, m, n, Mtype>::print() const
{
  for (uint32_t i=0 ; i<m ; ++i)
  {
    ...
    for (uint32_t k=0 ; k<n ; ++k) {
      ME_Debugger_F_Printf_v ("%g\t", (*this)(i,k));
    }
  }
  ME_Debugger_F_Printf_v ("\n");
}
...
{
  ME_Debugger_F_Printf_v ("%s", i_OutputFormat_rs.headingChars);
  for (uint32_t i=0 ; i<m ; i++)
  {

    for (uint32_t j=0 ; j<n-1 ; j++) {
      ME_Debugger_F_Printf_v ("%f%s", (*this)(i,j), i_OutputFormat_rs.columnSeperatorChars);
    }
    ME_Debugger_F_Printf_v("%f%s", (*this)(i,n-1), (m-1) != i ? i_OutputFormat_rs.rowSeperatorChars
