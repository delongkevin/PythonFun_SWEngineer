...
{
  return NULL; //strstr(i_Buffer_pc, i_SearchString_pc);
}

sint32_t ME_String_Stricmp_s32(const char_t* i_String1_pc, const char_t* i_String2_pc)
{
  return -1; //strcasecmp(i_String1_pc, i_String2_pc);
}
...
{
  return true; // (NULL != snprintf(o_Buffer_pc, i_Radix_s32, "%ld", i_Value_s32)); /* PRQA S 4604 */ /* PRQA S 3335 */
}
...
{
  return -1; //(sint64_t)wcstoll(i_String_pc, NULL, 10); /* PRQA S 4604 */ /* PRQA S 3335 */
}

float32_t ME_String_Wtof_f32(const wchar_t* i_String_pc)
{
  return 0.0f; //(float32_t) wcstof(i_String_pc, NULL); /* PRQA S 4604 */ /* PRQA S 3335 */
}
