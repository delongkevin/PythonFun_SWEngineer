...
const uint32_t c_DistPrincipalPointEntryBytes_u32   = 0x0008U;
const uint32_t c_DistPixelDensityEntryBytes_u32     = 0x0008U;
const uint32_t c_DistortLUTEntryBytes_u32           = 0x1800U;

const uint32_t c_UndistPrincipalPointEntryBytes_u32 = 0x0004U;
const uint32_t c_UndistLUTEntryBytes_u32            = 0x4000U;

const uint32_t c_MatMulEntryBytes_u32               = 0x0040U;
...
ViewGenConfig::ViewGenConfig()
  ...
  , harmonThreshold_pv(NULL)
  , harmonApplicationFactors_pv(NULL)
  , harmonApplicationOffsets_pv(NULL)
  , harmonComputationResults_pv(NULL)
{
...
bool_t ViewGenConfig::init_b(void* i_BaseAddress_pv)
{
  ...
  if(i_BaseAddress_pv != NULL)
  {
    ...
    for(uint32_t v_I_u32 = 0; v_I_u32 < variants::e_CameraCount; v_I_u32++)
    {
      distPrinciplePoints_apv[v_I_u32]      = &reinterpret_cast<uint8_t*>(axiLite_pv)[c_DistortPrinciplePointsOffset_u32  + (v_I_u32 * c_DistPrincipalPointEntryBytes_u32)];
      distPixelDensity_apv[v_I_u32]         = &reinterpret_cast<uint8_t*>(axiLite_pv)[c_DistortPixelDensityOffset_u32     + (v_I_u32 * c_DistPixelDensityEntryBytes_u32)];
      distLUT_apv[v_I_u32]                  = &reinterpret_cast<uint8_t*>(axiLite_pv)[c_DistortLUTOffset_u32              + (v_I_u32 * c_DistortLUTEntryBytes_u32)];
    }

    for(uint32_t v_I_u32 = 0; v_I_u32 < c_MaxLensCount_u32; v_I_u32++)
    {
      undistPrinciplePoints_apv[v_I_u32]    = &reinterpret_cast<uint8_t*>(axiLite_pv)[c_UndistortPrinciplePointsOffset_u32 + (v_I_u32 * c_UndistPrincipalPointEntryBytes_u32)];
      undistLUT_apv[v_I_u32]                = &reinterpret_cast<uint8_t*>(axiLite_pv)[c_UndistortLUTOffset_u32             + (v_I_u32 * c_UndistLUTEntryBytes_u32)];
    }

    for(uint32_t v_I_u32 = 0; v_I_u32 < c_MaxProjectionCount_u32; v_I_u32++)
    {
      matMult_apv[v_I_u32]                  = &reinterpret_cast<uint8_t*>(axiLite_pv)[c_MatMulOffset_u32                   + (v_I_u32 * c_MatMulEntryBytes_u32)];
    }
...
uint32_t ViewGenConfig::getUndistortLUTEntryCount_u32()
{
  return (c_UndistLUTEntryBytes_u32 / sizeof(uint32_t));
}

uint32_t ViewGenConfig::getUndistPrincipalPoitnsEntryCount_u32()
{
  return (c_UndistPrincipalPointEntryBytes_u32 / sizeof(uint32_t));
}

uint32_t ViewGenConfig::getDistPrincipalPointsEntryCount_u32()
{
  return (c_DistPrincipalPointEntryBytes_u32 / sizeof(uint32_t));
}

uint32_t ViewGenConfig::getDistPixelDensityEntryCount_u32()
{
  return (c_DistPixelDensityEntryBytes_u32 / sizeof(uint32_t));
}

uint32_t ViewGenConfig::getDistortLUTEntryCount_u32()
{
  return (c_DistortLUTEntryBytes_u32 / sizeof(uint32_t));
}

uint32_t ViewGenConfig::getMatMulEntryCount_u32()
{
  return (c_MatMulEntryBytes_u32 / sizeof(uint32_t));
}
