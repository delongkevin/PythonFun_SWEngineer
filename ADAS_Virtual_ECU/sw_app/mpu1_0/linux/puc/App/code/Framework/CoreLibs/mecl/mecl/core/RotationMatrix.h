...
#include "Matrix3x3.h"
#include "mecl/model/ModelTypes.h"
...
public:
  ...
  {
    return static_cast< RotationMatrix<T> >(Matrix<T, 3, 3>::eye());
  }
  ...
  void printEASafe_v(bool_t i_InDegrees_b = true) const
  {
    ...
    ME_Debugger_F_Printf_v("Euler Angles: (pitch [Rx], yaw [Ry] , roll[Ry])\n");
    if (i_InDegrees_b)
    {
      ME_Debugger_F_Printf_v("Solution 1: (%f, %f, %f)\n",
          math::toDegrees_x(v_Solution1_s.pitch_x),
          math::toDegrees_x(v_Solution1_s.yaw_x),
          math::toDegrees_x(v_Solution1_s.roll_x) );
      ME_Debugger_F_Printf_v("Solution 2: (%f, %f, %f)\n",
    ...
    if (!twoSolutions_b)
    {
      ME_Debugger_F_Printf_v("Solutions 1 and 2 are equal.");
    }
