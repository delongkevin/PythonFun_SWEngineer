...
//--------------------------------------------------------------------------
/// @file ME_String_AUTOSAR.c
/// @brief AUTOSAR implementation for standard-library String
///
/// --------------------------------------------------------------------------
...
sint32_t ME_String_Scanf_s32(const char_t* i_Format_pc, ...)
{
  ...
  va_list v_Argptr_t; /* Type to hold information about variable arguments */ /* PRQA S 3196 */
  (void)va_start(v_Argptr_t, i_Format_pc); /* Initialize a variable argument list */ /* PRQA S 3335 */

  v_Result_s32 = vscanf(i_Format_pc, v_Argptr_t); /* PRQA S 3335 */ /* PRQA S 4604 */

  (void)va_end(v_Argptr_t); /* Performs the appropriate actions to facilitate a normal return */ /* PRQA S 3335 */
...
{
  return vscanf(i_Format_pc, argptr); /* PRQA S 3335 */ /* PRQA S 4604 */
}
...
{
  size_t v_NumCopyWords_t = fmin(i_SizeInWords_t, i_Count_t); /* PRQA S 4604 */ /* PRQA S 3335 */
...
void ME_String_Strncat_v(char_t* o_StrDest_pc, size_t i_SizeInBytes_t, const char_t* i_StrSource_pc, size_t i_Count_t)
{
  ...
  if ( (NULL != i_StrSource_pc) && (NULL != o_StrDest_pc))
  {
    ...
    if (v_Length_t > 0)
    {
      v_NumCopyChars_t = fmin(v_Length_t - 1, i_Count_t); /* PRQA S 4604 */ /* PRQA S 3335 */ /* PRQA S 3760 */
...
void ME_String_Wcsncat_v(wchar_t* o_StrDest_pc, size_t i_SizeInWords_t, const wchar_t* i_StrSource_pc, size_t i_Count_t)
{
  ...
  if (v_Length_t > 0)
  {
    size_t v_NumCopyWords_t = fmin(v_Length_t - 1, i_Count_t); /* PRQA S 4604 */ /* PRQA S 3335 */ /* PRQA S 3760 */
...
{
  size_t v_NumCopyChars_t = fmin(i_SizeInBytes_t, i_Count_t); /* PRQA S 4604 */ /* PRQA S 3335 */ /* PRQA S 3760 */
...
sint32_t ME_String_Snprintf_s32(char_t* i_Buffer_pc, size_t i_Count_t, const char_t* i_Format_pc, ...)
{
  ...
  v_Result_s32 = vsnprintf(i_Buffer_pc, i_Count_t, i_Format_pc, v_Argptr_t); /* PRQA S 4604 */ /* PRQA S 3335 */
...
{
  return vsnprintf(i_Buffer_pc, i_Count_t, i_Format_pc, i_Argptr_t); /* PRQA S 4604 */ /* PRQA S 3335 */
}
...
sint32_t ME_String_Snwprintf_s32(wchar_t* i_Buffer_pc, size_t i_Count_t, const wchar_t* i_Format_pc, ...)
{
  ...
  result = vswprintf(i_Buffer_pc, i_Count_t, i_Format_pc, v_Argptr_t); /* PRQA S 4604 */ /* PRQA S 3335 */
...
{
  return vswprintf(i_Buffer_pc, i_Count_t, i_Format_pc, i_Argptr_t); /* PRQA S 4604 */ /* PRQA S 3335 */
}
...
{
  return (NULL != snprintf(o_Buffer_pc, i_Radix_s32, "%ld", i_Value_s32)); /* PRQA S 4604 */ /* PRQA S 3335 */
}
...
sint64_t ME_String_Atoi64_s64(const char_t* i_String_pc)
{
  ...
  {
    // PRQA S 5125 1 // using this until own implementation is implemented
    v_Result_s64 = (sint64_t)atoll(i_String_pc); /* PRQA S 4604 */ /* PRQA S 3335 */
  }
...
{
  return (sint64_t)wcstoll(i_String_pc, NULL, 10); /* PRQA S 4604 */ /* PRQA S 3335 */
}

float32_t ME_String_Wtof_f32(const wchar_t* i_String_pc)
{
  return (float32_t) wcstof(i_String_pc, NULL); /* PRQA S 4604 */ /* PRQA S 3335 */
}
...
sint32_t ME_String_CountCharacters_s32(const char_t* i_String_pc)
{
  ...
  if (NULL != i_String_pc)
  {
    while (*i_String_pc)
    {
       ...
       * Count up each byte that starts with a zero bit or two one bits.
       * */
      v_Count_s32 += (*i_String_pc++ & 0xc0) != 0x80; /* PRQA S 4130 */ /* PRQA S 4105 */ /* used to increase character count here, so it's ok */ /* must smaller than 0x80 or greaterequal 0xC0 */
      /*
       ...
       * This last test is quite easy: Bytes 0x01 through 0x7F in UTF-8 represent the corresponding ASCII characters, while bytes 0xC0 through 0xFF are the first byte of a multi-byte character. This results in the following inner loop (modulo some style changes to make it easier to compare this against later versions):
       */
    }
  }
