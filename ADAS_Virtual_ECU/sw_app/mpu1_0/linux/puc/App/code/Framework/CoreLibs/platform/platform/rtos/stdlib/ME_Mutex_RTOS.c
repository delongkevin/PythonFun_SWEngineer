...
PlatformError_t ME_Mutex_Init_t(Mutex_s* b_Mutex_po, bool_t i_Shared_b)
{
  ...
  b_Mutex_po->handle_t = xSemaphoreCreateMutex();

  if(b_Mutex_po->handle_t != NULL)
  {
    v_Error_t = e_PeOk;
  }
...
PlatformError_t ME_Mutex_Lock_t(Mutex_s* i_Mutex_po)
{
  ...
  if(pdPASS != xSemaphoreTake(i_Mutex_po->handle_t, portMAX_DELAY))
  {
    v_Error_t = e_PeFault;
  }
...
PlatformError_t ME_Mutex_TryLock_t(Mutex_s* i_Mutex_po)
{
  ...
  xSemaphoreTake(i_Mutex_po->handle_t, 0);

  if(pdPASS != xSemaphoreTake(i_Mutex_po->handle_t, 0))
  {
    v_Error_t = e_PeFault;
  }
...
PlatformError_t ME_Mutex_UnLock_t(Mutex_s* i_Mutex_po)
{
  ...
  if(pdPASS != xSemaphoreGive(i_Mutex_po->handle_t))
  {
    v_Error_t = e_PeFault;
  }
