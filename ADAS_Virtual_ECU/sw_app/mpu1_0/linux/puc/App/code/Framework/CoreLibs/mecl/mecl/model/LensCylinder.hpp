                                                ...
                                                core::Point2D<T>& o_Pos_rx) const
{
  ...
  AssertFunction((false == math::isAboutZero_b<T>(this->excentr_x)),
  ...
  v_Applicable_b = (false == math::isZero_b(c_Rho_x));
...
bool_t LensCylinder<T>::isApplicable_b(const core::Point3D<T>& i_Pos_rx) const
{
  ...
  return (false == math::isAboutZero_b(c_Rho_x));
}
...
LensCylinder<T>& LensCylinder<T>::get_rx(ILens<T>& i_ILens_rx)
{
  ...
  AssertFunction(isNotNull_b(v_Lens_px), "ILens reference is not a cylinder lens.");
  return *v_Lens_px;
}
