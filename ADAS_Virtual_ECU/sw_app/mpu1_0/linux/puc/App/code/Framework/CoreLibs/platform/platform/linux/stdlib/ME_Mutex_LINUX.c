...
PlatformError_t ME_Mutex_Init_t(Mutex_s* b_Mutex_po, bool_t i_Shared_b)
{
  ...
  if(NULL != b_Mutex_po)
  {
    ...
    if(e_PeOk == v_Error_t)
    {
      ...
      v_Result_s32 = pthread_mutex_init(&b_Mutex_po->qnxMutex_o, &v_qnxMutexAttributes_t);
      v_Error_t = ME_GetPlatformError_t(v_Result_s32);
    }
  }
...
PlatformError_t ME_Mutex_Lock_t(Mutex_s* i_Mutex_po)
{
  ...
  if(NULL != i_Mutex_po)
  {
    sint32_t v_Result_s32 = pthread_mutex_lock(&i_Mutex_po->qnxMutex_o);
    v_Error_t = ME_GetPlatformError_t(v_Result_s32);
  }
  else
  {
...
PlatformError_t ME_Mutex_TryLock_t(Mutex_s* i_Mutex_po)
{
  ...
  if(NULL != i_Mutex_po)
  {
    sint32_t v_Result_s32 = pthread_mutex_trylock(&i_Mutex_po->qnxMutex_o);
    v_Error_t = ME_GetPlatformError_t(v_Result_s32);
  }
  else
  {
...
PlatformError_t ME_Mutex_UnLock_t(Mutex_s* i_Mutex_po)
{
  ...
  if(NULL != i_Mutex_po)
  {
    sint32_t v_Result_s32 =  pthread_mutex_unlock(&i_Mutex_po->qnxMutex_o);
...
PlatformError_t ME_Mutex_Destroy_t(Mutex_s* b_Mutex_po)
{
  ...
  if(NULL != b_Mutex_po)
  {
    sint32_t v_Result_s32 = pthread_mutex_destroy(&b_Mutex_po->qnxMutex_o);
    v_Error_t = ME_GetPlatformError_t(v_Result_s32);
  }
  else
  {
