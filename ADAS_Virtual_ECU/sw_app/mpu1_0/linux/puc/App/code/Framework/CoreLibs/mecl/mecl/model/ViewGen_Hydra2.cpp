...
#define ViewGen_Hydra2_d_VERSION_ID "$Id: ViewGen_Hydra2.cpp 1.2 2021/08/17 08:52:01CEST Martin Rachor (MEE_MRACHO) draft  $"
//! @endcond
...
::init_v(void)
{
  ...
  v_Vgc_P3f32_u32.setSensorFlipped_v(this->lut_s.config_s.camera_po->getSensor_rx().isFlipped_b());

  core::Point2D<float32_t> v_Pos2d_f32;
  core::Point2D<float32_t> v_Ppp_f32(this->lut_s.config_s.camera_po->getSensor_rx().getPpp_rx());
  core::Point3D<float32_t> v_Pos3d_f32;
  core::Point3D<float32_t> v_Pos3dComp_f32;
...
//this->lut_s.config_s.camera_po->getSensor_rx().pixelToMetric_v(v_Ppp_f32, v_Ppp_f32);
  ...
  //PRQA S 3706 1 // subscript operator necessary here
      ...
      v_SupportLine_u32++)
  {
    ...
    //AssertMsg(v_Addr_pu32 <= c_AddrEnd_pu32, "Memory offset is out of bounds.");
      ...
      {
        const SideFaces_e     c_FaceSide_e = (v_Pos2d_f32(0) >= 0.0F) ? e_RightSide  : e_LeftSide;
        const VerticalFaces_e c_FaceVert_e = (v_Pos2d_f32(1) >= 0.0F) ? e_BottomSide : e_TopSide;
...
ViewGen< e_Hydra2RadialLens,        // ViewGen type
         ...
         core::Point2D<float32_t>,  // Type of value encoded by index value
         float32_t >
::getIndexedValue_x(const uint32_t i_Index_u32) const
{
  AssertFunction (true == this->isConfigured_b(), "ViewGen is not configured.");
  ...
  const uint32_t c_HMod_u32 = i_Index_u32 %  c_SupportPoints_u32;
  const uint32_t c_VDiv_u32 = (i_Index_u32 - c_HMod_u32) / c_SupportPoints_u32 ;
...
::init_v(void)
{
  ...
  {
    const float32_t c_YPos_f32 = c_StepWidth_f32 * static_cast<float32_t>(v);
    for (uint32_t u = 0; u <= this->lut_s.config_s.spread_u32; u++)
    {
      ...
      const float32_t c_XPos_f32 = c_StepWidth_f32 * static_cast<float32_t>(u);
  ...
  for (uint32_t v=0; v <= this->lut_s.config_s.spread_u32; v++)
  {
    ...
    for (uint32_t u=0; u <= c_Spread2x_u32; u++)
    {
      ...
      const float32_t c_ZPos_f32 = c_StepWidth_f32 * static_cast<float32_t>(u) - math::constants<float32_t>::one_x();
      ...
      if (true == this->lut_s.config_s.calcError_b)
      {
        v_NumSideFacesEntries_u32++; // PRQA S 4113 // cannot move into this scope, as used later on
        core::Point2D<float32_t> v_PosComp2d_f32;
        v_Vgc_P2f32_u32.decode_v(*v_Addr_pu32, v_PosComp2d_f32);
        const float32_t c_Error_f32 = (v_Pos2d_f32  - v_PosComp2d_f32).norm();
        v_SumError_f32 += c_Error_f32;
  ...
  {
    const float32_t c_ZPos_f32 = c_StepWidth_f32 * static_cast<float32_t>(v) - math::constants<float32_t>::one_x();
    for (uint32_t u=0; u <= this->lut_s.config_s.spread_u32; u++)
    {
      ...
      if (true == this->lut_s.config_s.calcError_b)
      {
        v_NumVerticalFacesEntries_u32++; // PRQA S 4113 // cannot move into this scope, as used later on
        core::Point2D<float32_t> v_PosComp2d_f32;
        v_Vgc_P2f32_u32.decode_v(*v_Addr_pu32, v_PosComp2d_f32);
        const float32_t c_Error_f32 = (v_Pos2d_f32  - v_PosComp2d_f32).norm();
        v_SumError_f32 += c_Error_f32;
...
::getIndexedValue_x(const uint32_t i_Index_u32) const
{
  ...
  if (i_Index_u32 <= c_SpreadSqr_u32) // front face
  {
    v_Pos3d_f32(0) += static_cast<float32_t>(i_Index_u32 / this->lut_s.config_s.spread_u32) * c_StepWidth_f32;
    v_Pos3d_f32(1) += static_cast<float32_t>(i_Index_u32 % this->lut_s.config_s.spread_u32) * c_StepWidth_f32;
    v_Pos3d_f32(2) = math::constants<float32_t>::one_x();

  }
  else if (i_Index_u32 <= (c_SpreadSqr_u32 + c_SpreadSqr2x_u32)) // side faces (left face)
  {
    v_Pos3d_f32(0) = math::constants<float32_t>::minusOne_x();
    v_Pos3d_f32(1) += static_cast<float32_t>(i_Index_u32 / this->lut_s.config_s.spread_u32) * c_StepWidth_f32;
    v_Pos3d_f32(2) += static_cast<float32_t>(i_Index_u32 % this->lut_s.config_s.spread_u32) * c_StepWidth_f32;
  }
  else if (i_Index_u32 <= (c_SpreadSqr_u32 + c_SpreadSqr2x_u32 + c_SpreadSqr2x_u32)) // vertical faces (top face)
  {
    v_Pos3d_f32(0) += static_cast<float32_t>(i_Index_u32 % this->lut_s.config_s.spread_u32) * c_StepWidth_f32;
