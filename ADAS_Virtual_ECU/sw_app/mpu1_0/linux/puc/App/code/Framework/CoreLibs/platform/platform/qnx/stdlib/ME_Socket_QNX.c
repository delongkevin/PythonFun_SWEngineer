...
/// @brief Qnx implementation for standard-library Socket
///
/// --------------------------------------------------------------------------
...
PlatformError_t ME_Socket_Send_t(const SocketHandle_t* b_Socket_pt, const char_t* b_Buffer_pc, size_t i_BufferLength_t, size_t* b_NumBytesSent_pt)
{
  ...
  else
  {
    ...
    (void)FD_SET(b_Socket_pt->socketHandle_s32, &socketSet); // PRQA S 4130 // Bitwise operations on signed data will give implementation defined results. => QNX lib
