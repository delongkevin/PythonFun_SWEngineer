...
namespace model {
                                          ...
                                          bool_t& b_CheckApplicable_rb) const
  {
    ...
    {
      b_CheckApplicable_rb = this->getLens_rx().applyDistortion_b(i_Pos_rx, o_Pos_rx);
    }
    else
    {
    ...
    if (    (static_cast<T>(3.99) < math::abs_x(o_Pos_rx(0)))
         || (static_cast<T>(3.99) < math::abs_x(o_Pos_rx(1))))
    {
      // value scaled by 2^13 can not be encoded in LUT (16-bit signed value)
      b_CheckApplicable_rb = false;
    }
  }
  ...
  // --------------------------------------------------------------------------
   ...
   {
     return GenericCamera<IM, LM, SM, T1> (this->getAsCamera_rx().template convert_x<T1>());
   }
