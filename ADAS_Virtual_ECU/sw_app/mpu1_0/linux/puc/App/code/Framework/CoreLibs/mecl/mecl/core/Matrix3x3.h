...
#define Matrix3x3_D_VERSION_ID "$Id: Matrix3x3.h 1.2 2021/08/17 08:52:00CEST Martin Rachor (MEE_MRACHO) draft  $"
//! @endcond

// PRQA S 467 EOF // ignore QACPP maintenance warning
// PRQA S 3362    // ignore pre-decrement used as sub-expression warning
// PRQA S 3360    // ignoew pre-increment used as sub-expression warning
...
public:
  ...
  T det(void) const
  {
    const T* v_Val1_px = & ( (*this) (6) );
    const T* v_Val2_px = & ( (*this) (1) );
    const T* v_Val3_px = v_Val1_px;

    T v_Determinate_x = *v_Val1_px;
    T v_Sum_x = *v_Val2_px * *(--v_Val1_px);

    v_Sum_x -= *(++v_Val2_px) * *(--v_Val1_px);
    v_Determinate_x *= v_Sum_x;

    v_Sum_x  = *v_Val2_px * *(++v_Val3_px);
    v_Sum_x -= *(--v_Val2_px) * *(++v_Val3_px);
    v_Determinate_x += v_Sum_x * *(--v_Val1_px);

    v_Sum_x  = *(++v_Val1_px) * *v_Val3_px;
    v_Sum_x -= *(++v_Val1_px) * *(--v_Val3_px);
    v_Determinate_x += v_Sum_x * *(--v_Val2_px);
  ...
  /// @return       Boolean, True = regular, False = not regular
  // --------------------------------------------------------------------------
  bool_t isRegular_b(T& o_Determinate_x) const
  {
    o_Determinate_x = this->det();

    return math::numeric_limits<T>::epsilon_x() < math::abs_x(o_Determinate_x);
  }
  ...
  Matrix3x3<T> inverse(void) const
  {
    ...
    const T* v_Val1_px = &( (*this)(4) );
    const T* v_Val2_px = &( (*this)(8) );
    const T* v_Val3_px = &( (*this)(2) );
    const T* v_Val4_px = &( (*this)(0) );

    *v_InvMat_px      = *v_Val1_px * *v_Val2_px;             // 0 (0,0)
    *v_InvMat_px     -= *(++v_Val1_px) * *(--v_Val2_px);     // 5,7,2,0

    *(++v_InvMat_px)  = *v_Val3_px  * *v_Val2_px;            // 1 (0,1)
    *v_InvMat_px     -= *(--v_Val3_px) * *(++v_Val2_px);     // 5,8,1,0

    *(++v_InvMat_px)  = *v_Val3_px  * *v_Val1_px;            // 2 (0,2)
    *v_InvMat_px     -= *(++v_Val3_px) * *(--v_Val1_px);     // 4,8,2,0

    *(++v_InvMat_px)  = *(++v_Val1_px);                      // 3 (1,0)
    *v_InvMat_px     *= *(++v_Val1_px);
    *v_InvMat_px     -= *(++v_Val3_px) * *v_Val2_px;         // 6,8,3,0

    *(++v_InvMat_px) = *v_Val4_px * *v_Val2_px;              // 4 (1,1)
    *v_InvMat_px    -= *(--v_Val3_px) * *v_Val1_px;          // 6,8,2,0

    *(++v_InvMat_px) = *v_Val3_px;                           // 5 (1,2)
    *v_InvMat_px    *= *(++v_Val3_px);
    *v_InvMat_px    -= *v_Val4_px * *(--v_Val1_px);          // 5,8,3,0

    *(++v_InvMat_px) = *v_Val3_px * *(--v_Val2_px);          // 6 (2,0)
    *v_InvMat_px    -= *(++v_Val3_px) * *(--v_Val2_px);      // 5,6,4,0

    *(++v_InvMat_px) = *(++v_Val4_px) * *v_Val2_px;          // 7 (2,1)
    *v_InvMat_px    -= *(--v_Val4_px) * *(++v_Val2_px);      // 5,7,4,0

    *(++v_InvMat_px) = *v_Val4_px     * *v_Val3_px;          // 8 (2,2)
    *v_InvMat_px    -= *(++v_Val4_px) * *(--v_Val3_px);      // 5,7,3,1
