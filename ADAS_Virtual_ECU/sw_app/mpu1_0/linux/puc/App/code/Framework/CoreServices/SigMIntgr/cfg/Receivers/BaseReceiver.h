//--------------------------------------------------------------------------
/// @file BaseReceiver.h
/// @brief Contains BaseClass implementation for a base request
///
/// --------------------------------------------------------------------------
...
#ifndef BASERECEIVER_H_
#define BASERECEIVER_H_
...
public:
  ...
  /// @param [in] i_MsgReceived_ro received message
  /// @return void 
  BaseReceiver(IpcMsgId_t i_ReceiveID_t, uint32_t i_FrameCountTimeout_u32);
  ...
  /// @param [in] i_MsgReceived_ro received message
  /// @return void 
  void setReceivedMsg_v(const ME_MsgRx_s& i_MsgReceived_ro);

  /// Copies the receive message
  /// @return true if the response should be sent. used in Signal Manager 
  bool_t handleReceive_b();

  /// Gets the receive id
  /// @return enum representing the receive id
  IpcMsgId_t getReceiveID_t() const;
  ...
  bool_t readFromBuffer(const uint8_t* i_Data_pc, uint32_t i_DataLength_u32,  uint8_t * dest);
  ...
  // add crc later here
  /// @return the  sizeof the struct specified in template argument
  template <typename T>
  static uint32_t getStructSize_u32()
  {
    return  sizeof(T);
  }
...
protected:
  ...
  virtual bool_t handleSpecificReceiver_b() = 0;
...
protected:
  ...
  uint32_t channel_u32;                                 ///< channel for the response message (always 0 at the moment)
  ME_MsgRx_s m_MsgReceived;                             ///< Message Object to receive
                          ///< Message Object to respond

private:
  osal::Mutex mutex_o;
  IpcMsgId_t receiveID_t;                              ///< receiver id, defined in constructor
  ReceiveCntrl_s receiveControl_s;
