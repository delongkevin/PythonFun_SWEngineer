...
public:
  ...
  /// Get smallest visible metric coordinates
  const core::Point2D<T>& getMinPos_rx(const bool_t i_DoAssert_b = true) const;

  /// Get largest visible metric coordinates
  const core::Point2D<T>& getMaxPos_rx(const bool_t i_DoAssert_b = true) const;
  ...
  void setUpperLeft_v(const core::Point2D<T>& v_Pos_rx, const bool_t i_doAssert_b = true);

  /// Set lower right corner of valid sensor area in pixel coordinates
  void setLowerRight_v(const core::Point2D<T>& v_Pos_rx, const bool_t i_doAssert_b = true);

  /// Set clipping area where points are visible
  void setClippingArea_v(const core::Point2D<T>& i_Pos1_rx, const core::Point2D<T>& i_Pos2_rx);
  ...
  /// check if image is flipped over y axis
  virtual bool_t isFlipped_b() const;
...
private:
  ...
  void initMinMax_v(const core::Point2D<T>& i_UpperLeft_rx, const core::Point2D<T>& i_LowerRight_rx);

  void recalcClipping_v(core::Point2D<T>& b_UpperLeft_rx, core::Point2D<T>& b_LowerRight_rx) const;
