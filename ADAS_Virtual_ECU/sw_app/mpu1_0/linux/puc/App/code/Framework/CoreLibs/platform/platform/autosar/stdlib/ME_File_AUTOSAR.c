...
PlatformError_t ME_File_Open_t(FileHandle_s* b_Handle_ps, const char_t* i_FileName_pc, uint32_t i_OpenFlags_u32)
{
  ...
  if ((NULL != i_FileName_pc) && (NULL != b_Handle_ps))
  {
    ...
    // 2. Open file
    b_Handle_ps->FileHandle_po = 0; //TODO: fopen(i_FileName_pc, (const char_t*) v_OpenMode_ac);

    // 3. Check for errors
    if (NULL == b_Handle_ps->FileHandle_po)
    {
...
uint64_t ME_File_Read_u64(FileHandle_s* i_Handle_ps, void* i_Buffer_pv, uint64_t i_BufferSize_u64) /* PRQA S 3673 */ /* keep this function parameter not const */
{
  ...
  {
    v_NumBytesRead_u64 = 0; //TODO: fread(i_Buffer_pv, 1, i_BufferSize_u64, i_Handle_ps->FileHandle_po);
  }
...
uint64_t ME_File_SetPointer_u64(FileHandle_s* i_Handle_ps, sint64_t i_DistanceToMove_s64, FilePointer_t i_StartingPoint_e)/* PRQA S 3673 */ /* keep this function parameter not const */
{
  ...
  if (i_Handle_ps != NULL)
  {
...
#if 0
    if( fseeko(i_Handle_ps->FileHandle_po, i_DistanceToMove_s64, seekModeMapping[i_StartingPoint_e]) != 0) /* TODO */
    {
      // seek fail
    ...
    {
      v_NewFilePointer_u64 = 0; //TODO: (uint64_t)ftello(i_Handle_ps->FileHandle_po);
    }
#endif
  }
...
uint64_t ME_File_GetSize_u64(FileHandle_s* i_Handle_ps)
{
  ...
  if (i_Handle_ps != NULL)
  {
    sint64_t v_CurrFilePointer_s64 = 0; //TODO: (sint64_t)ftello(i_Handle_ps->FileHandle_po);
    ...
    else
    {
      ...
      if (0) //TODO: fseeko(i_Handle_ps->FileHandle_po, 0, SEEK_END) != 0)
      {
        // something went wrong
        v_FileSize_u64 = 0;
      }
      ...
      {
        // get size
        v_FileSize_u64 = 0; //TODO: (uint64_t)ftello(i_Handle_ps->FileHandle_po);

        // Reset File pointer
        //TODO: (void)fseeko(i_Handle_ps->FileHandle_po, v_CurrFilePointer_s64, SEEK_SET);
      }
    }
  }
...
PlatformError_t ME_File_Close_t(const FileHandle_s* b_Handle_ps)
{
  ...
  if((b_Handle_ps != NULL) && (NULL != b_Handle_ps->FileHandle_po))
  {
    // returns: Zero for success, or -1 if an error occurs (errno is set).
    sint32_t v_Result_s32 = 0; //TODO: fclose(b_Handle_ps->FileHandle_po);
