...
#ifndef ME_CPP_HELPERS_H
#define ME_CPP_HELPERS_H

#include "platform/PlatformSpecs.h"

#ifndef ___AUTOSAR___
...
//    #define makeType(i_Var) mepl::ActualType<__typeof(i_Var)>::type
//  #endif // !__typeof
#endif // c++-11
...
/// General namespace for platform library
namespace mepl
{
...
/// Common template to remove the const qualifier
template<typename T>
struct RemoveConst
{
    typedef T type;
...
/// Template specialization that removes the actual const qualifier
template<typename T>
...
/// IsPrimitive is true basically for all types that are defined in PlatformTypes.h
...
/// Determines (at compile time) if a given type is a POD struct or class or a primitive type.
/// A pod type is a type that does not contain virtual methods (such as simple structs).
// PRQA S 1051 ++ // example code
/// @par Example
/// @code{.cpp}
/// StaticAssert(platform::IsSimplePod<MyClass>::value, "Virtual methods not allowed for MyClass!");
/// @endcode
// PRQA S 1051 -- // example code end
...
/// @par Example
/// @code{.cpp}
/// platform::checkSimple_v(myItem);
/// @endcode
// PRQA S 1051 -- // example code end
template <typename T>
void checkSimple_v(const T& i_CheckItem_o)
{
  StaticAssert(mepl::IsSimplePod<T>::value, "Please use only objects without vtable!");
  // parameter is only used for the static check of the parameter type
  mepl::UnusedParameter(i_CheckItem_o);
}
...
template<>
ME_INLINE void swapBytes_v<2U>(void* mem_pv)
{
...
template<>
ME_INLINE void swapBytes_v<4U>(void* mem_pv)
{
...
template<>
ME_INLINE void swapBytes_v<8U>(void* mem_pv)
{
...
template <typename T>
ME_INLINE void swapByteOrder_v(T& i_Value_rx)
{
...
// PRQA S 1051 -- // example code end
template <typename T>
ME_INLINE void swapByteOrderArray_v(T* i_Value_px, uint32_t i_Size_u32)
{
...
template<uint32_t size_u32>
ME_INLINE uint8_t* copyBytes_pv(uint8_t* o_MemDest_pv, const uint8_t* i_MemSrc_pv)
{
...
template <typename T>
ME_INLINE void* copyBytesTo_pv(void* o_Dest_pv,
...
template <typename T>
ME_INLINE const void* copyBytesFrom_pv(T& o_DestValue_rx,
...
/// @return the destination pointer behind the copied data.
template <typename T>
ME_INLINE void* copyBytesToArray_pv(void* o_Dest_pv,
...
/// @return the source pointer behind the copied data.
template <typename T>
ME_INLINE const void* copyBytesFromArray_pv(T* o_Value_px,
                                         const void* i_SrcPointer_pv,
                                         uint32_t i_Size_u32)
{
...
/// Helper struct used by CheckInheritance.
...
// PRQA S 1051 7 // comment contains example code
/// Check if a class (i.e. a template class parameter) derives from a specific class.
/// @par Example
/// @code{.cpp}
/// static const bool_t isDerived_b = platform::CheckInheritance<IFileIterator, FileIterator>::isDerived_b;
/// StaticAssert(isDerived_b, "FileIterator must derive from IFileIterator interface");
/// @endcode
/// see http://stackoverflow.com/questions/2910979/how-does-is-base-of-work
template <typename Base, typename Derived>
struct CheckInheritance
{
  typedef char_t (&Yes_t)[1];
  typedef char_t (&No_t)[2];

  template <typename T>
  static Yes_t check_t(Derived*, T);
  static No_t check_t(Base*, sint32_t);

  static const bool_t isDerived_b = sizeof(check_t(Host<Base,Derived>(), uint32_t())) == sizeof(Yes_t);
};


/// Helper used by HighestBit struct to calculate the highest bit.
/// @see HighestBit
template <uint32_t N, uint32_t LogN>
struct CalcHighestBit
{
  static const uint32_t c_Value_u32 = CalcHighestBit<N/2,LogN+1>::c_Value_u32;
};

/// Template specialization for the lowest number 1.
template<uint32_t LogN>
struct CalcHighestBit<1,LogN>
{
  static const uint32_t c_Value_u32 = LogN;
};

/// Determines for a given number N the index of the highest bit used.
/// E.g. for 1 it returns 0, for 15 it returns 3, for 16 it returns 4.
/// This is basically floor(log2(N)).
template <uint32_t N>
struct HighestBit
{
  static const uint32_t c_Value_u32 = CalcHighestBit<N, 0>::c_Value_u32;
};
