...
#ifndef __WINDOWS_DEBUGGER__H_
#define __WINDOWS_DEBUGGER__H_
...
namespace db
{
    ...
    */
    ...
    public:
        ...
        void Imshow( const char* winname, cv::Mat mat );
        template<typename T>
    ...
    private:
        ...
        cv::Point cvPoint( db::Point p );

        cv::ImageView* getImageView( const char* winname, int width = 0, int height = 0, int type = 0 );
        db::Plotter* getPlotter( const char* winname );

        template<typename T>
        cv::Mat createDisplayMat( image::Image<T>& image, T maxVal, T minVal );
        template<typename T>
        T FindMax( const image::Image<T>& image );
        template<typename T>
        T FindMin( const image::Image<T>& image );
