...
namespace core
{
    ...
    public:
        ...
        virtual IteratorType begin_o()
        {
            return const_cast<const Collection*>( this )->begin_o();
        }
        /// Non-const access to end of the collection. Simply wraps the const version by default.
        virtual IteratorType end_o()
        {
            return const_cast<const Collection*>( this )->end_o();
        }

        /// The maximum number of elements that this collection can hold
        inline SizeType maxSize_u() const
        {
            return MaxSize;
        }
        /// the number of items in the collection
        virtual SizeType size_u() const
        {
            return MaxSize;
        }
        /// checks whether a list contains elements
        virtual bool isEmpty_b() const
        {
            return size_u() == 0U;
        }
        ...
        virtual bool isFull_b() const
        {
            return size_u() == maxSize_u();
        }
