...
public:
    ...
    {}
        ...
        tscApi::enuCameraID getID_t() const
        {
            return id_t;
        }
        bool_t valid_b() const
        {
            return isValid_b;
        }
...
#endif
        mecl::model::Camera<float32_t>* getCameraObj_px( void ) const
        {
            return cameraObj_px;
        }
        float64_t getPreRollKmcs_f64() const
        {
            return preRollKmcs_f64;
        }
    // setter
        void PutID( tscApi::enuCameraID i_CameraID_t )
        {
            id_t = i_CameraID_t;
        }
...
//-------------------------------------------------------------------------
    ...
    inline bool_t CameraModelMecl::LoadConfig( tscApi::CameraModelMeclCfg_s const* i_CameraModelMeclCfg_pt )
    {
        ...
        if( i_CameraModelMeclCfg_pt->camera_px != reinterpret_cast<mecl::model::Camera<float32_t>*>( tsc_cfg::CAMERAMODEL_MECL_OBJ_UNDEFINED ) )
        {
            ...
            tsc_math::toRange( extrinsicRoll_f32, 180.0F );
