...
#define ENABLE_TSC_PROFILE 0
#define DUMP_ALL_CAM_IMAGES_CAN_DATA 0

#ifdef OC_LOG_PRINTF_ON
#define OC_DEBUG_PRINTF(x) ME_Debugger_F_Printf_v x
#else
...
public:
        ...
        videoHeight_u16( 0 )
    {
        ...
        const uint8_t* GetM_Cmrs( enuCameraID i_Param_t ) const
        {
            return c_Cameras_apu8[i_Param_t];    // PRQA S 4628
        }
        const uint8_t** GetMAddr_Cmrs( enuCameraID i_Param_t )
        {
            return &c_Cameras_apu8[i_Param_t];
        }
        float32_t getMSpd_f32( void ) const
        {
            return speed_f32;
        }
        float32_t getMWhlAngl_f32( void ) const
        {
            return wheelAngle_f32;
        }
        sint32_t getMGrDrctn_s32( void ) const
        {
            return gearDirection_s32;
        }
        bool_t getMCmrFrntOpn_b( void ) const
        {
            return cameraFrontOpen_b;
        }
        uint32_t getMFrmNmbr_u32( void ) const
        {
            return frameNumber_u32;
        }
        uint16_t getMVdWdth_u16( void ) const
        {
            return videoWidth_u16;
        }
        uint16_t getMVdHght_u16( void ) const
        {
            return videoHeight_u16;
        }
        TSCSavedDataInfo_s* getMPSvdDtInf_ps( uint8_t i_Param_u8 ) const
        {
            return savedDataInfo_aps[i_Param_u8];    // PRQA S 4628
        }

        void PutM_Cmrs( enuCameraID i_ID_t, const uint8_t* i_Param_pu8 )
        {
            c_Cameras_apu8[i_ID_t] = i_Param_pu8;
        }
        void PutM_Spd( float32_t i_Param_f32 )
        {
            speed_f32 = i_Param_f32;
        }
        void PutM_WhlAngl( float32_t i_Param_f32 )
        {
            wheelAngle_f32 = i_Param_f32;
        }
        void PutM_GrDrctn( sint32_t i_Param_s32 )
        {
            gearDirection_s32 = i_Param_s32;
        }
        void PutM_CmrFrntOpn( bool_t i_Param_b )
        {
            cameraFrontOpen_b = i_Param_b;
        }
        void PutM_FrmNmbr( uint32_t i_Param_u32 )
        {
            frameNumber_u32 = i_Param_u32;
        }
        void PutM_IncFrmNmbr( uint32_t i_Param_u32 )
        {
            frameNumber_u32 += i_Param_u32;
        }
        void PutM_VdWdth( uint16_t i_Param_u16 )
        {
            videoWidth_u16 = i_Param_u16;
        }
        void PutM_VdHght( uint16_t i_Param_u16 )
        {
            videoHeight_u16 = i_Param_u16;
        }
        //PRQA S 6200 1 //If i_Param_ps is const, compiler would complain for assignment
        void PutM_PSvdDtInf( uint8_t i_ID_u8, TSCSavedDataInfo_s* i_Param_ps )
        {
            savedDataInfo_aps[i_ID_u8] = i_Param_ps;
        }
...
bool_t TSC_GetFinalCalibrationResultStdDev( tscApi::enuCameraID cameraID, tscApi::CalibrationParams_s* finalCalibrationiResultStdDev );
#endif
// PRQA S 2642 --
// PRQA S 2000 --
