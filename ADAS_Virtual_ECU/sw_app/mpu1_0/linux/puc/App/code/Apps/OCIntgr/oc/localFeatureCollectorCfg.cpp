...
{
  /* DD-ID: {6FBF0720-B358-45fb-9840-B72B7C399582}*/
    bool_t v_Ret_b = true;
...
{
  /* DD-ID: {2743974A-9F4E-411d-850C-13173BC95391}*/
    bool_t v_Ret_b = true;
    ...
    while(v_FoundRoiPools_u32 < v_Endwh_u32)
    {
        ...
        while( v_FoundRoiPerPool_u32 < v_Endlp_u32 )
        {
            ...
            {
                TRACE_1( m_hTracer, "Element [ROI] #%u has invalid [Rect] attribute values", ( v_FoundRoiPerPool_u32 + 1 ) );
                v_Ret_b = false;
            }
            else
            {
        ...
        if( v_Ret_b )
        {
            ...
            {
                TRACE_3( m_hTracer, "Element [Pool] #%u has unexpected number of ROIs{%d}. Expected{%d}",  \
    ...
    {
        TRACE_2( m_hTracer, "Element [ROIPools] has unexpected number of Pool{%d}. Expected{%d}", v_FoundRoiPools_u32, params_o.getNmROIPls_u32() );
        v_Ret_b = false;
    }

    if( params_o.getNmPckdROIs_u32() > params_o.getNmROIPls_u32() )
    {
        TRACE_2( m_hTracer, "Invalid config parameter: NumROIs [%u] cannot be greater than total number of available ROI Pools [%u]",  \
...
{
  /* DD-ID: {A03D5BF2-B95F-45d2-87DC-0B1AFA48A93A}*/
    bool_t v_Ret_b = true;
