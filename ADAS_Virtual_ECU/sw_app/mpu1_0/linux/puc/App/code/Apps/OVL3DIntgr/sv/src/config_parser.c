...
#define LANG_TABLE_SZ 512

static int32_t   languges_table_dst[LANG_TABLE_SZ*4][4] = {0};
static float languges_table_src[LANG_TABLE_SZ][4] = {0};
static uint32_t languges_table_dst_sz = LANG_TABLE_SZ*4;
static uint32_t languges_table_src_sz = LANG_TABLE_SZ;

const char* languges_path = NULL;
...
static inline int parser_open_from_mem(yaml_parser_t *p, char* data, uint32_t size)
{
    /* ...open parser */
    if (!yaml_parser_initialize(p))
    {
    ...
    {
        /* ...set input file */
        yaml_parser_set_input_string(p, (uint8_t*)data, size);
    }
...
static inline int parse_float(float *f, yaml_document_t *doc, yaml_node_t *v)
{
    char    *t, *__p;
...
/* ...compounds parsing */
static int parse_array_of_2d_points(float32_t* m, int* out_size, yaml_document_t *doc, yaml_node_t *v)
{
    yaml_node_item_t   *item;
    ...
    int size = __SEQ_LENGTH(v);
    CHK_ERR(size > 0, syntax_error(doc, v, "sequence expected"));
    ...
    CHK_ERR(size <= max_size, syntax_error(doc, v, "invalid size: %d expected (%d got)", max_size, size));

    /* ...parse all nodes */
    for (item = v->data.sequence.items.start; size > 0; size--, item++)
    {
        CHK_API(parse_vector(m, 2, doc, yaml_document_get_node(doc, *item)));
        m+=2;
    }
...
/* ...compounds parsing */
static int parse_array_any_length(float32_t* m, int* out_size, yaml_document_t *doc, yaml_node_t *v)
{
    yaml_node_item_t   *item;
...
static int check_zeros_32(void *p)
{
    uint32_t *m = p;
    return (m[0] == 0 && m[1] == 0 && m[2] == 0 && m[3] == 0);
}

static int parse_vector5(float32_t **ppDst, yaml_document_t *doc, yaml_node_t *v)
{
    yaml_node_item_t   *item;
    ...
    int size = __SEQ_LENGTH(v);
    /* ...check its size is valid */
    CHK_ERR(__SEQ_LENGTH(v) == 5 || __SEQ_LENGTH(v) == 4, syntax_error(doc, v, "invalid size: 4-5 expected (%d got)", __SEQ_LENGTH(v)));
    ...
    if (size == 4)
    {
        if (languges_table_src_sz < 1)
            ret = -1;

        if (ret == 0)
        {
            indx = languges_table_src_sz - 1;

            if (!check_zeros_32(&languges_table_src[indx][0]))
                ret = -1;
        }

        if (ret == 0)
        {
            languges_table_src[indx][0] = buf[0];
            languges_table_src[indx][1] = buf[1];
            languges_table_src[indx][2] = buf[2];
            languges_table_src[indx][3] = buf[3];
            languges_table_src_sz--;
            *ppDst = &languges_table_src[indx][0];
        }
    }
    ...
    if (size == 5)
    {
        ...
        if (indx >= languges_table_src_sz)
            ret = -1;

        if (ret == 0)
        {
            memcpy(&languges_table_src[indx][0], buf, 4 * sizeof(buf[0]));
            *ppDst = &languges_table_src[indx][0];
        }
    }

    if (ret == -1)
        syntax_error(doc, v, "invalid language index");

    return ret;
}

/* ...compounds parsing */
static int parse_vector2(float32_t (*m)[2], int size, yaml_document_t *doc, yaml_node_t *v)
{
    yaml_node_item_t   *item;
...
static int parse_ivector5(int32_t **ppDst, yaml_document_t *doc, yaml_node_t *v)
{
    yaml_node_item_t   *item;
    ...
    uint32_t size = __SEQ_LENGTH(v);

    /* ...check its size is valid */
    CHK_ERR(__SEQ_LENGTH(v) == 4 || __SEQ_LENGTH(v) == 5, syntax_error(doc, v, "invalid size: 4-5 expected (%d got)", __SEQ_LENGTH(v)));
    ...
    if (size == 4)
    {
        if (languges_table_dst_sz < 1)
            ret = -1;

        if (ret == 0)
        {
            indx = languges_table_dst_sz - 1;

            if (!check_zeros_32(&languges_table_dst[indx][0]))
                ret = -1;
        }

        if (ret == 0)
        {
            languges_table_dst[indx][0] = buf[0];
            languges_table_dst[indx][1] = buf[1];
            languges_table_dst[indx][2] = buf[2];
            languges_table_dst[indx][3] = buf[3];
            languges_table_dst_sz--;
            *ppDst = &languges_table_dst[indx][0];
        }
    }
    ...
    if (size == 5)
    {
        ...
        if (indx >= languges_table_dst_sz)
            ret = -1;

        if (ret == 0)
        {
            memcpy(&languges_table_dst[indx][0], buf, 4 * sizeof(buf[0]));
            *ppDst = &languges_table_dst[indx][0];
        }
    }
...
static inline int parse_cc_config(cfg_cc_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        else if (!strcmp(t, "size-mm"))
        {
            CHK_API(parse_float(&cfg->size_mm, doc, value));
        }
        else if (!strcmp(t, "offset-x"))
        {
            CHK_API(parse_float(&cfg->offset_x, doc, value));
        }
        else if (!strcmp(t, "offset-y"))
        {
            CHK_API(parse_float(&cfg->offset_y, doc, value));
        }
        else if (!strcmp(t, "enable"))
        {
            CHK_API(parse_int(&cfg->enable, doc, value));
        }
        else
        {
...
static inline int parse_vehicle_config(cfg_vehicle_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        {
            CHK_API(parse_vector(cfg->steering_coef, 3, doc, value));
        }
...
{
    if (strstr(name, "q_")) return 2;
    if (strstr(name, "cs_")) return 1;
    if (strstr(name, "cl_")) return 0;

    return 0;
}

static inline int get_id_crit(const char* name)
{
    if (strstr(name, "-caution")) return 0;
    if (strstr(name, "-crit")) return 1;
...
static inline int parse_sonar_config(cfg_sonar_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (strstr(t, "sonar-car-model-dst") != NULL)
        {
            CHK_API(parse_ivector(cfg->sonar_car_model_dst[get_id(t)], 4, doc, value));
        }
        ...
        else if (strstr(t, "sonar-fc") != NULL)
        {
            CHK_API(parse_ivector(cfg->sonar_fc_pas_dst[get_id_crit(t)][get_id(t)], 4, doc, value));
        }
        else if (strstr(t, "sonar-fr") != NULL)
        {
            CHK_API(parse_ivector(cfg->sonar_fr_pas_dst[get_id_crit(t)][get_id(t)], 4, doc, value));
        }
        else if (strstr(t, "sonar-fl") != NULL)
        {
            CHK_API(parse_ivector(cfg->sonar_fl_pas_dst[get_id_crit(t)][get_id(t)], 4, doc, value));
        }
        else if (strstr(t, "sonar-rc") != NULL)
        {
            CHK_API(parse_ivector(cfg->sonar_rc_pas_dst[get_id_crit(t)][get_id(t)], 4, doc, value));
        }
        else if (strstr(t, "sonar-rr") != NULL)
        {
            CHK_API(parse_ivector(cfg->sonar_rr_pas_dst[get_id_crit(t)][get_id(t)], 4, doc, value));
        }
        else if (strstr(t, "sonar-rl") != NULL)
        {
            CHK_API(parse_ivector(cfg->sonar_rl_pas_dst[get_id_crit(t)][get_id(t)], 4, doc, value));
        }
        ...
        else if (strstr(t, "door-open-dst") != NULL)
        {
            CHK_API(parse_ivector4(cfg->sonar_door_open_dst[get_id(t)], 4, doc, value));
        }
        else if (strstr(t, "door-closed-dst") != NULL)
        {
            CHK_API(parse_ivector4(cfg->sonar_door_closed_dst[get_id(t)], 4, doc, value));
        }
        else if (!strcmp(t, "q_trunk-open-dst"))
        {
            CHK_API(parse_ivector(cfg->sonar_trunk_open_dst[2], 4, doc, value));
        }
        else if (!strcmp(t, "cs_trunk-open-dst"))
        {
            CHK_API(parse_ivector(cfg->sonar_trunk_open_dst[1], 4, doc, value));
        }
        else if (!strcmp(t, "cl_trunk-open-dst"))
        {
            CHK_API(parse_ivector(cfg->sonar_trunk_open_dst[0], 4, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
...
/**
 * @brief   Parse hitch configuration
 *
 * @param   cfg[in]         TRSC configuration data
 * @param   doc[in]         YAML document
 * @param   root[in]        YAML node of section
 *
 * @return                  Negative on failure
 */
static inline int parse_hitch_config(cfg_hitch_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "slider"))
        {
            CHK_API(parse_ivector(cfg->slider, 4, doc, value));
        }
        else if (!strcmp(t, "start"))
        {
            CHK_API(parse_ivector(cfg->start, 4, doc, value));
        }
        else if (!strcmp(t, "stop"))
        {
            CHK_API(parse_ivector(cfg->stop, 4, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
...
/**
 * @brief   Parse TRSC configuration
 *
 * @param   cfg[in]         TRSC configuration data
 * @param   doc[in]         YAML document
 * @param   root[in]        YAML node of section
 ...
 * @return                  Negative on failure
 */
static inline int parse_trsc_config(cfg_trsc_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "pivot"))
        {
            CHK_API(parse_vector(cfg->pivot, 2, doc, value));
        }
        else if (!strcmp(t, "base"))
        {
            CHK_API(parse_ivector(cfg->base, 4, doc, value));
        }
        else if (!strcmp(t, "dotted"))
        {
            CHK_API(parse_ivector(cfg->dotted, 4, doc, value));
        }
        else if (!strcmp(t, "trailer"))
        {
            CHK_API(parse_ivector(cfg->trailer, 4, doc, value));
        }
        else if (!strcmp(t, "q_vehicle"))
        {
            CHK_API(parse_ivector(cfg->q_vehicle, 4, doc, value));
        }
        else if (!strcmp(t, "cl_vehicle"))
        {
            CHK_API(parse_ivector(cfg->cl_vehicle, 4, doc, value));
        }
        else if (!strcmp(t, "cs_vehicle"))
        {
            CHK_API(parse_ivector(cfg->cs_vehicle, 4, doc, value));
        }
        else if (!strcmp(t, "offset"))
        {
            CHK_API(parse_ivector(cfg->offset, 5, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
...
static inline int parse_guide_dynamic_fr_config(cfg_guide_dynamic_fr_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        else if (!strcmp(t, "center-trajectory-width"))
        {
            CHK_API(parse_float(&cfg->center_trajectory_width, doc, value));
        }
        else if (!strcmp(t, "center-trajectory-z"))
        {
            CHK_API(parse_float(&cfg->center_trajectory_z, doc, value));
        }
        else if (!strcmp(t, "distance-marker-middle"))
        {
            CHK_API(parse_float(&cfg->distance_marker_middle, doc, value));
        }
        ...
        else if (!strcmp(t, "ffc-width"))
        {
            CHK_API(parse_float(&cfg->ffc_width, doc, value));
        }
        else if (!strcmp(t, "ffc-tire-width"))
        {
            CHK_API(parse_float(&cfg->ffc_tire_width, doc, value));
        }
        else if (!strcmp(t, "ffc-hidden"))
        {
            CHK_API(parse_float(&cfg->ffc_hidden, doc, value));
        }
        else if (!strcmp(t, "ffc-length"))
        {
            CHK_API(parse_float(&cfg->ffc_length, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
...
/**
 * @brief   Parse top-view hatched area configuration
 *
 * @param   cfg[in]         top-view hatched area data
 * @param   doc[in]         YAML document
 * @param   root[in]        YAML node of section
 *
 * @return                  Negative on failure
 */
static inline int parse_guide_tv_hatched_area_config(cfg_guide_tv_hatched_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "points"))
        {
            int max_size = TV_HATCHED_MAX_POINTS;
            CHK_API(parse_array_of_2d_points(&cfg->points[0][0], &max_size, doc, value));
            cfg->num_points = max_size;
        }
        else if (!strcmp(t, "rounding-off-corners"))
        {
            int max_size = TV_HATCHED_MAX_POINTS;
            CHK_API(parse_array_any_length(&cfg->rounding_off_corners[0], &max_size, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
...
/**
 * @brief   Parse THA guidelines configuration
 *
 * @param   cfg[in]         Guidelines configuration data
 * @param   doc[in]         YAML document
 * @param   root[in]        YAML node of section
 ...
 * @return                  Negative on failure
 */
static inline int parse_guide_tha_config(cfg_guide_tha_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "offset"))
        {
            CHK_API(parse_vector(cfg->offset, 3, doc, value));
        }
        else if (!strcmp(t, "angle"))
        {
            CHK_API(parse_float(&cfg->angle, doc, value));
        }
        else if (!strcmp(t, "det-angle"))
        {
            CHK_API(parse_float(&cfg->det_angle, doc, value));
        }
        else if (!strcmp(t, "length"))
        {
            CHK_API(parse_float(&cfg->length, doc, value));
        }
        else if (!strcmp(t, "line-width"))
        ...
        else if (!strcmp(t, "border-width"))
        {
            CHK_API(parse_float(&cfg->border_width, doc, value));
        }
        else if (!strcmp(t, "canvas-color"))
        {
            CHK_API(parse_ui8vector(cfg->canvas_color, 3, doc, value));
        }
        else if (!strcmp(t, "canvas-transmittance"))
        {
            CHK_API(parse_ui8(&cfg->canvas_transmittance, doc, value));
        }
        else if (!strcmp(t, "line-color"))
        {
            CHK_API(parse_ui8vector(cfg->line_color, 3, doc, value));
        }
        else if (!strcmp(t, "line-transmittance"))
        {
            CHK_API(parse_ui8(&cfg->line_transmittance, doc, value));
        }
        else if (!strcmp(t, "radius"))
        {
            CHK_API(parse_float(&cfg->radius, doc, value));
        }
        else if (!strcmp(t, "arc-len"))
        {
            CHK_API(parse_float(&cfg->arc_len, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
...
static inline int parse_guide_config(cfg_guide_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        else if (!strcmp(t, "hatched-area"))
        {
            CHK_API(parse_guide_tv_hatched_area_config(&cfg->tv_hatched, doc, value));
        }
        else if (!strcmp(t, "tha"))
        {
            CHK_API(parse_guide_tha_config(&cfg->tha, doc, value));
        }
...
/**
 * @brief   Parse STV-PAS configuration
 *
 * @param   cfg[in]         STV-PAS configuration data
 * @param   doc[in]         YAML document
 * @param   root[in]        YAML node of section
 *
 * @return                  Negative on failure
 */
static inline int parse_stv_pas_config(cfg_stv_pas_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "sonar-origin"))
        {
            CHK_API(parse_vector2(cfg->sonar_origin, 6, doc, value));
        }
        else if (!strcmp(t, "sonar-direction"))
        {
            CHK_API(parse_vector(cfg->sonar_direction, 6, doc, value));
        }
        ...
        else if (!strcmp(t, "stv-obstacle-dim"))
        {
            CHK_API(parse_vector(cfg->stv_obstacle_dim, 3, doc, value));
        }
        else if (!strcmp(t, "stv-obstacle-dim-scale"))
        {
            CHK_API(parse_vector(cfg->stv_obstacle_dim_scale, 3, doc, value));
        }
        else if (!strcmp(t, "stv-obstacle-switch-factor"))
        {
            CHK_API(parse_float(&cfg->stv_obstacle_switch_factor, doc, value));
        }
        else if (!strcmp(t, "stv-obstacle-fadeout-duration"))
        {
            CHK_API(parse_uint(&cfg->stv_obstacle_fadeout_duration, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
 ...
 * @return                  Negative on failure
 */
static inline int parse_ctlval_mesh_config(cfg_ctlval_mesh_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "cameras"))
        {
            CHK_API(parse_uint(&cfg->cameras, doc, value));
        }
        else if (!strcmp(t, "output-point"))
        {
            CHK_API(parse_ivector(cfg->output_point, 2, doc, value));
        }
...
static inline int parse_scv_mesh_config(cfg_scv_mesh_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        else if (!strcmp(t, "fov-adj-coef"))
        {
            CHK_API(parse_vector(cfg->fov_adj_coef, 4, doc, value));
        }
        else if (!strcmp(t, "yaw-adj-coef"))
        {
            CHK_API(parse_float(&cfg->yaw_adj_coef, doc, value));
        }
        else if (!strcmp(t, "distortion"))
        {
        ...
        else if (!strcmp(t, "zoom"))
        {
            CHK_API(parse_float(&cfg->zoom, doc, value));
        }
        else if (!strcmp(t, "zoom-point"))
        {
            CHK_API(parse_vector(cfg->zoom_point, 2, doc, value));
        }
...
/**
 * @brief   Parse raw multiples-camera-view configuration
 *
 * @param   cfg[in]         MCV configuration data
 * @param   doc[in]         YAML document
 * @param   root[in]        YAML node of section
 ...
 * @return                  Negative on failure
 */
static inline int parse_raw_mcv_mesh_config(cfg_raw_mcv_mesh_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        else if (!strcmp(t, "items"))
        {
            ...
            /* ...allocate array of raw mesh configurations */
            CHK_ERR(cfg->raw = calloc(cfg->number, sizeof(*cfg->raw)), -1);

            for (subitem = value->data.sequence.items.start; subitem != value->data.sequence.items.top; subitem++)
            {
                CHK_API(parse_raw_mesh_config(&cfg->raw[__SEQ_INDEX(value, subitem)], doc, yaml_document_get_node(doc, *subitem)));
            }
        }
        else
        {
...
static inline int parse_mesh_config(cfg_mesh_t **mesh, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "type"))
        {
            ...
            else if (!strcmp(v, "ctl"))
            {
                CHK_ERR(*mesh = calloc(1, sizeof((*mesh)->ctl)), -1);
                (*mesh)->base.type = MESH_CTL;
                CHK_API(parse_ctlval_mesh_config(&(*mesh)->ctl, doc, root));
            }
            ...
            else if (!strcmp(v, "raw-mc"))
            {
                CHK_ERR(*mesh = calloc(1, sizeof((*mesh)->raw_mc)), -1);
                (*mesh)->base.type = MESH_RAW_MCV;
                CHK_API(parse_raw_mcv_mesh_config(&(*mesh)->raw_mc, doc, root));
            }
...
static uint32_t cfg_icon_name_lookup(char *v)
{
    ...
    else if (strstr(v, "car-model") != NULL)
    {
        id = ICON_CAR_MODEL0 + get_id(v);
    }
    else if (strstr(v, "fl_open") != NULL)
    {
        id = ICON_DOOR_FL_OPEN0 + get_id(v);
    }
    else if (strstr(v, "fr_open") != NULL)
    {
        id = ICON_DOOR_FR_OPEN0 + get_id(v);
    }
    else if (strstr(v, "rl_open") != NULL)
    {
        id = ICON_DOOR_RL_OPEN0 + get_id(v);
    }
    else if (strstr(v, "rr_open") != NULL)
    {
        id = ICON_DOOR_RR_OPEN0 + get_id(v);
    }
    else if (strstr(v, "fl_closed") != NULL)
    {
        id = ICON_DOOR_FL_CLOSED0 + get_id(v);
    }
    else if (strstr(v, "fr_closed") != NULL)
    {
        id = ICON_DOOR_FR_CLOSED0 + get_id(v);
    }
    else if (strstr(v, "rl_closed") != NULL)
    {
        id = ICON_DOOR_RL_CLOSED0 + get_id(v);
    }
    else if (strstr(v, "rr_closed") != NULL)
    {
        id = ICON_DOOR_RR_CLOSED0 + get_id(v);
    }
    else if (strstr(v, "tg_open") != NULL)
    {
        id = ICON_TRUNK_OPEN0 + get_id(v);
    }
    else if (strstr(v, "fl_caution")!= NULL)
    {
        id = ICON_PAS_FL_CAUTION0 + get_id(v);
    }
    else if (strstr(v, "fc_caution")!= NULL)
    {
        id = ICON_PAS_FC_CAUTION0 + get_id(v);
    }
    else if (strstr(v, "fr_caution")!= NULL)
    {
        id = ICON_PAS_FR_CAUTION0 + get_id(v);
    }
    else if (strstr(v, "rl_caution")!= NULL)
    {
        id = ICON_PAS_RL_CAUTION0 + get_id(v);
    }
    else if (strstr(v, "rc_caution")!= NULL)
    {
        id = ICON_PAS_RC_CAUTION0 + get_id(v);
    }
    else if (strstr(v, "rr_caution")!= NULL)
    {
        id = ICON_PAS_RR_CAUTION0 + get_id(v);
    }
    else if (strstr(v, "fl_crit")!= NULL)
    {
        id = ICON_PAS_FL_CRITICAL0 + get_id(v);
    }
    else if (strstr(v, "fc_crit")!= NULL)
    {
        id = ICON_PAS_FC_CRITICAL0 + get_id(v);
    }
    else if (strstr(v, "fr_crit")!= NULL)
    {
        id = ICON_PAS_FR_CRITICAL0 + get_id(v);
    }
    else if (strstr(v, "rl_crit")!= NULL)
    {
        id = ICON_PAS_RL_CRITICAL0 + get_id(v);
    }
    else if (strstr(v, "rc_crit")!= NULL)
    {
        id = ICON_PAS_RC_CRITICAL0 + get_id(v);
    }
    else if (strstr(v, "rr_crit")!= NULL)
    {
        id = ICON_PAS_RR_CRITICAL0 + get_id(v);
    }

    else if (!strcmp(v, "ic-coupler"))
    {
        id = ICON_HITCH_COUPLER;
    }
    else if (!strcmp(v, "ic-coupler.locked"))
    {
        id = ICON_HITCH_COUPLER_LOCKED;
    }
    else if (!strcmp(v, "ic-slider"))
    {
        id = ICON_HITCH_SLIDER;
    }
    else if (!strcmp(v, "ic-slider.locked"))
    {
        id = ICON_HITCH_SLIDER_LOCKED;
    }
    else if (!strcmp(v, "ic-target"))
    {
        id = ICON_COUPLER_TARGET;
    }

    else if (!strcmp(v, "ic-trsc-base.svc"))
    {
        id = ICON_TRSC_BASE_SVC;
    }
    else if (!strcmp(v, "ic-trsc-orange.svc"))
    {
        id = ICON_TRSC_ORANGE_SVC;
    }
    else if (!strcmp(v, "ic-trsc-red.svc"))
    {
        id = ICON_TRSC_RED_SVC;
    }
    else if (!strcmp(v, "ic-trsc-orange-red.svc"))
    {
        id = ICON_TRSC_ORANGE_RED_SVC;
    }
    else if (!strcmp(v, "ic-trsc-dotted.svc"))
    {
        id = ICON_TRSC_DOTTED_SVC;
    }
    else if (!strcmp(v, "ic-trsc-trailer.svc"))
    {
        id = ICON_TRSC_TRAILER_SVC;
    }
    else if (strstr(v, "ic-trsc-vehicle.svc") != NULL)
    {
        id = ICON_TRSC_VEHICLE_SVC0 + get_id(v);
    }
    else if (!strcmp(v, "ic-trsc-base.trg"))
    {
        id = ICON_TRSC_BASE_TRG;
    }
    else if (!strcmp(v, "ic-trsc-orange.trg"))
    {
        id = ICON_TRSC_ORANGE_TRG;
    }
    else if (!strcmp(v, "ic-trsc-red.trg"))
    {
        id = ICON_TRSC_RED_TRG;
    }
    else if (!strcmp(v, "ic-trsc-orange-red.trg"))
    {
        id = ICON_TRSC_ORANGE_RED_TRG;
    }
    else if (!strcmp(v, "ic-trsc-dotted.trg"))
    {
        id = ICON_TRSC_DOTTED_TRG;
    }
    else if (!strcmp(v, "ic-trsc-trailer.trg"))
    {
        id = ICON_TRSC_TRAILER_TRG;
    }
    else if (!strcmp(v, "ic-trsc-trailer-red.trg"))
    {
        id = ICON_TRSC_TRAILER_RED_TRG;
    }
    else if (strstr(v, "ic-trsc-vehicle.trg") != NULL)
    {
        id = ICON_TRSC_VEHICLE_TRG0 + get_id(v);
    }
    ...
    else if (!strcmp(v, "repeat-ffc-tex"))
    {
        id = ICON_GL_FFC;
    }
    else
...
static inline int parse_vm_config(cfg_vm_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        else if (!strcmp(t, "hitch"))
        {
            CHK_API(parse_hitch_config(&cfg->hitch, doc, value));
        }
        else if (!strcmp(t, "trsc-svc"))
        {
            CHK_API(parse_trsc_config(&cfg->trsc_svc, doc, value));
        }
        else if (!strcmp(t, "trsc-trg"))
        {
            CHK_API(parse_trsc_config(&cfg->trsc_trg, doc, value));
        }
        else
        {
            TRACE(DEBUG, _b("ignore unrecognized parameter '%s'"), t);
...
static inline int parse_ui_buttons_config(cfg_ui_file_t *cfg, ui_parse_data_t *ui, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (item = root->data.sequence.items.start; item != root->data.sequence.items.top; item++)
    {
        ...
        for (pair = key->data.mapping.pairs.start; pair != key->data.mapping.pairs.top; pair++)
        {
            ...
            {
                CHK_API(parse_vector5(&button->src, doc, value));
            }
            else
            {
                TRACE(DEBUG, _b("ignore unknown key '%s'"), t);
...
static inline int parse_ovl_guide_config(cfg_ovl_guide_t *ovl, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "subtype"))
        {
            ...
            if (!strcmp(v, "ffc"))
            {
                ovl->subtype = GUIDE_FFC;
            }
            else if (!strcmp(v, "hatched"))
            {
                ovl->subtype = GUIDE_HATCHED;
            }
            else if (!strcmp(v, "front"))
            {
            ...
            else if (!strcmp(v, "tha"))
            {
                ovl->subtype = GUIDE_THA;
            }
            else
            {
...
static inline int parse_ovl_icon_config(cfg_ovl_icon_t *ovl, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "icon"))
        {
            ...
            else if (!strcmp(v, "hitch"))
            {
                ovl->subtype = ICON_OVL_HITCH;
            }
            else if (!strcmp(v, "coupler"))
            {
                ovl->subtype = ICON_OVL_COUPLER;
            }
            else if (!strcmp(v, "trsc-svc"))
            {
                ovl->subtype = ICON_OVL_TRSC_SVC;
            }
            else if (!strcmp(v, "trsc-trg-l1"))
            {
                ovl->subtype = ICON_OVL_TRSC_TRG_L1;
            }
            else if (!strcmp(v, "trsc-trg-l2"))
            {
                ovl->subtype = ICON_OVL_TRSC_TRG_L2;
            }
            else if (!strcmp(v, "trsc-trg-r1"))
            {
                ovl->subtype = ICON_OVL_TRSC_TRG_R1;
            }
            else if (!strcmp(v, "trsc-trg-r2"))
            {
                ovl->subtype = ICON_OVL_TRSC_TRG_R2;
            }
            else if (!strcmp(v, "trsc-trg-c"))
            {
                ovl->subtype = ICON_OVL_TRSC_TRG_C;
            }
            else if (!strcmp(v, "ap-sonars"))
            {
                ovl->subtype = ICON_PDT_SONAR;
            }
            else
            {
                return syntax_error(doc, value, "unknown type '%s'", v);
...
static inline int parse_ovl_config(cfg_ovl_t **ovl, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "type"))
        {
            ...
            else if (!strcmp(v, "odo"))
            {
                CHK_ERR(*ovl = calloc(1, sizeof((*ovl)->odo)), -1);
                (*ovl)->type = OVL_ODO;
            }
            else
            {
...
/**
 * @brief   Calculate length of a flattened sequence
 */
static int ui_buttons_array_get_length(yaml_document_t *doc, yaml_node_t *root)
{
    int                 length = 0;
    yaml_node_item_t   *item;
    ...
    /* ...first pass - calculate total length of an array */
    for (item = root->data.sequence.items.start; item != root->data.sequence.items.top; item++)
    {
        yaml_node_t            *node = yaml_document_get_node(doc, *item);
        ...
        {
            int     nested = ui_buttons_array_get_length(doc, node);
        ...
        {
            return syntax_error(doc, node, "sequence or mapping expected");
        }
    }
...
/**
 * @brief   Parse individual buttons array item
 */
static int parse_ui_buttons_array_item(cfg_ui_button_pos_t *button, ui_parse_data_t *ui, yaml_document_t *doc, yaml_node_t *node)
{
    yaml_node_pair_t   *pair;
    ...
    /* ...default visibility control value */
    button->target = 1;
    ...
    for (pair = node->data.mapping.pairs.start; pair != node->data.mapping.pairs.top; pair++)
    {
        ...
        {
            CHK_API(parse_ivector5(&button->dst, doc, value));
        }
        ...
        else if (!strcmp(t, "target"))
        {
            CHK_API(parse_ui8(&button->target, doc, value));
        }
        else if (!strcmp(t, "back"))
        {
            CHK_API(parse_ui8(&button->back, doc, value));
        }
 ...
 * @return                  Negative on failure
 */
static int parse_ui_buttons_array_nested(cfg_ui_page_t *cfg, ui_parse_data_t *ui, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_item_t    *item;
    ...
    {
        yaml_node_t         *node = yaml_document_get_node(doc, *item);
        cfg_ui_button_pos_t *button = &cfg->buttons[cfg->buttons_num];

        if (node->type == YAML_SEQUENCE_NODE)
        {
            /* ...nested sequence; flatten recursively */
            CHK_API(parse_ui_buttons_array_nested(cfg, ui, doc, node));
        }
        else
        {
            /* ...process button item */
            CHK_API(parse_ui_buttons_array_item(button, ui, doc, node));

            /* ...increment total number of buttons processed */
            cfg->buttons_num++;
        }
    }
 ...
 * @return                  Negative on failure
 */
static int parse_ui_buttons_array(cfg_ui_page_t *cfg, ui_parse_data_t *ui, yaml_document_t *doc, yaml_node_t *root)
{
    int                 num;
    ...
    /* ...first pass - calculate total length of an array */
    CHK_API(num = ui_buttons_array_get_length(doc, root));

    /* ...allocate buttons array */
    CHK_ERR(cfg->buttons = calloc(num, sizeof(*cfg->buttons)), -1);

    /* ...get flattened buttons array */
    CHK_API(parse_ui_buttons_array_nested(cfg, ui, doc, root));

    return 0;
}

static inline int parse_ui_buttons_array_1(cfg_ui_page_t *cfg, ui_parse_data_t *ui, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_item_t   *item;
...
static inline int parse_toplevel_config(cfg_data_t *cfg, yaml_document_t *doc, yaml_node_t *root)
{
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        else if (!strcmp(t, "stv-pas"))
        {
            CHK_API(parse_stv_pas_config(&cfg->stv_pas, doc, value));
        }
...
static inline int parse_cfg_overide(cfg_data_t *cfg, uint32_t variant_id, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_pair_t   *pair;
    ...
    for (pair = root->data.mapping.pairs.start; pair != root->data.mapping.pairs.top; pair++)
    {
        ...
        if (!strcmp(t, "id"))
        {
            CHK_API(parse_int(&var_id, doc, value));
        }
        else if (!strcmp(t, "cfg"))
        {
            if (var_id == variant_id)
            {
                CHK_API(parse_toplevel_config(cfg, doc, value));
            }
        }
        else
        {
 ...
 * @return                  Negative on failure
 */
static inline int parse_cfg_overide_array(cfg_data_t *cfg, uint32_t variant_id, yaml_document_t *doc, yaml_node_t *root)
{
    yaml_node_item_t   *item;
    ...
    for (item = root->data.sequence.items.start; item != root->data.sequence.items.top; item++)
    {
        CHK_API(parse_cfg_overide(cfg, variant_id, doc, yaml_document_get_node(doc, *item)));
    }
...
int config_parse(cfg_data_t *cfg, char *data, uint32_t sz)
{
    yaml_parser_t   p;
    yaml_node_t    *root;
    int             r;
    ...
    /* ...create parser */
    CHK_API(parser_open_from_mem(&p, data, sz));
...
int cfg_overide_var_data(cfg_data_t *cfg)
{
    cfg_ui_page_t  *page;

    for(int i = 0; i < cfg->ui.pages_num; i++)
    {
        page = &cfg->ui.pages[i];

        if(!strcmp("fcpv", page->id))
        {
            for(int j = 0; j < page->views_num; j++)
            {               
                if(MESH_SCV == page->views[j]->base.type)
                {
                    uint32_t camera_id = page->views[j]->scv.id; 
                    
                    for(int k = 0; k < 3; k++)
                    {
                        TRACE(DEBUG, _b("cfg->camera[%d].position[%d] = %f"), page->views[j]->scv.id, k, cfg->camera[camera_id].position[k]);
                        TRACE(DEBUG, _b("cfg->camera[%d].rotation[%d] = %f"), page->views[j]->scv.id, k, cfg->camera[camera_id].rotation[k]);
                        
                        page->views[j]->scv.position[k] = cfg->camera[camera_id].position[k] + page->views[j]->scv.position[k];
                            
                        TRACE(DEBUG, _b("New page->views[%d]->scv.position[%d] = %f"), j, k, page->views[j]->scv.position[k]);
                        TRACE(DEBUG, _b("New page->views[%d]->scv.rotation[%d] = %f"), j, k, page->views[j]->scv.rotation[k]);
                    }
                    break;
                }
            }
...
/* ...main parsing function*/
int variant_config_parse(cfg_data_t *cfg, uint32_t variant_id, char *fname)
{
    yaml_parser_t   p;
    yaml_node_t    *root;
    int             r;
    ...
    do
    {
        ...
        if ((root = yaml_document_get_root_node(&document)) != NULL)
        {
            if (parse_cfg_overide_array(cfg, variant_id, &document, root) < 0)
            {
                goto error;
            }
        }
...
error:
    TRACE(ERROR, _b("configuration parsing failed: %s"),strerror(errno));
    r = -errno;
    goto out;
}
...
static int store_vector2(yaml_document_t *doc, float32_t (*value)[2], int n)
{
    int     seq;
    ...
    while (n-- > 0)
    {
        ...
        CHK_ERR((item = store_vector(doc, *value++, 2)) > 0, -1);
        CHK_ERR(yaml_document_append_sequence_item(doc, seq, item) > 0, -1);
    }
...
static int store_cc_cfg(yaml_document_t *doc, cfg_cc_t *cfg)
{
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"size-mm", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->size_mm)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"offset-x", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->offset_x)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"offset-y", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->offset_y)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
static int store_vehicle_cfg(yaml_document_t *doc, cfg_vehicle_t *cfg)
{
    ...
    CHK_ERR((value = store_vector(doc, cfg->steering_coef, 3)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
/**
 * @brief   Store hitch configuration
 *
 * @param   doc[in]     YAML output document
 * @param   cfg[in]     Hitch configuration
 *
 * @return              Positive node ID on success
 */
static int store_hitch_cfg(yaml_document_t *doc, cfg_hitch_t *cfg)
{
    int     map, key, value;
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"slider", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->slider, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"start", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->start, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"stop", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->stop, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
/**
 * @brief   Store TRSC configuration
 *
 * @param   doc[in]     YAML output document
 * @param   cfg[in]     TRSC configuration
 *
 * @return              Positive node ID on success
 */
static int store_trsc_cfg(yaml_document_t *doc, cfg_trsc_t *cfg)
{
    int     map, key, value;
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"pivot", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_vector(doc, cfg->pivot, 2)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"base", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->base, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"dotted", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->dotted, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"trailer", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->trailer, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"cl_vehicle", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->cl_vehicle, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"cs_vehicle", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->cs_vehicle, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"q_vehicle", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->q_vehicle, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"offset", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ivector(doc, cfg->offset, 5)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
static int store_guide_dynamic_fr_cfg(yaml_document_t *doc, cfg_guide_dynamic_fr_t *cfg)
{
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"center-trajectory-width", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->center_trajectory_width)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"center-trajectory-z", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->center_trajectory_z)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
/**
 * @brief   Store THA guidelines configuration
 *
 * @param   doc[in]     YAML output document
 * @param   cfg[in]     Guidelines configuration
 *
 * @return              Positive node ID on success
 */
static int store_guide_tha_cfg(yaml_document_t *doc, cfg_guide_tha_t *cfg)
{
    int     map, key, value;
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"offset", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_vector(doc, cfg->offset, 3)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"angle", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->angle)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"border-width", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->border_width)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"canvas-color", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ui8vector(doc, cfg->canvas_color, 3)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"canvas-transmittance", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_uint(doc, cfg->canvas_transmittance)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"line-color", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_ui8vector(doc, cfg->line_color, 3)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
static int store_guide_cfg(yaml_document_t *doc, cfg_guide_t *cfg)
{
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"tha", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_guide_tha_cfg(doc, &cfg->tha)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
/**
 * @brief   Store STV-PAS configuration
 *
 * @param   doc[in]     YAML output document
 * @param   cfg[in]     Guidelines configuration
 *
 * @return              Positive node ID on success
 */
static int store_stv_pas_cfg(yaml_document_t *doc, cfg_stv_pas_t *cfg)
{
    int     map, key, value;
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"sonar-origin", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_vector2(doc, cfg->sonar_origin, 6)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"sonar-direction", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_vector(doc, cfg->sonar_direction, 6)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"stv-obstacle-dim", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_vector(doc, cfg->stv_obstacle_dim, 3)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"stv-obstacle-dim-scale", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_vector(doc, cfg->stv_obstacle_dim_scale, 3)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"stv-obstacle-switch-factor", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->stv_obstacle_switch_factor)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"stv-obstacle-fadeout-duration", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_uint(doc, cfg->stv_obstacle_fadeout_duration)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
static int store_scv_mesh_cfg(yaml_document_t *doc, cfg_scv_mesh_t *cfg)
{
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"fov-adj-coef", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_vector(doc, cfg->fov_adj_coef, 4)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"yaw-adj-coef", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_float(doc, cfg->yaw_adj_coef)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
static int store_vm_cfg(yaml_document_t *doc, cfg_vm_t *cfg)
{
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"hitch", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_hitch_cfg(doc, &cfg->hitch)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"trsc-svc", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_trsc_cfg(doc, &cfg->trsc_svc)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"trsc-trg", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_trsc_cfg(doc, &cfg->trsc_trg)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
static int store_toplevel_cfg(yaml_document_t *doc, cfg_data_t *cfg)
{
    ...
    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"guidelines", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_guide_cfg(doc, &cfg->guide)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);

    CHK_ERR((key = yaml_document_add_scalar(doc, NULL, (yaml_char_t *)"stv-pas", -1, YAML_PLAIN_SCALAR_STYLE)) > 0, -1);
    CHK_ERR((value = store_stv_pas_cfg(doc, &cfg->stv_pas)) > 0, -1);
    CHK_ERR(yaml_document_append_mapping_pair(doc, map, key, value) > 0, -1);
...
uint32_t cfg_get_body_length_variant(cfg_data_t *cfg)
{
    float wheel_base = cfg->vehicle.wheel_base;

    if (wheel_base < 3622.02)
        return 2;
    else if (wheel_base < 3785.86)
        return 1;
    else
        return 0;
}

void cfg_get_vehModel_pos_rot(uint8_t variantId, uint8_t* posSelect, uint8_t* rotSelect)
{
    switch ((DTCC_VARIANT_IDS_t)variantId)
    {
        case VAR1_BIGHORN_1_140WB_STD_Mirror ...VAR3_BIGHORN_3_153WB_STD_Mirror:
        case VAR6_LARAMIE_2_144WB_STD_Mirror ...VAR9_LARAMIE_6_153WB_TOW_Mirror:
        case VAR12_LARAMIE_1_140WB_STD_Mirror ...VAR18_BIGHORN_6_153WB_TOW_Mirror:
            *posSelect = 1;
            *rotSelect = 0;
            break;
        case VAR4_REBEL_1_144WB_STD_Mirror:
        case VAR5_REBEL_2_144WB_TOW_Mirror:
            *posSelect = 0;
            *rotSelect = 0;
            break;
        case VAR10_TRX_1_144WB_STD_Mirror:
        case VAR11_TRX_2_144WB_TOW_Mirror:
            *posSelect = 2;
            *rotSelect = 1;
            break;
        default:
...
void cfg_get_vehModel_gl(uint8_t variantId, uint8_t* glSelect)
{
    switch ((DTCC_VARIANT_IDS_t)variantId)
    {
        case VAR1_BIGHORN_1_140WB_STD_Mirror ...VAR9_LARAMIE_6_153WB_TOW_Mirror:
            *glSelect = 0;
            break;
        case VAR10_TRX_1_144WB_STD_Mirror:
        case VAR11_TRX_2_144WB_TOW_Mirror:
...
/* Obs zone details:
* Type 0 : Wheelbase (140) Variants (1/12/13/16)
* Type 1 : Wheelbase (144) Non TRX Variants (2/4/5/6/8/14/15/17)
* Type 2 : Wheelbase (144) TRX (10/11)
* Type 3 : Wheelbase (153) Variants (3/7/9/18)
*/
void cfg_get_vehModel_hatchedZone(uint8_t variantId, uint8_t* h_glSelect)
{
    switch ((DTCC_VARIANT_IDS_t)variantId)
    {
        case VAR1_BIGHORN_1_140WB_STD_Mirror:
        case VAR12_LARAMIE_1_140WB_STD_Mirror:
        case VAR13_LARAMIE_4_140WB_STD_Mirror:
        case VAR16_BIGHORN_4_140WB_TOW_Mirror:
            *h_glSelect = 0;
            break;
        case VAR2_BIGHORN_2_144WB_STD_Mirror:
        case VAR4_REBEL_1_144WB_STD_Mirror ...VAR6_LARAMIE_2_144WB_STD_Mirror:
        case VAR8_LARAMIE_5_144WB_TOW_Mirror:
        case VAR14_TUNGSTEN_1_144WB_STD_Mirror:
        case VAR15_TUNGSTEN_2_144WB_TOW_Mirror:
        case VAR17_BIGHORN_5_144WB_TOW_Mirror:
            *h_glSelect = 1;
            break;
        case VAR10_TRX_1_144WB_STD_Mirror:
        case VAR11_TRX_2_144WB_TOW_Mirror:
            *h_glSelect = 2;
            break;
        case VAR3_BIGHORN_3_153WB_STD_Mirror:
        case VAR7_LARAMIE_3_153WB_STD_Mirror:
        case VAR9_LARAMIE_6_153WB_TOW_Mirror:
        case VAR18_BIGHORN_6_153WB_TOW_Mirror:
            *h_glSelect = 3;
            break;
        default:
            *h_glSelect = 0;
...
void cfg_get_vehModel_blind_area(uint8_t variantId, uint8_t* blind_area)
{
    switch ((DTCC_VARIANT_IDS_t)variantId)
    {
        case VAR1_BIGHORN_1_140WB_STD_Mirror:
        ...
        case VAR4_REBEL_1_144WB_STD_Mirror:
        case VAR5_REBEL_2_144WB_TOW_Mirror:
        case VAR6_LARAMIE_2_144WB_STD_Mirror:
        case VAR8_LARAMIE_5_144WB_TOW_Mirror:
        case VAR14_TUNGSTEN_1_144WB_STD_Mirror:
        case VAR15_TUNGSTEN_2_144WB_TOW_Mirror:
        case VAR17_BIGHORN_5_144WB_TOW_Mirror:
...
void config_set_languages_path(const char* path)
{
    languges_path = path;
}

int config_change_language_runtime(uint32_t num, char* file_name)
{
    if (languges_path == NULL)
        return -1;

    FILE * f = fopen(languges_path, "r");

    char buffer[512];

    if (f == NULL)
    {
        TRACE(ERROR, _x("failed to open file"));
        return -1;
    }
    ...
    {
        if (buffer[0] == '#' && (memcmp(buffer, "#VARIANT", 8) == 0))
        {
            char* variant_name = buffer+9;
        ...
        char* p = strstr(buffer, "src : [");
        int32_t a0, a1, a2, a3, a4;

        if (p)
        {
            p += 7;
            sscanf(p, "%d, %d, %d, %d, %d", &a0, &a1, &a2, &a3, &a4);

            if (a4 > LANG_TABLE_SZ)
            {
                TRACE(ERROR, _x("wrong language index"));
                return -1;
            }

            languges_table_src[a4][0] = (float)a0;
            languges_table_src[a4][1] = (float)a1;
            languges_table_src[a4][2] = (float)a2;
            languges_table_src[a4][3] = (float)a3;
        }
        ...
        if (p)
        {
            *p = 0;
            ...
            languges_table_dst[a4][0] = (float)a0;
            languges_table_dst[a4][1] = (float)a1;
            languges_table_dst[a4][2] = (float)a2;
            languges_table_dst[a4][3] = (float)a3;
        }

        p = strstr(buffer, "file : \"");
