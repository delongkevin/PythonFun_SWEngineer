...
#endif
{
    /* DD-ID: {6845E295-F0DF-4e69-BBAF-AD66EE6EE78E}*/
    bool_t v_Ret_b = true;
    rpo_po = i_ROI_po;
...
{
  /* DD-ID: {80B2D22B-9829-482b-AE97-A43DBA6A23A5}*/
    bool_t v_Ret_b = true;
    if( !initOK_b )
    {
...
//                    OC_DEBUG_PRINTF(( "[%s] track starting at (%u, %u) does not have enough features, only got %u features \n",  \
//                        tsc_cfg::MODULE_NAME_FEATUREFILTER, v_Ft_ro.getTrckLst_rx().front_ro().getPt_t().x_x, v_Ft_ro.getTrckLst_rx().front_ro().getPt_t().y_x, v_TrackSz_t) );
*/
                    continue;
                }
...
#endif
//                    OC_DEBUG_PRINTF( ("[%s] track starting at (%u, %u) ending at (%u, %u) does not meet min pixel motion thresh \n",
//                        tsc_cfg::MODULE_NAME_FEATUREFILTER, v_Ft_ro.getTrckLst_rx().front_ro().getPt_t().x_x, v_Ft_ro.getTrckLst_rx().front_ro().getPt_t().y_x,
                ...
                if( !UnwarpFeatureTrack( v_Ft_ro ) )
                {
                    v_Ft_ro.PutIsValid(false);
                    diagnosticData_po->incInvalidFeatures_v();
//                    ME_Debugger_F_Printf_v("[%s] UnwarpFeatureTrack Invalid \n",tsc_cfg::MODULE_NAME_FEATUREFILTER);
                    continue;
                }
                ...
                if( !CheckValidSlope( v_Ft_ro, false ) )
                {
                    v_Ft_ro.PutIsValid(false);
                    diagnosticData_po->incInvalidFeatures_v();
//                    ME_Debugger_F_Printf_v("[%s] Slope Invalid \n",tsc_cfg::MODULE_NAME_FEATUREFILTER);
#ifdef DEBUG_TSC_ALG
...
{
  /* DD-ID: {F0DFDE86-1A19-4662-95F3-507130FBFFC2}*/
    bool_t v_Ret_b = true;
...
{   
  /* DD-ID: {31416D8F-94D6-403b-8AD1-E958DCBE20D8}*/
    ...
    for( ; v_It_t != v_Endlp_t; ++v_It_t )
    {
        ...
        if( !v_Feature_ro.getIsUnWarped_b() )
        {
            ...
            mecl::model::LensRadial<float32_t,5,5> *lens = &dynamic_cast<mecl::LensRadial_t&>(cameraModel_po->getCameraObj_px()->getLens_rx());
            mecl::model::LensRadial<float32_t, 5, 5>::Config_s c_lensCfg_s;
            lens->copyConfig_v(c_lensCfg_s);
            ...
            if(counter % 50 == 0)
            {
                for(int i =0; i < 6; i++)
                {
                  OC_DEBUG_PRINTF(("i2w %.6f, ", c_lensCfg_s.image2world_x[i]));
                }
                OC_DEBUG_PRINTF(("%d \n", counter));
                for(int i =0; i < 6; i++)
                {
                  OC_DEBUG_PRINTF(("w2i %.6f, ", c_lensCfg_s.world2image_x[i]));
                }
                
                OC_DEBUG_PRINTF(("\ngetPpp_rx %.6f, %.6f \n", cameraModel_po->GetSensor().getPpp_rx().getPosX(), cameraModel_po->GetSensor().getPpp_rx().getPosY()));
            }
            #endif 
...
{
  /* DD-ID: {ECCFD8BB-9D5E-4a70-BF47-8779C68177C3}*/
    bool_t v_Ret_b = true;
...
{
  /* DD-ID: {522E1CD4-BB77-4a95-B946-E64718621D85}*/
    sint32_t v_K_s32 = 0;
    T v_C20_x = i_C_x;
    T v_C21_x = i_C_x;
    T v_S20_x = i_S_x;
...
{
  /* DD-ID: {B0E98204-59BC-442b-A220-46BD3255C52D}*/
    T v_RetVal_x = 0.0;
...
{
  /* DD-ID: {D2CC46C4-380C-4f91-A467-2D91431C7521}*/
    T v_Result_x = 0.0;
...
{
  /* DD-ID: {64F11848-26C5-4962-9BA4-110AAFCE6B27}*/
    T v_A_x = 1.;
    T v_Scale_x = 1.;
...
{
  /* DD-ID: {B04C678B-330E-4054-9D4F-F191826ADD71}*/
    bool_t v_Ret_b = true;
    uint8_t v_M_u8 = 3;
    uint8_t v_N_u8 = 2;
...
{
  /* DD-ID: {D9C7E054-8100-4643-9D4D-3B18CCA63DB1}*/
    float64_t invK[3][3];
...
{
  /* DD-ID: {B5DDD498-4B59-44a7-BCE0-4C07902E9395}*/
    bool_t v_Ret_b = true;
...
{
  /* DD-ID: {87AE0B43-84DD-4ffa-9C0D-2ED3CAC13F9F}*/
    ...
    else if( !ComputeEta( &v_EtaY1_x, v_Pair1CamPts_ax, 1, v_KnownRotColumn_ax ) )
    {
        PrintPointPairOnError("eta_x1", i_ImPair1Pt1_ro, i_ImPair1Pt2_ro, i_ImPair2Pt1_ro, i_ImPair2Pt2_ro);
    }
    else if( !ComputeEta( &v_EtaX2_x, v_Pair2CamPts_ax, 0, v_KnownRotColumn_ax ) )
    {
        PrintPointPairOnError("eta_x2", i_ImPair1Pt1_ro, i_ImPair1Pt2_ro, i_ImPair2Pt1_ro, i_ImPair2Pt2_ro);
    }
    else if( !ComputeEta( &v_EtaY2_x, v_Pair2CamPts_ax, 1, v_KnownRotColumn_ax ) )
    {
    ...
    { 
        ...
        else
        {
            ...
            if ( v_ThetaDeviation_x <= static_cast<T>(deviationPercentageIG_u32) )
            {
                ...
                float32_t v_ExtrinsicX_x = 0;
                float32_t v_ExtrinsicY_x = 0;
                float32_t v_ExtrinsicZ_x = 0;
#ifdef USE_SVSCM
                ...
                v_ExtrinsicX_x = cameraModel_po->GetExtrinsics().pos_x.cVal_ax[0];
                v_ExtrinsicY_x = cameraModel_po->GetExtrinsics().pos_x.cVal_ax[1];
                v_ExtrinsicZ_x = cameraModel_po->GetExtrinsics().pos_x.cVal_ax[2];
#endif
                static uint32_t v_count_u32 = 0;
                if (v_count_u32 % 50)
                {
                  OC_DEBUG_PRINTF(("CamPos %.1f, %.1f, %.1f\n", v_ExtrinsicX_x, v_ExtrinsicY_x, v_ExtrinsicZ_x));
                }
                v_count_u32++;
                ...
                {
                    fc::InitialGuess v_Ig_o( v_PitchDeg_x, v_YawDeg_x, v_RollDeg_x );
                    initialGuesses_x.pushBack_v( v_Ig_o );
                    v_Ret_b = true;
                    OC_DEBUG_PRINTF(( "Added initial guess #(%u): { %.2f, %.2f, %.2f, %.2f , %.2f, %.2f, %.2f }",  \
                             initialGuesses_x.size_u32(), v_PitchDeg_x, v_YawDeg_x, v_RollDeg_x, v_ThetaDeviation_x, v_ExtrinsicPitchDeg_x, v_ExtrinsicYawDeg_x, v_ExtrinsicRollDeg_x));
                }
#ifdef DEBUG_TSC_ALG
                else
                {
...
{
  /* DD-ID: {3787C2F8-6EAC-48de-8ECA-8A16B92303D6}*/
    bool_t v_Ret_b = true;
    CREATE_TIMER( timer );
    START_TIMER( timer );
...
{
  /* DD-ID: {C1C8B7CF-2638-46b9-A097-93625498839E}*/
    bool_t v_Ret_b = true;
...
{
    /* DD-ID: {BC3199A4-730C-4c2a-8B87-EE4E0930B3B4}*/
  if( validIndices_x.size_u32() >= 2 )
  {
