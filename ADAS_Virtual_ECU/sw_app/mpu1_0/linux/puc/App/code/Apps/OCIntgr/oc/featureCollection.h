...
namespace fc
{
    ...
    public:
#ifdef USE_SVSCM
        camera_model::CameraModel* getMAddrCmrMdls_po( void )
        {
            return &cameraModels_ao[0];
        }
#else
        camera_model::CameraModelMecl* getMAddrCmrMdls_po( void )
        {
            return &cameraModels_ao[0];
        }
#endif
        km::KinematicModelImpl* getMPKnmtcMdl_po( void ) const
        {
            return pKinematicModel_po;
        }
        // PRQA S 6200 1 //Input can't be const here, the variable to which it is assigned isn't const.
        void putMPKnmtcMdl_v( km::KinematicModelImpl* i_Param_po )
        {
            pKinematicModel_po = i_Param_po;
        }
    ...
    public:
        ...
        uint32_t GetValidFeaturesCount( tscApi::enuCameraID i_CameraID_t )
        {
            return implArray_t[i_CameraID_t].getDiagnosticData_po()->getNumValidFeatures_u32();
        }
        uint32_t GetIgnoredValidFeaturesCount( tscApi::enuCameraID i_CameraID_t )
        {
            return implArray_t[i_CameraID_t].getDiagnosticData_po()->getNumIgnoredValidFeatures_u32();
        }
        uint32_t GetInvalidFeaturesCount( tscApi::enuCameraID i_CameraID_t )
        {
            return implArray_t[i_CameraID_t].getDiagnosticData_po()->getNumInvalidFeatures_u32();
        }
        uint32_t GetUndetectedFeaturesCount( tscApi::enuCameraID i_CameraID_t )
        {
            return implArray_t[i_CameraID_t].getDiagnosticData_po()->getNumUndetectedFeatures_u32();
        }
        uint32_t GetSkippedFramesCount( tscApi::enuCameraID i_CameraID_t )
        {
            return implArray_t[i_CameraID_t].getDiagnosticData_po()->getNumSkippedFrames_u32();
        }
        uint32_t GetProcessedFramesCount( tscApi::enuCameraID i_CameraID_t )
        {
            return implArray_t[i_CameraID_t].getDiagnosticData_po()->getNumProcessedFrames_u32();
        }
...
#ifdef DEBUG_TSC_ALG
        const tscApi::DebugCounters* GetDebugCounters( tscApi::enuCameraID cameraID )
        {
            return m_implArray[cameraID].GetDebugCounters();
        }
#endif
        //#if defined (DEBUG) && defined (TRACING) && defined (APP_CTRL)
        mecl::core::ArrayList< fc::ValidFeature, tsc_cfg::NUM_VALID_INDICES>& GetValidFeatures( tscApi::enuCameraID cameraID )
        {
            return implArray_t[cameraID].getDiagnosticData_po()->getValidFeatures_u32();
        }
        //#endif
