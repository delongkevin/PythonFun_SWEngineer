...
class FeatureCollectionImpl : public control::ModuleImpl<FeatureCollectionInfo,FeatureCollectionConfig_s,FeatureCollectionCameraConfig_s>
{
    ...
    virtual void reset_v(void);
        ...
        fc::ValidFeatureCollection* GetValidFeatureCollection( void )
        {
            return &m_validFeatureCollection;
        }
#endif
        const mecl::core::ArrayList< fc::InitialGuess, tsc_cfg::NUM_INITIAL_GUESSES >& GetInitialGuesses() const
        {
            return initialGuesses_x;
        }
        ...
        uint32_t getFrameCounter_u32( void ) const
        {
            return frameCounter_u32;
        }
        bool_t ValidateSavedData( tscApi::TSCSavedDataInfo_s const* i_TSCSavedData_pt );
        bool_t restoreSavedData_b();
        fc::DiagnosticData* getDiagnosticData_po( void )
        {
            return &diagnosticData_o;
        }
        bool_t LFCValidateROIRect( const tsc_math::ROIRect& i_Rect_rt ) const
        {
            return lfc_x[0].ValidateROIRect( i_Rect_rt );
        }
#ifdef DEBUG_TSC_ALG
        tscApi::DebugCounters* GetDebugCounters( void )
        {
