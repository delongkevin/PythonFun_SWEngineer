...
class FeatureFilter
{
    ...
    bool_t Process( uint32_t i_CurrFrameNum_u32, lfc::LocalFeatureCollector *b_LFC_po );
        const mecl::core::ArrayList< fc::InitialGuess, tsc_cfg::NUM_INITIAL_GUESSES_PER_FRAME >& GetInitialGuesses() const
        {
            return initialGuesses_x;
        }
        const mecl::core::ArrayList< uint32_t, tsc_cfg::NUM_VALID_INDICES >& GetValidIndices() const
        {
            return validIndices_x;
        }
