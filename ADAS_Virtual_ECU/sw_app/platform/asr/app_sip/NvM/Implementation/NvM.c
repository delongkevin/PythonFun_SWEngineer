...
\ID FETA_NVM_IntUpdateCurrentBlockCRCCompareData
 \DESCRIPTION Invalidation of CRC buffer.
 \R The used function to determine the iterations of this loop 'NvM_DataIntegrity_GetIntegrityCodeLength' ensures that
    ...
    The theoretical MAC size can be omitted as it is ensured via configuration validation that MAC and CRCCompare
    mechanism cannot be used in parallel for a given block.
