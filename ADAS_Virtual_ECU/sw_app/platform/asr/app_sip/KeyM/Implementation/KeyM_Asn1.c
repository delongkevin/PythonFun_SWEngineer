  ...
  P2VAR(KeyM_Asn1_ParamType, AUTOMATIC, KEYM_APPL_VAR) asn1Param)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* # Set tag type for octet string and bit string */
  ...
  P2VAR(KeyM_Asn1_ParamType, AUTOMATIC, KEYM_APPL_VAR) asn1Param)
{
  ...
  if (((asn1Param->last_tag) == KEYM_ASN1_TYPE_BIT_STRING) &&
    ((((KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)certId) == KEYM_X509) && (
      ...
      || (((KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)certId) == KEYM_AC)
      && (((KeyM_GetCertAlgorithmFamilyOfCertificate((KeyM_CertificateIterType)certId) == KEYM_ECDSA) && (KeyM_Asn1_CertElementDescr_AC[asn1ParseParam->structureTypeIdx].structureType == KEYM_CERTIFICATE_SIGNATURE)))
      ))
      ))
  {
    asn1Param->last_tag_has_data = FALSE;
  ...
  if (((asn1Param->last_tag) == KEYM_ASN1_TYPE_BIT_STRING) &&
    ((KeyM_GetCertAlgorithmFamilyOfCertificate((KeyM_CertificateIterType)certId) == KEYM_EDDSA) || (KeyM_GetCertAlgorithmFamilyOfCertificate((KeyM_CertificateIterType)certId) == KEYM_RSA)) &&
      ...
      || ((KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)certId) == KEYM_AC)
      && (KeyM_Asn1_CertElementDescr_AC[asn1ParseParam->structureTypeIdx].structureType == KEYM_CERTIFICATE_SIGNATURE))
      ))
  {
    /* Skip leading octet containing the count of added padding bits for constructed bit string tag */
    KeyM_Asn1_SkipLeadingOctet(asn1Param);
...
{
  /* ----- Implementation ----------------------------------------------- */
  /* # Reset iterable flag except for certificate revocation list element */
  if ((KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)certId) != KEYM_CRL)
      && (KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].structureType != KEYM_REVOKED_CERTIFICATES))
  {
    asn1ParseParam->isIterableElementIdx = FALSE;
