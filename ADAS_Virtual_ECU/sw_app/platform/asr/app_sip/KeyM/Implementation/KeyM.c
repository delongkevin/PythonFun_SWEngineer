...
/* Check the version of KeyM header file */
    ...
    || (KEYM_SW_MINOR_VERSION != (5u)) \
    || (KEYM_SW_PATCH_VERSION != (3u)) )
...
# endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    ...
    {
      /* # Request the global lock */
      hasLock = KeyM_GetGlobalLock_CheckTaskState();

# if (KEYM_SERVICE_CERTIFICATE_FUNCTION_ENABLED == STD_ON) /* COV_KEYM_REQUIRED_FOR_SAFE */
      /* # Return E_BUSY only if (lock could not be acquired) AND (no certificate update service is currently in progress) */
      if (!hasLock && KeyM_Cert_UpdateFlag)
      {
        retVal = KEYM_E_BUSY;
      }
      /* # Else, call local implementation of KeyM_CertInfoGet */
      else
# endif
      {
        retVal = KeyM_Cert_CertInfoGet(CertId, CertInfoType, CertInfoData, CertInfoDataLength);

        /* # Free the global lock if it was previously acquired */
        if(hasLock)
        {
          KeyM_FreeGlobalLock();
        }
