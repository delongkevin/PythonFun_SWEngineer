...
#if (KEYM_CERTATTRIBUTEHOLDERCERTREFIDXOFCERTIFICATE == STD_ON)
/**********************************************************************************************************************
 * KeyM_Cert_CopyAttributeCertificateIssuerName()
 *********************************************************************************************************************/
/*! \brief         Copy attribute certificate issuer name data to output buffer.
 *  \details       -
 *  \param[in]     certId                           Holds the identifier of the certificate.
 *  \param[in]     elementIdx                       Holds the certificate element index.
 *  \param[in]     issuerNameOffset                 Holds the issuer name offset.
 *  \param[out]    certStructureData                Certificate structure data returned by the function.
 *  \param[in,out] certStructureDataLength          In: Max number of bytes available in certStructureData.
 *  \return        E_OK                             Element index was retrieved successfully.
 ...
 *  \synchronous   TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(Std_ReturnType, KEYM_CODE) KeyM_Cert_CopyAttributeCertificateIssuerName(
  KeyM_CertificateIdType certId,
  KeyM_SizeOfCertificateElementType elementIdx,
  uint16 issuerNameOffset,
...
#if (KEYM_CERTATTRIBUTEHOLDERCERTREFIDXOFCERTIFICATE == STD_ON)
/**********************************************************************************************************************
 * KeyM_Cert_GetAttributeCertificateIssuerName()
 *********************************************************************************************************************/
/*! \brief         Retrieve distinguished issuer name of Attribute Certificate
 *  \details       -
 *  \param[in]     certId                           Holds the identifier of the certificate.
 ...
 *  \synchronous   TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(Std_ReturnType, KEYM_CODE) KeyM_Cert_GetAttributeCertificateIssuerName(
  KeyM_CertificateIdType certId,
  P2VAR(uint8, AUTOMATIC, KEYM_APPL_VAR) certStructureData,
  P2VAR(uint32, AUTOMATIC, KEYM_APPL_VAR) certStructureDataLength);
#endif
 ...
 *                                                  structure.
 *                 KEYM_E_KEY_CERT_SIZE_MISMATCH    Certificate structure could not be retrieved due to a size
 *                                                  mismatch.
 *  \pre           All provided pointers must be valid.
 *                 CertId must be a valid certificate ID, i.e., CertId < KeyM_GetSizeOfCertificate()
 *  \context       TASK
  ...
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, AUTOMATIC) certId)
{
  ...
  if (*responseData == KEYM_ASN1_OCSP_RESPONSE_STATUS_SUCCESSFUL)
  {
  ...
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, AUTOMATIC) certId)
{
  ...
  retVal = KeyM_Cert_VerifyOCSPResponseData(requestData, ocspResp, responseData, responseDataLength, certId);
  ...
  uint32 responseDataLength)
{
  ...
  if (retVal == E_OK)
  {
    ...
    retVal = KeyM_Cert_VerifyOCSPResponse(requestData, ocspResp, responseData, responseDataLength, &certId);
  }
  ...
  P2VAR(uint32, AUTOMATIC, KEYM_APPL_VAR) outputBufferLen)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* # Determine issuing certificate for used signature algorithm */
  subRetVal = KeyM_Cert_GetIssuerCertId(certId, &issuerCertId);

  /* In case of a dynamic issuer, it is possible that the issuer is optional (KEYM_DYNAMIC_OPTIONAL_ISSUER) and not available.
     It is also possible that the issuer can not be determined, because at this point of parsing the certificate, its issuer
     is not installed yet. If the dynamic group is configured with KEYM_DYNAMIC_OPTIONAL_ISSUER or the order in which
     the certificates are installed is not the issuance order, then the signature format can't be determined. */
  ...
  P2VAR(uint32, AUTOMATIC, KEYM_APPL_VAR) dataLengthPtr)
{
  ...
  if (KeyM_Cert_SearchElementByStructureType(certId, elementStructureType, &elementIdx))
  {
    ...
    && (KeyM_GetOfStructureOfCertificateElement(elementIdx) == KEYM_CERTIFICATE_SIGNATURE))
    {
  ...
  KeyM_CertificateIdType issuerCertId)
{
  /* ----- Local Variables ---------------------------------------------- */
  Std_ReturnType retVal = E_NOT_OK;
  uint8 subjectNameBuffer[KEYM_MAX_CERTIFICATE_MAX_LENGTH] = {0u};
  uint32 subjectNameDataLength = KEYM_MAX_CERTIFICATE_MAX_LENGTH;
  uint8 issuerNameBuffer[KEYM_MAX_CERTIFICATE_MAX_LENGTH] = {0u};
  uint32 issuerNameDataLength = KEYM_MAX_CERTIFICATE_MAX_LENGTH;

  /* ----- Implementation ----------------------------------------------- */
  /* # Get subject distinguished name of certificate in the upper hierarchy */
  if (KeyM_Cert_CertStructureGet(issuerCertId, KEYM_CERTIFICATE_SUBJECT_NAME, subjectNameBuffer, &subjectNameDataLength) == E_OK)
  {
    /* # Get issuer distinguished name of certificate in the lower hierarchy */
    if (KeyM_Cert_CertStructureGet(certId, KEYM_CERTIFICATE_ISSUER_NAME, issuerNameBuffer, &issuerNameDataLength) == E_OK)
    {
      /* # Compare distinguished names */
      if (KeyM_Utils_BufferCmp(subjectNameBuffer, subjectNameDataLength, issuerNameBuffer, issuerNameDataLength))
      {
        retVal = E_OK;
      }
    }
  ...
  KeyM_CertificateIdType certId)
{
  ...
  if (retVal == E_OK)
  {
    /* # Compare issuer distinguished name with subject distinguished name of certificate in upper hierarchy */
  ...
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, KEYM_APPL_VAR) issuerCertId)
{
  ...
  else
  {
    ...
    if (retVal == E_OK)
    {
      ...
      retVal = KeyM_Cert_GetDynamicIssuerCertIdByDN(KeyM_GetCertificateGroupIdxOfCertificate((KeyM_CertificateIterType)certId), certId, &issuerCertIdLocal);
  ...
  P2VAR(uint32, AUTOMATIC, KEYM_APPL_VAR) certStructureDataLength)
{
  ...
  if (issuerNameOffset < KeyM_GetElementStorageRef((KeyM_ElementStorageRefIterType)(elementIdx)).length)
  {
    ...
    elementLength = KeyM_GetElementStorageRef((KeyM_ElementStorageRefIterType)(elementIdx)).length  - issuerNameOffset;

    /* # Check if output buffer is sufficiently large */
    if (*certStructureDataLength >= elementLength)
    {
      /* # Copy issuer name data and datalength to output parameters */
      VStdLib_MemCpyLarge(certStructureData, &KeyM_Cert_GetCertElementDataPointer(certId, elementIdx)[issuerNameOffset], (uint32)elementLength);  /* PRQA S 0315 */ /* MD_MSR_VStdLibCopy */ /* VCA_KEYM_CERT_GETELEMENTBYSTRUCTURETYPE_MEMCPY */
      *certStructureDataLength = (uint32)elementLength;
      retVal = E_OK;
    }
    else
    {
  ...
  P2VAR(uint32, AUTOMATIC, KEYM_APPL_VAR) certStructureDataLength)
{
  ...
  if (KeyM_Cert_SearchElementByStructureType(certId, KEYM_CERTIFICATE_ISSUER_NAME, &elementIdx))
  {
    ...
    if (retVal == E_OK)
    {
      /* # Copy issuer name to output buffer */
      retVal = KeyM_Cert_CopyAttributeCertificateIssuerName(certId, elementIdx, issuerNameOffset, certStructureData, certStructureDataLength);
    }
  }
...
# if (KEYM_CERTATTRIBUTEHOLDERCERTREFIDXOFCERTIFICATE == STD_ON)
  ...
  if (KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)CertId) == KEYM_AC)
  {
    ...
    {
      retVal = KeyM_Cert_GetAttributeCertificateIssuerName(CertId, CertStructureData, CertStructureDataLength);
    }
    else
    {
      retVal = KEYM_E_PARAMETER_MISMATCH;
...
# endif
  {
    ...
    retVal = KeyM_Asn1_GetCertStructureByStructurePath(CertId, CertStructure, &certData, &certStructureOffset, &certStructureLength);

    if (retVal == E_OK)
    {
      if (*CertStructureDataLength < certStructureLength)
      {
        retVal = KEYM_E_KEY_CERT_SIZE_MISMATCH;
      }
      else
      {
        /* # Copy structure data to output buffer */
        VStdLib_MemCpyLarge(CertStructureData, &certData.certData[certStructureOffset], certStructureLength);  /* PRQA S 0315 */ /* MD_MSR_VStdLibCopy */ /* VCA_KEYM_CERT_CERTSTRUCTUREGET_MEMCPY */
        *CertStructureDataLength = certStructureLength;
      }
    }
  }
