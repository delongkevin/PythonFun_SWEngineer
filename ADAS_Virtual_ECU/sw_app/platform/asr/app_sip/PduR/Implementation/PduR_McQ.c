 ...
 *
 *********************************************************************************************************************/
PDUR_LOCAL_INLINE FUNC(void, PDUR_CODE) PduR_McQ_ReadFlush(PduR_McQBufferRomIdxOfSrcApplicationRomType mcQBufferRomIdx, PduR_MemIdxType memIdx) /* COV_PDUR_ROBUSTNESS_FLUSH_INVALID_QUEUE */
{
  PduR_McQBufferArrayRamWriteIdxOfMcQBufferRamType writeIdx = PduR_McQ_GetMcQBufferArrayRamWriteIdxSafe(mcQBufferRomIdx, memIdx);

  PduR_SetMcQBufferArrayRamReadIdxOfMcQBufferRam(mcQBufferRomIdx, writeIdx, memIdx);      /* SBSW_PDUR_IDX_PARAM_INTERNAL_FUNCTION */
