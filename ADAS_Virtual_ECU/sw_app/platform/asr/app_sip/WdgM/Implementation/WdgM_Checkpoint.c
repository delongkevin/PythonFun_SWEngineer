 ...
 *
 */
WDGM_LOCAL FUNC(void, WDGM_CODE) ProgramFlowAndDeadlineMonitoring( /* PRQA S 6010, 6030, 6060, 6080 */ /* MD_MSR_STPTH, MD_MSR_STCYC, MD_MSR_STPAR, MD_MSR_STMIF */
  P2CONST(WdgM_SupervisedEntityType, AUTOMATIC, WDGM_APPL_CONST) current_entity_ptr,
  P2CONST(WdgM_CheckPointType, AUTOMATIC, WDGM_APPL_CONST) current_checkpoint_ptr,
  VAR(WdgM_CheckpointIdType, AUTOMATIC) CPID,
  VAR(WdgM_TimeBaseTickType, AUTOMATIC) current_tick_count,
  ...
  )
{
  ...
  else
  {
    ...
    if (current_checkpoint_ptr->NrOfGlobalTransitions > 0u)
    {
      ...
      while (nr_of_global_transitions > 0u) /* FETA_WDGM_FLOW_MONITORING_GLOBAL_TRANSITIONS */
      {
  ...
  )
{
  ...
  while (transition_id > 0u) /* FETA_WDGM_FIND_TRANSITION_ID */
  {
 ...
 *
 */
WDGM_LOCAL FUNC(WdgM_SharedBooleanType, WDGM_CODE) WdgM_ReadAndClearGlobalTransitionFlags( /* PRQA S 6010, 6030, 6060, 6080 */ /* MD_MSR_STPTH, MD_MSR_STCYC, MD_MSR_STPAR, MD_MSR_STMIF */
  P2CONST(WdgM_SupervisedEntityType, AUTOMATIC, WDGM_APPL_CONST) current_entity_ptr,
  P2CONST(WdgM_SupervisedEntityType, AUTOMATIC, WDGM_APPL_CONST) previous_entity_ptr,
  VAR(WdgM_CheckpointIdType, AUTOMATIC) g_tr_source_cp_id,
  VAR(WdgM_TransitionIdType, AUTOMATIC) current_g_tr_flag_id,
  ...
  )
{
  ...
  if (previous_remembered_cpid != WDGM_INVALID_CP_ID)
  {
       ...
       )
    {
      ...
      while (prev_cp_started_g_tr_id > 0u) /* FETA_WDGM_READ_AND_CLEAR_STARTED_GLOBAL_TRANSITION_FLAGS */
      {
  ...
  )
{
  ...
  while (nr_of_global_transitions > 0u) /* FETA_WDGM_CLEAR_INCOMING_TRANSITIONS */
  {
...
FUNC(Std_ReturnType, WDGM_CODE) WdgM_CheckpointReached( /* PRQA S 6010, 6030, 6050, 6080 */ /* MD_MSR_STPTH, MD_MSR_STCYC, MD_MSR_STCAL, MD_MSR_STMIF */
...
#else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        for (g_tr = 0u; g_tr < current_checkpoint_ptr->NrOfStartedGlobalTransitions; g_tr++) /* FETA_WDGM_WRITE_TRANSITION_FLAG_STARTED_GLOBAL_TRANSITIONS */
        {
