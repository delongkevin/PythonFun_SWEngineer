  ...
  )
{
  ...
  for (cp_id = 0u; cp_id < current_entity_ptr->NrOfCheckpoints; cp_id++) /* FETA_WDGM_INIT_ALIVE_COUNTERS */
  {
  ...
  )
{
  ...
  /* Initialize the all Alive counters */
  for (cp_id = 0u; cp_id < current_entity_ptr->NrOfCheckpoints; cp_id++) /* FETA_WDGM_INIT_ALIVE_COUNTERS_CHECKPOINTS */
  {
  ...
  )
{
  ...
  for (entity_id = 0u; entity_id < nr_of_entities; entity_id++) /* FETA_WDGM_CHECKSUM_OVER_ALL_ENTITIES */ 
  {
...
#endif /* (WDGM_MULTICORE_USAGE == STD_ON) */
    {
      ...
      for (cp_id = 0u; cp_id < current_entity_ptr->NrOfCheckpoints; cp_id++) /* FETA_WDGM_ADD_CHECKPOINTS */
      {
  ...
  )
{
  ...
  /* WDGM002 */
  for (i = 0u; i < wdgm_cfg_ptr->NrOfWatchdogDevices; i++)  /* FETA_WDGM_TRIGGER_WATCHDOG_DEVICES */
  {
  ...
  )
{
  ...
  /* #20 Loop over all configured watchdog devices */
  for (i = 0u; i < wdgm_cfg_ptr->NrOfWatchdogDevices; i++) /* FETA_WDGM_RESET_WATCHDOG_DEVICES */
  {
    device_ptr = &wdgm_cfg_ptr->WdgMWatchdogDeviceRef[i];
#if (WDGM_OFF_MODE_ENABLED == STD_ON)
    if (device_ptr->WdgMTriggerModeRef[trigger_mode].WdgMWatchdogMode != WDGIF_OFF_MODE)
 ...
 *
 */
WDGM_LOCAL FUNC(void, WDGM_CODE) BuildEntitiesStatus( /* PRQA S 6010, 6030, 6050, 6080 */ /* MD_MSR_STPTH, MD_MSR_STCYC, MD_MSR_STCAL, MD_MSR_STMIF */
  P2VAR(WdgM_LocalStatusType, AUTOMATIC, WDGM_APPL_DATA) local_status_failed_ptr,
  P2VAR(WdgM_LocalStatusType, AUTOMATIC, WDGM_APPL_DATA) local_status_expired_ptr,
  P2CONST(WdgM_ConfigType, AUTOMATIC, WDGM_APPL_CONST) wdgm_cfg_ptr,
  P2VAR(WdgM_SupervisedEntityIdType, AUTOMATIC, WDGM_APPL_DATA) expired_seid_ptr
  ...
  )
{
  ...
  *expired_seid_ptr = WDGM_INVALID_SE_ID; /* SBSW_WDGM_008 */
  ...
  for (entity_id = 0u; entity_id < wdgm_cfg_ptr->NrOfSupervisedEntities; entity_id++) /* FETA_WDGM_NEW_LOCAL_SUPERVISION_STATUS_ENTITIES */ 
  {
    current_entity_ptr = &wdgm_cfg_ptr->WdgMSupervisedEntityRef[entity_id];
 ...
 *
 */
WDGM_LOCAL FUNC(WdgM_LocalStatusType, WDGM_CODE) DeactivateEntity( /* PRQA S 6080 */ /* MD_MSR_STMIF */
  P2CONST(WdgM_SupervisedEntityType, AUTOMATIC, WDGM_APPL_CONST) current_entity_ptr,
  VAR(WdgM_LocalStatusType, AUTOMATIC) local_status,
  VAR(WdgM_SupervisedEntityIdType, AUTOMATIC) SEID,
  P2CONST(WdgM_ConfigType, AUTOMATIC, WDGM_APPL_CONST) wdgm_cfg_ptr
     ...
     )
  {
    ...
    if (target_entity_state == WDGM_SE_STATE_REQUEST_DEACTIVATE)
    {
      ...
      if (current_entity_ptr->EntityStatusLRef->LocalActivityFlag == FALSE)
      {
        ...
        current_entity_ptr->EntityStatusGRef->FailedSupervisionRefCycles = 0u; /* SBSW_WDGM_008 */
        for (cp_id = 0u; cp_id < current_entity_ptr->NrOfCheckpoints; cp_id++) /* FETA_WDGM_RESET_ALIVE_SUPERVISION_VALUES_CHECKPOINTS  */
        {
 ...
 *
 */
WDGM_LOCAL FUNC(WdgM_LocalStatusType, WDGM_CODE) BuildAliveLocalStatus( /* PRQA S 6080 */ /* MD_MSR_STMIF */
  P2CONST(WdgM_SupervisedEntityType, AUTOMATIC, WDGM_APPL_CONST) current_entity_ptr
#if (WDGM_FIRSTCYCLE_ALIVECOUNTER_RESET == STD_ON)
  , P2CONST(WdgM_ConfigType, AUTOMATIC, WDGM_APPL_CONST) wdgm_cfg_ptr
#endif /* (WDGM_FIRSTCYCLE_ALIVECOUNTER_RESET == STD_ON) */
    )
{
  ...
  for (cp_id = 0u; cp_id < current_entity_ptr->NrOfCheckpoints; cp_id++) /* FETA_WDGM_BUILD_SUPERVISION_STATUS_CHECKPOINTS  */
  {
    /* Pointer to CP */
    current_checkpoint_ptr = &current_entity_ptr->WdgMCheckpointRef[cp_id];
 ...
 *
 */
WDGM_LOCAL FUNC(void, WDGM_CODE) CheckDeadlineHangup( /* PRQA S 6060, 6080 */ /* MD_MSR_STPAR, MD_MSR_STMIF */
  P2CONST(WdgM_SupervisedEntityType, AUTOMATIC, WDGM_APPL_CONST) current_entity_ptr,
  P2VAR(WdgM_LocalStatusType, AUTOMATIC, WDGM_APPL_DATA) aperiodic_local_status_deadline,
  VAR(WdgM_CheckpointIdType, AUTOMATIC) remembered_checkpoint_id,
  VAR(WdgM_BooleanParamType, AUTOMATIC) local_activity_flag,
  ...
  if (remembered_checkpoint_id != WDGM_INVALID_CP_ID)
  {
       ...
       )
    {
      ...
      if (outgoing_deadline_max != 0u)
      {
        ...
        for (g_tr = 0u; g_tr < remembered_checkpoint_ptr->NrOfStartedGlobalTransitions; g_tr++) /* FETA_WDGM_CHECK_STARTED_GLOBAL_TRANSITION_FLAG */
        {
 ...
 *
 */
FUNC(void, WDGM_CODE) WdgM_Init(P2CONST(WdgM_ConfigType, AUTOMATIC, WDGM_APPL_CONST) WdgMConfigPtr) /* PRQA S 2889, 6010, 6030, 6050 */ /* MD_WfgM_2889, MD_MSR_STPTH, MD_MSR_STCYC, MD_MSR_STCAL */
{
  /* ----- Local Variables ---------------------------------------------- */
  ...
  for (wdg_dev = 0u; wdg_dev < WdgMConfigPtr->NrOfWatchdogDevices; wdg_dev++) /* FETA_WDGM_SET_INIT_MODE_FOR_ALL_DEVICES */
  {
    ...
    for (trg_mode = 0u; trg_mode < WdgMConfigPtr->NrOfTriggerModes; trg_mode++) /* FETA_WDGM_ERRONEOUS_OFF_MODE_TRIGGER_MODES */
    {
...
# endif /* (WDGM_MULTICORE_USAGE == STD_ON) */
  ...
  for (entity_id = 0u; entity_id < nr_of_entities; entity_id++) /* FETA_WDGM_INIT_SUPERVISED_ENTITIES */
  {
...
# endif /* (WDGM_TICK_OVERRUN_CORRECTION == STD_ON) */
  ...
  for (g_tr = 0u; g_tr < WdgMConfigPtr->NrOfGlobalTransitions; g_tr++)   /* FETA_WDGM_INIT_GLOBAL_TANSITIONS */
  {
# if (WDGM_MULTICORE_USAGE == STD_ON)
 ...
 *
 */
FUNC(Std_ReturnType, WDGM_CODE) WdgM_SetMode(VAR(WdgM_ModeType, AUTOMATIC) Mode, VAR(uint16, AUTOMATIC) CallerID) /* PRQA S 6010, 6080 */ /* MD_MSR_STPTH, MD_MSR_STMIF */
{
  /* ----- Local Variables ---------------------------------------------- */
  VAR(Std_ReturnType, AUTOMATIC) res = E_NOT_OK;
...
#else
  {
    ...
    else
    {
      ...
      /* WDGM245 */
      for (i = 0u; i < wdgm_cfg_ptr->NrOfAllowedCallers; i++) /* FETA_WDGM_CHECK_IF_CALLER_IS_ALLOWED */
      {
 ...
 *
 */
FUNC(void, WDGM_CODE) WdgM_MainFunction(void) /* PRQA S 2889, 6010, 6030, 6050 */ /* MD_WfgM_2889, MD_MSR_STPTH, MD_MSR_STCYC, MD_MSR_STCAL */
{
  /* ----- Local Variables ---------------------------------------------- */
...
# endif
  ...
  if (trg_mode != wdgm_cfg_ptr->DataGRef->PreviousTriggerModeId)
  {
    ...
    for (wdg_dev = 0u; wdg_dev < wdgm_cfg_ptr->NrOfWatchdogDevices; wdg_dev++) /* FETA_WDGM_SET_NEW_MODE_DEVICE */
    {
...
/* FETA_WDGM_JUSTIFICATION_BEGIN

  \ID FETA_WDGM_INIT_ALIVE_COUNTERS_CHECKPOINTS                                                                                           
  \DESCRIPTION Initialize the all Alive counters
  \COUNTERMEASURE \S SMI-494148
                  \T 'Test 1' in the Verifier confirms of NrOfCheckpoints checkpoints for this supervised entity.
                  \T 'Test 2' in the Verifier confirms that WDGM_NR_OF_CHECKPOINTS is valid

  \ID FETA_WDGM_CHECKSUM_OVER_ALL_ENTITIES                                                                                     
  \DESCRIPTION through all Entities and Checkpoints and build a checksum from Entity array and from Checkpoint array.
  \COUNTERMEASURE \S SMI-494148, SMI-506

  \ID FETA_WDGM_ADD_CHECKPOINTS                                                                                                
  \DESCRIPTION Addup checkpoint ids.
  \COUNTERMEASURE \S SMI-494148
                  \T 'Test 1' in the Verifier confirms of NrOfCheckpoints checkpoints for this supervised entity.
                  \T 'Test 2' in the Verifier confirms that WDGM_NR_OF_CHECKPOINTS is v

  \ID FETA_WDGM_TRIGGER_WATCHDOG_DEVICES                                                                                        
  \DESCRIPTION Trigger all Watchdog devides.
  \COUNTERMEASURE \S SMI-494148, SMI-508

  \ID FETA_WDGM_RESET_WATCHDOG_DEVICES 
  \DESCRIPTION Resets the Watchdog devices. 
  \COUNTERMEASURE \S SMI-494148, SMI-508

  \ID FETA_WDGM_NEW_LOCAL_SUPERVISION_STATUS_ENTITIES
  \DESCRIPTION Build a new local supervision status for all supervised entities running on the current core. 
  \COUNTERMEASURE \S SMI-494148, SMI-506

  \ID FETA_WDGM_RESET_ALIVE_SUPERVISION_VALUES_CHECKPOINTS  
  \DESCRIPTION Reset all alive supervision values.
  \COUNTERMEASURE \S SMI-494148
                  \T 'Test 1' in the Verifier confirms of NrOfCheckpoints checkpoints for this supervised entity.
                  \T 'Test 2' in the Verifier confirms that WDGM_NR_OF_CHECKPOINTS is valid

  \ID FETA_WDGM_BUILD_SUPERVISION_STATUS_CHECKPOINTS 
  \DESCRIPTION Builds the local supervision status of a supervised entity for alive supervision. 
  \COUNTERMEASURE \S SMI-494148
                  \T 'Test 1' in the Verifier confirms of NrOfCheckpoints checkpoints for this supervised entity.
                  \T 'Test 2' in the Verifier confirms that WDGM_NR_OF_CHECKPOINTS is valid

  \ID FETA_WDGM_CHECK_STARTED_GLOBAL_TRANSITION_FLAG
  \DESCRIPTION Check global transition flag for all outgoing global transitions out of a checkpoint. 
  \COUNTERMEASURE \S SMI-494148, SMI-506
                  \T 'Test 17' A checkpoint's NrOfStartedGlobalTransitions value shall match the number of global 
                    transitions having that checkpoint as starting point according to the EDF.

  \ID FETA_WDGM_SET_INIT_MODE_FOR_ALL_DEVICES
  \DESCRIPTION Set the initial mode for all devices. 
  \COUNTERMEASURE \S SMI-494148, SMI-508

  \ID FETA_WDGM_ERRONEOUS_OFF_MODE_TRIGGER_MODES
  \DESCRIPTION Check if a erroneous WDGIF_OFF_MODE is configured.
  \COUNTERMEASURE \S SMI-494148, SMI-494148

  \ID FETA_WDGM_INIT_SUPERVISED_ENTITIES
  \DESCRIPTION Initialize all supervised entities.
  \COUNTERMEASURE \S SMI-494148, SMI-506

  \ID FETA_WDGM_INIT_GLOBAL_TANSITIONS 
  \DESCRIPTION Initializes global transitions running on the current core.
  \COUNTERMEASURE \S SMI-494148, SMI-506

  \ID FETA_WDGM_CHECK_IF_CALLER_IS_ALLOWED
  \DESCRIPTION Checks if caller is in the list of allowed callers. 
  \COUNTERMEASURE \S SMI-494148, SMI-506

  \ID FETA_WDGM_SET_NEW_MODE_DEVICE
  \DESCRIPTION Set the initial mode for all devices. 
  \COUNTERMEASURE \S SMI-494148, SMI-508

  \ID FETA_WDGM_FLOW_MONITORING_GLOBAL_TRANSITIONS
  \DESCRIPTION Checks logical and deadline supervision for a certain supervised entity.
  \COUNTERMEASURE \S SMI-494148, SMI-506

  \ID FETA_WDGM_FIND_TRANSITION_ID 
  \DESCRIPTION Parses all the defined local incoming transitions and finds the actual transition. 
  \COUNTERMEASURE \S SMI-494148
                  \T 'Test 27' Checkpoint's NrOfLocalTransitions value shall be set to the number of local transitions 
                     having that checkpoint as destination point according to the EDF.

  \ID FETA_WDGM_READ_AND_CLEAR_STARTED_GLOBAL_TRANSITION_FLAGS 
  \DESCRIPTION Read and clear global transition flags. 
  \COUNTERMEASURE \S SMI-494148, SMI-506
                  \T 'Test 17' A checkpoint's NrOfStartedGlobalTransitions value shall match the number of global 
                    transitions having that checkpoint as starting point according to the EDF.

  \ID FETA_WDGM_CLEAR_INCOMING_TRANSITIONS 
  \DESCRIPTION Clear global transition flags for this global incoming transition
  \COUNTERMEASURE \S SMI-494148, SMI-506

  \ID FETA_WDGM_WRITE_TRANSITION_FLAG_STARTED_GLOBAL_TRANSITIONS
  \DESCRIPTION Write the global transition flag for all global outgoing transitions (set true). 
  \COUNTERMEASURE \S SMI-494148, SMI-506
