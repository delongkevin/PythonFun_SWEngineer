 ...
 *
 */
FUNC(MemIf_StatusType, MEMIF_CODE) MemIf_GetStatus(uint8 DeviceIndex) /* PRQA S 6080 */ /* MD_MSR_STMIF */
{
  /* ----- Local Variables ------------------------------------------------------------------------------*/
  uint8 errorId;
...
#endif
  {
    ...
    if (MEMIF_BROADCAST_ID == DeviceIndex)
    {
      ...
      for (MemIf_LoopCounter = 0; MemIf_LoopCounter < MemIf_NumberOfDevices; MemIf_LoopCounter++) /* FETA_MEMIF_GET_STATUS */
      {
...
FUNC(void, MEMIF_CODE) MemIf_SetMode(MemIf_ModeType Mode)
{
  ...
  for(MemIf_LoopCounter = 0; MemIf_LoopCounter < MemIf_NumberOfDevices; MemIf_LoopCounter++) /* FETA_MEMIF_SET_MODE */
  {
...
/* FETA_JUSTIFICATION_BEGIN
  ...
  \ID FETA_MEMIF_GET_STATUS
  \DESCRIPTION MemIf fetches all statuses of the memory devices.
  \COUNTERMEASURE \N Loop terminates when the monotonic incremented counter reaches the upper limit which is a constant. 

  \ID FETA_MEMIF_SET_MODE
  \DESCRIPTION Set mode of all devices.
  \COUNTERMEASURE \N Loop terminates when the monotonic incremented counter reaches the upper limit which is a constant. 
