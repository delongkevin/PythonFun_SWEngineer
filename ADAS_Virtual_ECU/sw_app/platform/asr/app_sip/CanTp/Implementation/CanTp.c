...
# endif
  ...
  if ( (lReceivedDataLength >= lMinPayloadLength) &&                                                                                                       /* min SF DL check   */
       ...
       (CanTp_IsCanDlValid(pPdu->SduLength, CanTp_Util_GetMinFittingCanDl(lPayloadOffset + lReceivedDataLength, CanTp_RxCfg_HasPaddingActivation(pTpRxState->RxHdl)))) ) /* CAN_DL check */
  {
...
# endif
  ...
  if (pTpRxState->ChannelState == CANTP_RXSTATE_WAIT_CF)
  {
    ...
    else
    {
      ...
      if (CanTp_IsCanDlValid(pPdu->SduLength, CanTp_Util_GetMinFittingCanDl((pTpRxState->PayloadLength + lPayloadOffset), CanTp_RxCfg_HasPaddingActivation(pTpRxState->RxHdl)))) /* max CAN_DL check */
      {
        /* DLC check passed */
        lErrorId = CANTP_E_NO_ERROR;
      }
