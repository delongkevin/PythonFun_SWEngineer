...
#ifndef  OS_HAL_COVERAGE_RCARX3INT_H
# define OS_HAL_COVERAGE_RCARX3INT_H
...
\ID COV_OS_HALPLATFORM_UNHANDLEDINT_ON_NESTED_INTC
  \ACCEPT TX
  \REASON Only the nIRQ interrupt on the core local interrupt controller is used to forward all the interrupts from
          the GIC-RT. All other sources must not be used in a nested interrupt controller configuration.
          These interrupts cause an unhandled interrupt and are checked by review.
...
\ID COV_OS_HALPLATFORMPFRTUNSUPPORTED_XX
  \ACCEPT XX
  \REASON Timer type PFRT is not available on this derivative but it is tested on reference derivative ZUxxx.
...
\ID COV_OS_HALPLATFORMTPUNSUPPORTED_XX
  ...
  \REASON This derivative does not meet the hardware requirements to support timing protection.
          Functionality has been tested on reference derivative ZUxxx.
...
\ID COV_OS_HALPLATFORMSPURIOUSINTERRUPT
  \ACCEPT XF
  \REASON This code is reached only in special cases of race condition between different cores, attempting to serve the same
          interrupt service request. More in details if both requests are accepted by the target cores, only the first
          one that reads the ICCIAR value will get a valid interrupt id, while the second the spurious ID 1023.
...
#endif /* OS_HAL_COVERAGE_RCARX3INT_H */

/*!
 * \}
 */
/***********************************************************************************************************************
 *  END OF FILE: Os_Hal_Coverage_RCARx3xxInt.h
 **********************************************************************************************************************/
