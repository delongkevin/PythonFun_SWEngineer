 ...
 * \{
 *
 * \file       Os_Hal_MemoryProtection_PMSAv7.h
...
#ifndef  OS_HAL_MEMORYPROTECTION_PMSAV7_H
 ...
 *  \pre          -
 **********************************************************************************************************************/
extern FUNC(void, OS_CODE) Os_Hal_FlushDataCache(void);


/***********************************************************************************************************************
 *  Os_Hal_FlushInstructionCache()
 **********************************************************************************************************************/
/*! \brief        Invalidates instruction cache and handles branch prediction maintenance.
 *  \details      -
 *  \context      ANY
 *  \reentrant    FALSE
 *  \synchronous  TRUE
 *  \pre          -
 **********************************************************************************************************************/
extern FUNC(void, OS_CODE) Os_Hal_FlushInstructionCache(void);
...
/***********************************************************************************************************************
 *  Os_Hal_DisableAndReturnCacheState()
 **********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE Os_Hal_IntGlobalStateType, OS_CODE,
OS_ALWAYS_INLINE, Os_Hal_DisableAndReturnCacheState, (void))
{
  /* #10 Get SCTLR. */
  uint32 systemControl = Os_Hal_GetSystemControl();

  /* #20 Get only cache and branch prediction related bits. */
  Os_Hal_IntGlobalStateType cacheSettings = (Os_Hal_IntGlobalStateType)(systemControl & OS_HAL_MP_SYSTEMCONTROL_CACHEING_MASK);

  /* #30 Flush instruction cache and do branch prediction maintenance. */
  Os_Hal_FlushInstructionCache();

  /* #40 Synchronization barriers. */
  ...
  /* #60 Synchronization barriers. */
  Os_Hal_InstructionSyncBarrier();
  Os_Hal_DataSyncBarrier();

  /* #70 Turn off caches and branch prediction. */
  systemControl &= ~(OS_HAL_MP_SYSTEMCONTROL_CACHEING_MASK);
...
OS_ALWAYS_INLINE, Os_Hal_RestoreCacheState,
(
...
))
{
  ...
  /* #20 Restore passed cache and branch prediction settings. */
  systemControl |= state;
  Os_Hal_SetSystemControl(systemControl);
...
))
{
  uint32 index;
  Os_Hal_IntGlobalStateType state;
  Os_Hal_IntGlobalStateType cacheState;

  /* #10 Disable interrupts and save old interrupt state. */
  state = Os_Hal_DisableAndReturnPriorState();

  /* #20 Flush and disable caches and branch prediction. */
  cacheState = Os_Hal_DisableAndReturnCacheState();
  ...
  /* #90 Restore old cache and branch prediction state. */
  Os_Hal_RestoreCacheState(cacheState);
...
#endif /* OS_HAL_MEMORYPROTECTION_PMSAV7_H */

/*!
 * \}
 */
/**********************************************************************************************************************
 *  END OF FILE: Os_Hal_MemoryProtection_PMSAv7.h
 *********************************************************************************************************************/
