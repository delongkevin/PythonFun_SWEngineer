 ...
 *  \{
 *
 *  \file  Os_Hal_Context_v7a.h
 *  \brief  Context switching primitives for Cortex-A.
...
#ifndef  OS_HAL_CONTEXT_V7A_H
# define OS_HAL_CONTEXT_V7A_H
...
#  include "Os_Hal_Context_v7a_FpuOn.h"
# else
#  include "Os_Hal_Context_FpuOff.h"
# endif

# if (OS_CFG_MEMORY_PROTECTION == STD_ON)
#  include "Os_Hal_Context_v7a_MpuOn.h"
# else
...
))
{
  ...
  /* #40 Restore stack region. */                   
  Os_Hal_MpSetStackRegion((Next)->StackRegionStart, (Next)->StackRegionInfo);   
}
...
))
{
  ...
  Context->Psr = ((Config->Psr & (~OS_HAL_CORE_MSR_USER_BIT_MSK)) |
                  (Context->Psr & OS_HAL_CORE_MSR_USER_BIT_MSK));                                                       /* SBSW_OS_HAL_PWA_CALLER */
...
))
{
  ...
  Context->Parameter = (uint32)Parameter;                                                                               /* PRQA S 0306 */ /* MD_Os_Hal_Rule11.4_0306 */ /* SBSW_OS_HAL_PWA_CALLER */
}
...
))
{
  ...
  Context->StackRegionInfo = Stack->StackRegionInfo;                                                                    /* SBSW_OS_HAL_PWA_CALLER */
}
...
))
{
  ...
  Stack->StackRegionInfo  = Context->StackRegionInfo;                                                                   /* SBSW_OS_HAL_PWA_CALLER */
}
...
))
{
  ...
  /* #20 Set new PC, LR and set CPSR = SPSR. This results in a switch to next. */
  Os_Hal_ContextGo(Next->Pc, Next->Lr);
}
...
))
{
  ...
  /* #30 Restore permanent registers, set new PC, LR and set CPSR = SPSR. This results in a switch to next. */
  Os_Hal_ContextRestorePermanentAndGo(Next->Pc, Next->Lr, (uint32)(Next->PermanentRegisters));                          /* PRQA S 0306 */ /* MD_Os_Hal_Rule11.4_0306 */
...
))
{
  ...
  /* #20 Restore permanent registers, set new PC, LR and set CPSR = SPSR. This results in a switch to next. */
  Os_Hal_ContextRestorePermanentAndGo(Next->Pc, Next->Lr, (uint32)(Next->PermanentRegisters));                          /* PRQA S 0306 */ /* MD_Os_Hal_Rule11.4_0306 */
  /* Function never returns. */
}
...
/***********************************************************************************************************************
 *  END OF FILE: Os_Hal_Context_v7a.h
 **********************************************************************************************************************/
