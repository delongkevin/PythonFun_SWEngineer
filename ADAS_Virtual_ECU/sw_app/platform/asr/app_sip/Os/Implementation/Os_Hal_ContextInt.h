...
#  include "Os_Hal_Context_v7aInt.h"
# elif defined(OS_HAL_ARCH_V7R) || defined(OS_HAL_ARCH_V8R)
#  include "Os_Hal_Context_vXrInt.h"
# else
 ...
 *********************************************************************************************************************/
/*! \brief        Store current PC, set r0, PC, LR and switch mode + stack.
 *  \details      This is an assembler function.
 *  \param[in]    Parameter    Future value of r0.
 *  \param[in]    NextPc       Future value of PC.
 *  \param[in]    NextLr       Future value of LR.
 *  \param[in]    CurrentPcPtr Address to store the current PC. Parameter must not be NULL.
 *  \context      OS_INTERNAL
 *  \reentrant    TRUE
 *  \synchronous  TRUE
 ...
 *********************************************************************************************************************/
/*! \brief        Store current PC, set r0, PC, LR and switch MPU + mode + stack.
 *  \details      This is an assembler function.
 *  \param[in]    StackStart   MPU stack region start address.
 *  \param[in]    StackSize    MPU stack region size.
 *  \param[in]    CurrentPtr   Pointer to current config data. Parameter must not be NULL.
 *  \param[in]    NextPtr      Pointer to next config data. Parameter must not be NULL.
 *  \context      OS_INTERNAL
 *  \reentrant    TRUE
 *  \synchronous  TRUE
 *  \pre          Stack has to be already prepared.
 *  \pre          Future mode has to be already prepared.
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_Hal_ContextGoWithParameterMpSwitch
(
  VAR(uint32, AUTOMATIC) StackStart,
  VAR(uint32, AUTOMATIC) StackSize,
  VAR(uint32, AUTOMATIC) CurrentPtr,
  VAR(uint32, AUTOMATIC) NextPtr
);
 ...
 *********************************************************************************************************************/
/*! \brief        Store current PC, load permanent registers, set PC, LR and switch mode, stack and eventually
 *                stack region.
 *  \details      This is an assembler function.
 *  \param[in]    Next          Pointer to the next context.
 *  \param[in]    Current       Pointer to the current context.
 *  \context      OS_INTERNAL
 *  \reentrant    TRUE
 *  \synchronous  TRUE
 *  \pre          Stack has to be already prepared.
 *  \pre          Future mode has to be already prepared.
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_Hal_ContextRestorePermanentAndGoWithReturn
(
  P2CONST(Os_Hal_ContextType, AUTOMATIC, OS_VAR_NOINIT) Next,
  P2VAR(Os_Hal_ContextType, AUTOMATIC, OS_VAR_NOINIT) Current
);
