...
#ifndef  OS_HAL_COVERAGE_TMS570LS21X_31XINT_H
# define OS_HAL_COVERAGE_TMS570LS21X_31XINT_H
...
\ID COV_OS_HALPLATFORMCONTEXT
  \ACCEPT XF
  \REASON This state can never be true, since it is a do-while-zero construct.
...
\ID COV_OS_HALPLATFORMPFRTUNSUPPORTED_XX
  \ACCEPT XX
  \REASON Timer type PFRT is not available on this derivative but it is tested on reference derivative S6J32xx.
...
\ID COV_OS_HALPLATFORMLEVELSUPPORT
  \ACCEPT XF
  \ACCEPT XX
  \REASON This derivative does not support interrupt level switching.
...
\ID COV_OS_HALPLATFORMNOCENTRALECXEPTIONHANDLER
  \ACCEPT XX
  \REASON This path is never covered. It has no functionality for TMS570 and is never used.

\ID COV_OS_HALPLATFORMCLEARPENDINGAVAILABILITY
  \ACCEPT XX
  \ACCEPT XF
  \REASON This derivative does not meet the hardware requirements to support clearing of pending interrupts.
          Functionality has been tested on another derivative.

\ID COV_OS_HALPLATFORMISRNESTINGSUPPORT_TX
  \ACCEPT TX
  \REASON This derivative does not meet the hardware requirements to support interrupt nesting.
          Functionality has been tested on another derivative.

\ID COV_OS_HALPLATFORMISRNESTINGSUPPORT_XF
  \ACCEPT XF
  \REASON This derivative does not meet the hardware requirements to support interrupt nesting.
          Functionality has been tested on another derivative.
...
\ID COV_OS_HALPLATFORMFRTUNSUPPORTED_XX
  \ACCEPT XX
  \REASON This derivative does not meet the hardware requirements to support timing protection, even if high resolution
          timer feature is available, therefore some timer interfaces, used only for timing protection are not covered.
          Functionality has been tested on another derivative.
...
#endif /* OS_HAL_COVERAGE_TMS570LS21X_31XINT_H */

/*!
 * \}
 */
/***********************************************************************************************************************
 *  END OF FILE: Os_Hal_Coverage_TMS570LS21x_31xInt.h
 **********************************************************************************************************************/
