 ...
 *          @c camera_set_videoencoder_parameter() function if you configured
 *          @c #CAMERA_IMGPROP_VIDEOCODEC to @c #CAMERA_VIDEOCODEC_AVC1 or
 *          @c #CAMERA_VIDEOCODEC_H264 using @c camera_set_video_property().
 *
 *          Use the parameters provided in this file for parameter-value
 *          pairs that you pass to the video encoder configuration functions
 *          in camera_encoder.h.
 */
 ...
 * argument list to video encoder configuration functions.
 */
typedef enum {
    /**
    ...
    CAMERA_H264AVC_QPP,
    /**
     * An end-of-list identifier, which indicates the list of supported video
     * encoder parameters supported by this library.
     */
 ...
 * Use @c camera_get_supported_videoencoder_parameter_values() to determine
 * the supported values at runtime.
 */
typedef enum {
    /**
     * The frame is encoded as a single slice NAL unit
     * (network abstraction layer).
     */
    CAMERA_H264AVC_SLICE_FRAME = 0,
    /**
     * The frame is encoded as multiple slice NAL units. The size
     * of each slice's NAL unit can't exceed the configured
     * @c #CAMERA_H264AVC_SLICESIZE in bytes.
     */
   CAMERA_H264AVC_SLICE_BYTES,
   /**
    * The frame is encoded as multiple slice NAL units. The size
    * of each slice's NAL unit can't exceed the configured number
    * of @c #CAMERA_H264AVC_SLICESIZE macroblocks.
    */
 ...
 * values to use when configuring the @c #CAMERA_H264AVC_ENTROPYCODING
 * parameter.
 *
 * Use @c camera_get_supported_videoencoder_parameter_values() to determine
 * the supported values at runtime.
...
typedef enum {
     ...
     * for some of the supported profiles for H.264/AVC (see
     * @c ::camera_h264avc_profile_t).
     */
...
typedef enum {
    /**
     * No rate control algorithm is used, therefore rate control is disabled
     * (referred to as constant quality encoding). Constant quality encoding
     * means that the bitrate varies with the scene content, but the quality
     * stays constant.
     */
     ...
     * The bitrate that is maintained is specified by the
    ...
    * average bitrate that is maintained is specified by the
    * @c #CAMERA_H264AVC_BITRATE parameter.
   */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/multimedia/branches/ADAS2.0/lib/camapi/public/camera/camera_h264avc.h $ $Rev: 832123 $")
#endif
