...
qnx_libs:
ifeq ($(BUILD_QNX_A72),yes)
	$(MAKE) -C $(PSDK_QNX_PATH)/qnx qnx_libs   VISION_APPS_BUILD_FLAGS_MAK=$(VISION_APPS_BUILD_FLAGS_MAK) QNX_BASE=$(QNX_BASE) PROFILE=$(PROFILE) BOARD=$(BOARD) -s
endif

qnx_bsp:
ifeq ($(BUILD_QNX_A72),yes)
	$(MAKE) -C $(PSDK_QNX_PATH)/qnx bsp         VISION_APPS_BUILD_FLAGS_MAK=$(VISION_APPS_BUILD_FLAGS_MAK) QNX_BASE=$(QNX_BASE) PROFILE=$(PROFILE) BOARD=$(BOARD) -s
endif
...
ifeq ($(BUILD_CPU_C7x_1),yes)
	...
	cp $(VISION_APPS_PATH)/out/$(TARGET_SOC)/$(C7X_TARGET)/$(RTOS_C7x)/$(QNX_APP_PROFILE)/vx_app_rtos_qnx_c7x_1.out $(QNX_AUX_FS_PATH)/lib/firmware/$(QNX_FIRMWARE_PREFIX)-c71_0-fw
