...
HSM_IMAGE = $(wildcard $(ME_REPO_ROOT)/sw/hsm/L2H4060Hsm.out)
...
ifneq ($(HSM_IMAGE),)
	HSM_RPRC := $(SBL_BOOTFILES_PATH)/rprcs/hsm.out.rprc
	REMOTE_CORE_LIST_HSM_APPIMAGE := 23 $(HSM_RPRC)
endif
...
TIFS_BIN_SRC := $(PDK_PATH)/packages/ti/drv/sciclient/soc/$(SCICLIENT_VERSION)/tifs.bin
TIFS_BIN_DST := $(SBL_BOOTFILES_PATH)/tifs.bin

ifeq ($(J7ES_SR), 1_1)
TIFS_BIN_SIGNED_SRC := $(PDK_PATH)/packages/ti/drv/sciclient/soc/$(SCICLIENT_VERSION)/tifs_sr1.1-hs-enc.bin
else
TIFS_BIN_SIGNED_SRC := $(PDK_PATH)/packages/ti/drv/sciclient/soc/$(SCICLIENT_VERSION)/tifs-hs-enc.bin
endif
TIFS_BIN_SIGNED_DST := $(SBL_BOOTFILES_PATH)/tifs.bin.signed

$(TIFS_BIN_DST) : $(TIFS_BIN_SRC)
	mkdir -p $(dir $(TIFS_BIN_DST))
	cp $(TIFS_BIN_SRC) $(TIFS_BIN_DST)

$(TIFS_BIN_SIGNED_DST) : $(TIFS_BIN_SIGNED_SRC)
	mkdir -p $(dir $(TIFS_BIN_SIGNED_DST))
	cp $(TIFS_BIN_SIGNED_SRC) $(TIFS_BIN_SIGNED_DST)

sbl_pdk_sd: $(TIFS_BIN_DST)
...
sbl_pdk_sd_hs: $(TIFS_BIN_SIGNED_DST)
...
sbl_pdk_ospi_hs: $(TIFS_BIN_SIGNED_DST)
...
sbl_pdk_nand: $(TIFS_BIN_DST)
	$(MAKE) -C $(PDK_PATH)/packages/ti/build sbl_qspi_img DISABLE_RECURSE_DEPS=no BOARD=$(BOARD) CORE=$(SBL_CORE) -s
	cp $(PDK_PATH)/packages/ti/boot/sbl/binary/$(BOARD)/qspi/bin/sbl_qspi_img_$(SBL_CORE)_release.tiimage $(SBL_BOOTFILES_PATH)/

sbl_pdk_nand_hs: $(TIFS_BIN_SIGNED_DST)
	$(MAKE) -C $(PDK_PATH)/packages/ti/build sbl_qspi_img_hs DISABLE_RECURSE_DEPS=no BOARD=$(BOARD) CORE=$(SBL_CORE) -s
	cp $(PDK_PATH)/packages/ti/boot/sbl/binary/$(BOARD)_hs/qspi/bin/sbl_qspi_img_$(SBL_CORE)_release.tiimage $(SBL_BOOTFILES_PATH)/sbl_qspi_img_$(SBL_CORE)_release.tiimage.signed

#################
## MCU BootApp
#################

# Due to non-reentrant make in $(MCUSW_PATH)/build and no clean method to avoid
# parallel execution of particular recipes, have to combine all flavors into
# single recipe. Particular flavors are enabled by checking MAKECMDGOALS, and
# a safeguard inside particular flavors goals causes build error if that gets
# wrong

ifeq ($(BOARD), j721s2_hyd)
SBL_BOOTIMAGE_DEPS := sbl_bootimage_sd sbl_bootimage_nand
else
SBL_BOOTIMAGE_DEPS := sbl_bootimage_sd sbl_bootimage_ospi
endif
SBL_BOOTIMAGE_HS_DEPS := $(SBL_BOOTIMAGE_DEPS:%=%_hs)

MCUSW_SD_GOALS := sbl_mcusw_bootimage_sd sbl_bootimage_sd
ifneq (, $(filter sbl_bootimage_sd, $(SBL_BOOTIMAGE_DEPS)))
MCUSW_SD_GOALS += sbl_bootimage
endif
MCUSW_SD_GOALS += $(MCUSW_SD_GOALS:%=%_hs)

MCUSW_OSPI_GOALS := sbl_mcusw_bootimage_ospi sbl_bootimage_ospi
ifneq (, $(filter sbl_bootimage_ospi, $(SBL_BOOTIMAGE_DEPS)))
MCUSW_OSPI_GOALS += sbl_bootimage
endif
MCUSW_OSPI_GOALS += $(MCUSW_OSPI_GOALS:%=%_hs)

MCUSW_NAND_GOALS := sbl_mcusw_bootimage_nand sbl_bootimage_nand
ifneq (, $(filter sbl_bootimage_nand, $(SBL_BOOTIMAGE_DEPS)))
MCUSW_NAND_GOALS += sbl_bootimage
endif
MCUSW_NAND_GOALS += $(MCUSW_NAND_GOALS:%=%_hs)

ifneq (, $(filter $(MCUSW_SD_GOALS), $(MAKECMDGOALS)))
MCUSW_BUILD_SD := yes
endif
ifneq (, $(filter $(MCUSW_OSPI_GOALS), $(MAKECMDGOALS)))
MCUSW_BUILD_OSPI := yes
endif
ifneq (, $(filter $(MCUSW_NAND_GOALS), $(MAKECMDGOALS)))
MCUSW_BUILD_NAND := yes
endif

ifeq ($(BUILD_QNX_A72)-$(BUILD_LINUX_A72), yes-no)
MCUSW_HLOSBOOT := qnx
else ifeq ($(BUILD_QNX_A72)-$(BUILD_LINUX_A72), no-yes)
MCUSW_HLOSBOOT := linux
else
MCUSW_HLOSBOOT := UNKNOWN
endif

sbl_mcusw_bootimage_touch:
	touch $(MCUSW_PATH)/mcuss_demos/boot_app_mcu_rtos/boot.c
	touch $(MCUSW_PATH)/mcuss_demos/boot_app_mcu_rtos/main_rtos.c
	touch $(MCUSW_PATH)/mcuss_demos/boot_app_mcu_rtos/soc/$(SOC)/boot_core_defs.c

sbl_mcusw_bootimage:
	mkdir -p $(SBL_BOOTFILES_PATH)
ifeq ($(MCUSW_BUILD_SD), yes)
	$(MAKE) sbl_mcusw_bootimage_touch
	$(MAKE) -C $(MCUSW_PATH)/build can_boot_app_mcu_rtos -s HLOSBOOT=$(MCUSW_HLOSBOOT) BOOTMODE=mmcsd BOARD=$(BOARD) CORE=$(SBL_CORE) BUILD_OS_TYPE=freertos CANFUNC=none
	cp $(MCUSW_PATH)/binary/can_boot_app_mcu_rtos/bin/$(BOARD)/can_boot_app_mcu_rtos_$(SBL_CORE)_release.appimage $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_sd.appimage
endif
ifeq ($(MCUSW_BUILD_OSPI), yes)
	$(MAKE) sbl_mcusw_bootimage_touch
	$(MAKE) -C $(MCUSW_PATH)/build can_boot_app_mcu_rtos -s HLOSBOOT=$(MCUSW_HLOSBOOT) BOOTMODE=ospi BOARD=$(BOARD) CORE=$(SBL_CORE) BUILD_OS_TYPE=freertos CANFUNC=none
...
ifeq ($(MCUSW_BUILD_NAND), yes)
	$(MAKE) sbl_mcusw_bootimage_touch
	$(MAKE) -C $(MCUSW_PATH)/build can_boot_app_mcu_rtos -s HLOSBOOT=$(MCUSW_HLOSBOOT) BOOTMODE=qspi BOARD=$(BOARD) CORE=$(SBL_CORE) BUILD_OS_TYPE=freertos CANFUNC=none
	cp $(MCUSW_PATH)/binary/can_boot_app_mcu_rtos/bin/$(BOARD)/can_boot_app_mcu_rtos_$(SBL_CORE)_release.appimage $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_nand.appimage
endif

sbl_mcusw_bootimage_sd: sbl_mcusw_bootimage
	@
ifneq ($(MCUSW_BUILD_SD), yes)
	@echo Makefile BUG: sbl_mcusw_bootimage_sd requested but MCUSW_BUILD_SD is not enabled
	@exit 1
endif

sbl_mcusw_bootimage_sd_hs: sbl_mcusw_bootimage_sd
	$(CERT_SCRIPT)  -b $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_sd.appimage -o $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_sd.appimage.signed -c R5 -l 0x0 -k $(PDK_PATH)/packages/ti/build/makerules/k3_dev_mpk.pem

sbl_mcusw_bootimage_ospi: sbl_mcusw_bootimage
	@
ifneq ($(MCUSW_BUILD_OSPI), yes)
	@echo Makefile BUG: sbl_mcusw_bootimage_ospi requested but MCUSW_BUILD_OSPI is not enabled
	@exit 1
endif

sbl_mcusw_bootimage_ospi_hs: sbl_mcusw_bootimage_ospi
...
sbl_mcusw_bootimage_nand: sbl_mcusw_bootimage
	@
ifneq ($(MCUSW_BUILD_NAND), yes)
	@echo Makefile BUG: sbl_mcusw_bootimage_nand requested but MCUSW_BUILD_NAND is not enabled
	@exit 1
endif

sbl_mcusw_bootimage_nand_hs: sbl_mcusw_bootimage_nand
	$(CERT_SCRIPT)  -b $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_nand.appimage -o $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_nand.appimage.signed -c R5 -l 0x0 -k $(PDK_PATH)/packages/ti/build/makerules/k3_dev_mpk.pem
...
ifneq ($(HSM_IMAGE),)
	$(SBL_OUT2RPRC_GEN_TOOL_PATH)/out2rprc.exe $(HSM_IMAGE) $(HSM_RPRC)
	$(MULTICORE_APPIMAGE_GEN_TOOL_PATH)/MulticoreImageGen LE $(DEV_ID) $(SBL_BOOTFILES_PATH)/hsm.appimage $(REMOTE_CORE_LIST_HSM_APPIMAGE)
endif
...
ifeq ($(BUILD_CPU_C7x_1),yes)
	$(CGT7X_ROOT)/bin/strip7x -p $(VISION_APPS_PATH)/out/$(TARGET_SOC)/$(C7X_TARGET)/$(RTOS_C7x)/$(QNX_APP_PROFILE)/vx_app_rtos_qnx_c7x_1.out
...
sbl_qnx_bootimage: sbl_atf_optee
ifeq ($(BUILD_QNX_A72), yes)
...
bootapp_nand: sbl_mcusw_bootimage_nand
bootapp_nand_hs: sbl_mcusw_bootimage_ospi_hs
endif

sbl_bootimage_sd: sbl_pdk_sd bootapp_sd sbl_vision_apps_bootimage sbl_qnx_bootimage
sbl_bootimage_ospi: sbl_pdk_ospi bootapp_ospi sbl_vision_apps_bootimage sbl_qnx_bootimage
sbl_bootimage_nand: sbl_pdk_nand bootapp_nand sbl_vision_apps_bootimage sbl_qnx_bootimage

sbl_bootimage_sd_hs: sbl_pdk_sd_hs bootapp_hs sbl_vision_apps_bootimage_hs sbl_qnx_bootimage_hs
sbl_bootimage_ospi_hs: sbl_pdk_ospi_hs bootapp_ospi_hs sbl_vision_apps_bootimage_hs sbl_qnx_bootimage_hs
sbl_bootimage_nand_hs: sbl_pdk_nand_hs bootapp_nand_hs sbl_vision_apps_bootimage_hs sbl_qnx_bootimage_hs

sbl_bootimage: $(SBL_BOOTIMAGE_DEPS)
sbl_bootimage_hs: $(SBL_BOOTIMAGE_HS_DEPS)
...
sbl_bootimage_install_sd: sbl_vision_apps_bootimage sbl_qnx_bootimage sbl_linux_bootimage
	...
	cp $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_sd.appimage $(SBL_SD_FS_PATH)/app
...
sbl_bootimage_hs_install_sd: sbl_vision_apps_bootimage_hs sbl_qnx_bootimage_hs
	...
	cp $(SBL_BOOTFILES_PATH)/can_boot_app_mcu_rtos_$(SBL_CORE)_release_sd.appimage.signed $(SBL_SD_FS_PATH)/app
