...
ptk_clean:
ifeq ($(BUILD_PTK),yes)
	-$(MAKE) -C $(PTK_PATH) TARGET_SOC=$(TARGET_SOC) RTOS=$(RTOS_C7x) C7X_TARGET=$(C7X_TARGET) clean
endif
...
ifeq ($(BUILD_PTK),yes)
ifeq ($(BUILD_EMULATION_MODE),yes)
	$(MAKE) -C $(PTK_PATH) TARGET_SOC=$(TARGET_SOC) RTOS=$(RTOS_C7x) C7X_TARGET=$(C7X_TARGET) BUILD_TARGET_MODE=no scrub
endif
ifeq ($(BUILD_TARGET_MODE),yes)
	$(MAKE) -C $(PTK_PATH) TARGET_SOC=$(TARGET_SOC) RTOS=$(RTOS_C7x) C7X_TARGET=$(C7X_TARGET) BUILD_TARGET_MODE=yes scrub
endif
endif
