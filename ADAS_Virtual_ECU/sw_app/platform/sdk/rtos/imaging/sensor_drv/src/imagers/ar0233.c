...
/* AR0233 (onsemi):   ti9x4.dvp_bus=0 ti9x4.ser_id=0x30 ti9x4.gpio0=1 ti9x4.gpio1=2 ov106xx.trigger=3 */
...
static IssSensor_CreateParams  ar0233_CreatePrms = {
	...
	.sensorInfo = {
		.raw_params = {
			.width = AR0233_DEFAULT_WIDTH,
			.height = AR0233_DEFAULT_HEIGHT,
			.num_exposures = 1,
...
#define AR0233_MAX_CONT_I2C_WRITE	128
...
static int ar0233_set_regs(struct i2c_client *client, const struct ar0233_reg **pregs)
{
	...
	int ret = 0;
	int base = 0x0000;
	uint8_t buf[AR0233_MAX_CONT_I2C_WRITE + 2];
	...
	for (j = 0; ; j++) {
		...
		for (i = 0; ; i++) {
			...
			if (((base + l) != regs[i].reg) || (l >= AR0233_MAX_CONT_I2C_WRITE) || (regs[i].reg == AR0233_DELAY)) {
				if (l == AR0233_MAX_CONT_I2C_WRITE)
					dev_info(&client->dev, "chain is full @ 0x%04x!\n", base);
				if (l) {
					/* write chained */
					ret = i2c_tx_array(client, buf, l + 2);
					if (ret)
						mdelay(1);
					l = 0;
					base = 0x0000;
				...
				buf[2] = regs[i].val >> 8 & 0xff;
				buf[3] = regs[i].val >> 0 & 0xff;
				base = regs[i].reg;
				l = 2;
			} else {
				buf[2 + l]     = regs[i].val >> 8 & 0xff;
				buf[2 + l + 1] = regs[i].val >> 0 & 0xff;
				l+=2;
			}
		}
	}
...
static int ar0233_detect(struct i2c_client *client, void *camera)
{
	...
	for (i = 0; i < ARRAY_SIZE(ar0233_i2c_addr); i++) {
		if (client->addr == 0x6f)
			ret = ser_camera_i2c_set_mapping(camera, 1, client->addr, ar0233_i2c_addr[i]);
		else
...
#ifdef BANDWIDTH_WORKAROUND
	/* impement delay between frames */
	reg16_read16(client, 0x300A, &val);	// read VTS
	val += 0x10 * client->idx;
	reg16_write16(client, 0x300A, val);
#endif
