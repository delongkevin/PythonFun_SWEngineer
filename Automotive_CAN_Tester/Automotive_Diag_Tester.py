...
CONFIG_FILE = "can_diag_automation_config.json"

class VectorCanDiagAutomation:
    def __init__(self, root):
        self.root = root
        self.root.title("Vector CAN/CAN-FD - Test Automation & Sequencer")
        ...
        self.dbs = {0: None, 1: None} 
        
        self.log_filename = f"can_log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        
        # Split queue into a Bank (storage) and a Script (execution)
        self.config = {
            "bitrate": "500000",
            ...
            "fc_rx_id": "7E8",
            "fc_tx_id": "7E0",
            "bank_data": [],   # Pool of available commands
            "script_data": []  # The actual sequence to run
        }
        self.load_config()
        self.setup_gui()
    ...
    def setup_gui(self):
        ...
        ttk.Label(hw_frame, text="Bitrate:").grid(row=0, column=0, padx=2, pady=5, sticky="e")
        self.bitrate_entry = ttk.Entry(hw_frame, width=8)
        self.bitrate_entry.grid(row=0, column=1, padx=2, pady=5)

        ttk.Label(hw_frame, text="FD Bitrate:").grid(row=0, column=2, padx=2, pady=5, sticky="e")
        ...
        ttk.Checkbutton(hw_frame, text="CAN-FD", variable=self.is_fd_var).grid(row=0, column=4, padx=5, pady=5)

        # Flow Control inside Hardware box to save space
        self.is_fc_enabled = tk.BooleanVar()
        ttk.Checkbutton(hw_frame, text="Auto Flow-Control", variable=self.is_fc_enabled).grid(row=1, column=0, columnspan=2, sticky="w", padx=5)
        ...
        self.btn_connect = ttk.Button(hw_frame, text="Connect Hardware", command=self.connect_hardware)
        self.btn_connect.grid(row=0, column=6, rowspan=2, padx=15, pady=5, ipady=10)
        self.btn_disconnect = ttk.Button(hw_frame, text="Disconnect", command=self.disconnect_hardware, state="disabled")
        ...
        ttk.Label(db_frame, text="Map DBC to:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
        self.dbc_map_cb = ttk.Combobox(db_frame, values=["Channel Index 0", "Channel Index 1"], state="readonly", width=15)
        self.dbc_map_cb.current(0)
        self.dbc_map_cb.grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(db_frame, text="Load .dbc", command=self.load_dbc).grid(row=0, column=2, padx=5, pady=5)
        ...
        ttk.Button(db_frame, text="Import Diag Log (.asc/.txt)", command=self.import_log).grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky="ew")
        ...
        ttk.Label(tx_frame, text="ID (Hex):").grid(row=1, column=0, padx=2, sticky="e", pady=5)
        self.arb_id_entry = ttk.Entry(tx_frame, width=8)
        self.arb_id_entry.grid(row=1, column=1, padx=2, sticky="w", pady=5)

        ttk.Label(tx_frame, text="Payload (Hex):").grid(row=1, column=2, padx=2, sticky="e", pady=5)
        self.data_entry = ttk.Entry(tx_frame, width=25)
        self.data_entry.grid(row=1, column=3, padx=2, sticky="w", pady=5)

        ttk.Button(tx_frame, text="Send Now", command=lambda: self.send_command()).grid(row=2, column=1, pady=5)
        ttk.Button(tx_frame, text="Save to Bank \u2193", command=self.add_to_bank).grid(row=2, column=3, sticky="w", pady=5)

        # Command Bank
        bank_frame = ttk.LabelFrame(left_pane, text="2. Command Bank (Available Messages)")
        ...
        self.search_var.trace("w", self.filter_bank) # Triggers filtering on keystroke
        ttk.Entry(search_frame, textvariable=self.search_var, width=30).pack(side="left", padx=5)

        self.bank_tree = ttk.Treeview(bank_frame, columns=("Name", "ID", "Data"), show="headings", height=8)
        ...
        ttk.Button(b_btn_frame, text=">> Add to Script >>", command=self.move_to_script).pack(pady=2, ipady=5)
        ttk.Button(b_btn_frame, text="Edit", command=self.edit_bank_item).pack(pady=2)
        ttk.Button(b_btn_frame, text="Delete", command=self.delete_from_bank).pack(pady=2)
        ttk.Button(b_btn_frame, text="Clear Bank", command=self.clear_bank).pack(side="bottom", pady=2)

        # === RIGHT PANE: TEST SCRIPT QUEUE ===
        right_pane = tk.Frame(paned_window)
        paned_window.add(right_pane, weight=1)

        script_frame = ttk.LabelFrame(right_pane, text="3. Test Script Queue (Execution Sequence)")
        script_frame.pack(fill="both", expand=True, padx=(5, 0))

        self.script_tree = ttk.Treeview(script_frame, columns=("Name", "ID", "Data"), show="headings", height=12)
        ...
        self.btn_run_script = ttk.Button(s_btn_frame, text="\u25B6 Run Script", command=self.start_script, state="disabled")
        self.btn_run_script.pack(pady=2, ipady=5)
        
        self.btn_stop_script = ttk.Button(s_btn_frame, text="\u23F9 Stop", command=self.stop_script, state="disabled")
        self.btn_stop_script.pack(pady=2)
        
        ttk.Separator(s_btn_frame, orient='horizontal').pack(fill='x', pady=10)
        
        ttk.Button(s_btn_frame, text="Move Up", command=lambda: self.move_item(-1)).pack(pady=2)
        ttk.Button(s_btn_frame, text="Move Down", command=lambda: self.move_item(1)).pack(pady=2)
        ttk.Button(s_btn_frame, text="Remove Step", command=self.remove_from_script).pack(pady=2)
        ttk.Button(s_btn_frame, text="Clear Script", command=self.clear_script).pack(pady=2)
        
        ttk.Button(s_btn_frame, text="\U0001F4BE Save Layout", command=self.force_save_config).pack(side="bottom", pady=2)

        # --- Bottom Frame: Logging ---
        log_frame = ttk.LabelFrame(self.root, text="Traffic Log (Parsed UDS & ASCII)")
    ...
    def apply_config_to_gui(self):
        ...
        for item in self.config.get("script_data", []):
            self.script_tree.insert("", "end", values=(item["name"], item["id"], item["data"]))
    ...
    def force_save_config(self):
        ...
        for child in self.script_tree.get_children():
            ...
            self.config["script_data"].append({"name": name, "id": arb_id, "data": data})
            
        self.save_config() # bank_data is updated on the fly
        self.write_log("Configuration and Sequences saved successfully.")
        
    # --- BANK & SCRIPT DATA MANAGEMENT ---

    def refresh_bank_tree(self, filter_text=""):
        self.bank_tree.delete(*self.bank_tree.get_children())
        filter_text = filter_text.lower()
        for item in self.config.get("bank_data", []):
            if filter_text in item["name"].lower() or filter_text in item["id"].lower() or filter_text in item["data"].lower():
                self.bank_tree.insert("", "end", values=(item["name"], item["id"], item["data"]))

    def filter_bank(self, *args):
        self.refresh_bank_tree(self.search_var.get())

    def add_to_bank(self):
        arb_id = self.arb_id_entry.get().strip()
        data = self.data_entry.get().strip()
        name = self.dbc_msg_cb.get() if self.dbc_msg_cb.get() and self.dbc_msg_cb.get() != "No DBC mapped" else f"Custom {arb_id}"
        if not arb_id or not data: return
        self.config["bank_data"].append({"name": name, "id": arb_id, "data": data})
        self.refresh_bank_tree(self.search_var.get())

    def move_to_script(self):
        """Copies selected items from Bank to Script."""
    ...
    def delete_from_bank(self):
        selected = self.bank_tree.selection()
        if not selected: return
        
        # Build a set of selected values to remove from the backing list
        ...
        self.config["bank_data"] = new_bank
        self.refresh_bank_tree(self.search_var.get())

    def clear_bank(self):
        if messagebox.askyesno("Confirm", "Are you sure you want to clear the entire Command Bank?"):
            self.config["bank_data"] = []
            self.refresh_bank_tree()

    def remove_from_script(self):
        for item in self.script_tree.selection(): self.script_tree.delete(item)

    def clear_script(self):
        for item in self.script_tree.get_children(): self.script_tree.delete(item)

    def move_item(self, direction):
        """Moves an item up (-1) or down (1) in the script tree."""
    ...
    def edit_bank_item(self, event=None):
        self._create_editor_popup(self.bank_tree, True)

    def edit_script_item(self, event=None):
        self._create_editor_popup(self.script_tree, False)

    def _create_editor_popup(self, treeview, is_bank):
        ...
        data_entry = ttk.Entry(edit_win, width=35)
        data_entry.insert(0, current_data)
        data_entry.pack(pady=2)

        def save_edits():
            new_name = name_entry.get().strip()
            new_id = id_entry.get().strip().replace('x', '').replace('X', '')
            new_data = data_entry.get().strip()
            
            if not new_id or not new_data: return
                
            treeview.item(item_id, values=(new_name, new_id, new_data))
            
            # If editing the bank, update the backing data list
            if is_bank:
                # We need to find the specific item by old values and replace it. 
                # (Simple approach: just reconstruct the bank list from the treeview)
                self.config["bank_data"] = []
                for child in self.bank_tree.get_children():
                    n, i, d = self.bank_tree.item(child, 'values')
                    self.config["bank_data"].append({"name": n, "id": i, "data": d})
    ...
    def start_script(self):
        if not self.is_connected or self.is_script_running: return
        if not self.script_tree.get_children(): return

        try:
            delay_ms = int(self.delay_entry.get())
        except ValueError:
            messagebox.showerror("Error", "Delay must be an integer (milliseconds).")
            return
        ...
        threading.Thread(target=self._run_script_thread, args=(delay_ms,), daemon=True).start()

    def _run_script_thread(self, delay_ms):
        delay_sec = delay_ms / 1000.0
        
        for child in self.script_tree.get_children():
            if not self.is_script_running: break
            
            # Highlight item in GUI thread securely
            self.root.after(0, lambda c=child: self.script_tree.selection_set(c))
            self.root.after(0, lambda c=child: self.script_tree.see(c))
        ...
        self.root.after(0, self._finalize_script)

    def _finalize_script(self):
        self.is_script_running = False
    ...
    # --- IMPORT & HARDWARE (Unchanged core logic) ---

    def import_log(self):
        filepath = filedialog.askopenfilename(filetypes=[("Vector Logs", "*.asc *.txt"), ("All Files", "*.*")])
                    ...
                    if ' Tx ' in line and '[' in line and ']' in line:
                        ...
                        try:
                            ...
                            if key not in seen and payload:
                                ...
                                name = "UDS Command"
                                if sid == '10': name = "Session Control (10)"
                                elif sid == '11': name = "ECU Reset (11)"
                                elif sid == '14': name = "Clear DTCs (14)"
                                elif sid == '19': name = "Read DTCs (19)"
                                elif sid == '22': name = f"Read Data (22) {payload.replace(' ', '')[2:6]}"
                                elif sid == '2E': name = "Write Data (2E)"
                                elif sid == '31': name = "Routine Control (31)"
                                elif sid == '3E': name = "Tester Present (3E)"
                                
                                self.config["bank_data"].append({"name": name, "id": can_id, "data": payload})
                                added += 1
                        except Exception: continue
            
            self.refresh_bank_tree()
            self.write_log(f">> Log Imported: Added {added} unique Tx commands to Bank.")
            
        except Exception as e:
            messagebox.showerror("Import Error", f"Failed to parse log:\n{e}")
    ...
    def process_dbc(self, filepath, ch_index):
        try:
            self.dbs[ch_index] = cantools.database.load_file(filepath)
            filename = os.path.basename(filepath)
            if ch_index == 0: self.dbc_label_0.config(text=f"Ch 0: {filename}", foreground="blue")
            else: self.dbc_label_1.config(text=f"Ch 1: {filename}", foreground="blue")
        except Exception: pass

    def on_tx_channel_change(self, event):
        if not self.tx_channel_cb.get(): return
    ...
    def on_dbc_msg_select(self, event):
    ...
    def connect_hardware(self):
        ...
        try:
            ...
            if len(configs) < 2: raise ValueError("Minimum of 2 Vector channels required.")
            ...
            for i, cfg in enumerate(configs[:2]):
                ...
                bus = can.interface.Bus(
                    interface='vector', channel=ch_id,
                    bitrate=int(self.bitrate_entry.get()),
                    fd=self.is_fd_var.get(),
                    data_bitrate=int(self.data_bitrate_entry.get()) if self.is_fd_var.get() else None
                )
                self.buses.append(bus)
                self.vector_channels.append(ch_id)
                channel_names.append(f"Vector Ch {ch_id} (Idx {i})")
                threading.Thread(target=self.rx_loop, args=(bus, ch_id, i), daemon=True).start()
    ...
    def disconnect_hardware(self):
        for bus in self.buses: bus.shutdown()
        self.buses.clear()
    ...
    def rx_loop(self, bus, channel_id, list_index):
                ...
                if data_bytes:
                    ...
                    if uds_payload:
                        if pci_type in (0, 1):
                            sid = uds_payload[0]
                            if sid == 0x7F:
                                nrc = uds_payload[2] if len(uds_payload) > 2 else 0x00
                                uds_status = f" | ❌ FAIL (7F) [NRC: {nrc:02X}]"
                            elif 0x50 <= sid <= 0x7E:
                                uds_status = " | ✅ PASS"
                        
                        ascii_chars = [chr(b) if 32 <= b <= 126 else '.' for b in uds_payload]
                        ascii_candidate = "".join(ascii_chars).replace('.', '')
                        if re.search(r'[a-zA-Z0-9]{3,}', ascii_candidate):
                            ascii_trans = f" | ASCII: '{ascii_candidate}'"
                
                self.write_log(f"RX [Ch {channel_id}] | ID: {msg.arbitration_id:X} | Data: {hex_out}{uds_status}{ascii_trans}")

                if self.is_fc_enabled.get() and len(data_bytes) > 0:
                    try:
                        watch_id = int(self.fc_rx_entry.get(), 16)
                        reply_id = int(self.fc_tx_entry.get(), 16)
                        
                        if msg.arbitration_id == watch_id and (data_bytes[0] & 0xF0) == 0x10:
                            fc_payload = bytes([0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
                            fc_msg = can.Message(
                                arbitration_id=reply_id, data=fc_payload,
                                is_extended_id=(reply_id > 0x7FF), is_fd=self.is_fd_var.get(),
                                bitrate_switch=self.is_fd_var.get()
                            )
                            bus.send(fc_msg)
                            self.write_log(f"TX [Ch {channel_id}] (Auto-FC) | ID: {reply_id:X} | Data: 30 00 00 00 00 00 00 00")
                    except ValueError: pass
            except Exception:
                break

    def send_command(self, arb_id_str=None, data_str=None):
        ...
        try:
            ...
            msg = can.Message(
                arbitration_id=arb_id, data=data_bytes,
                is_extended_id=(arb_id > 0x7FF), is_fd=is_fd, bitrate_switch=is_fd
            )
            target_bus.send(msg)
            hex_out = " ".join(f"{b:02X}" for b in data_bytes)
            self.write_log(f"TX [Ch {hw_channel_id}] | ID: {arb_id:X} | Data: {hex_out}")

        except ValueError:
            pass # Avoid throwing popups during script loop
        except Exception as e:
            print(f"TX Error: {e}")

if __name__ == "__main__":
    root = tk.Tk()
    app = VectorCanDiagAutomation(root)
    def on_closing():
        if app.is_connected: app.disconnect_hardware()
        app.force_save_config()
        root.destroy()
